...
**30 秒电梯演讲：** 就像在64个仓库里找一个客户的货物——逐个翻找太慢，要么同时派人去找，要么提前整理好清单。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【滴滴面经】极端情况下，一个用户的短链可能分布在 64 张表 | "就像在64个仓库里找一个客户的货物——逐个翻找太慢，要么同时派人去找，要么提前整理好清单。" | 引入 |
| 0:20 | 概念图解 | "+合并" | 并发查询64张表 |
| 0:45 | 对比表格 | "表" | 引入用户维度路由 |
| 1:15 | 代码截图 | "D+短码双写）" | 数据冗余（用户I |
| 2:15 | 总结卡 | "记住三个词：并发查询64张表、引入用户维度路由、数据冗余（用户I" | 收尾 |
