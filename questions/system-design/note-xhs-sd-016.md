...
**30 秒电梯演讲：** 就像演唱会售票——检票口（Redis）用原子操作瞬间判断还有没有票，有票就给你一个号码牌（消息队列），你在等候区等叫号正式出票（异步创建订单）。检票口不等待出票，所以吞吐量极高。

**展开框架：**
1. **Redis+Lu** — a保证库存扣减的原子性（CHECK + DEDUCT一步完成）
2. **消息队列异步创建订单** — 解耦扣减和落库
3. **三层防护** — Redis预扣减 → MQ异步落库 → DB乐观锁兜底
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多一面】大促时如何用 Redis+消息队列做库存扣减与 | "就像演唱会售票——检票口（Redis）用原子操作瞬间判断还有没有票，有票就给你一个号码牌（消息队列）" | 引入 |
| 0:20 | 概念图解 | "a保证库存扣减的原子性（CHECK + DEDUCT一步完成）" | Redis+Lu |
| 0:45 | 对比表格 | "解耦扣减和落库" | 消息队列异步创建订单 |
| 1:15 | 代码截图 | "Redis预扣减 → MQ异步落库 → DB乐观锁兜底" | 三层防护 |
| 2:15 | 总结卡 | "记住三个词：Redis+Lu、消息队列异步创建订单、三层防护" | 收尾 |
