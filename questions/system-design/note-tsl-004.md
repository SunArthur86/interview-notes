...
**30 秒电梯演讲：** 像打车软件派单——用户发出充电请求，系统像"调度中心"一样在地图上找最近的空闲充电桩（打车找最近的车），同时考虑排队人数和电价，推荐最优选择。

**展开框架：**
1. **Redis** — GEO实现O(logN)附近搜索
2. **实时状态通过MQ** — /长连接同步
3. **调度算法兼顾距离** — 和负载均衡

**收尾：** 多个用户同时抢占同一个充电桩怎么办？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：全球数万座充电桩实时调度，如何设计后端架构，实现用户就近分配 | "像打车软件派单——用户发出充电请求，系统像"调度中心"一样在地图上找最近的空闲充电桩（打车找最近的车" | 引入 |
| 0:20 | 概念图解 | "GEO实现O(logN)附近搜索" | Redis |
| 0:45 | 对比表格 | "/长连接同步" | 实时状态通过MQ |
| 1:15 | 代码截图 | "和负载均衡" | 调度算法兼顾距离 |
| 2:15 | 总结卡 | "记住三个词：Redis、实时状态通过MQ、调度算法兼顾距离" | 收尾 |
