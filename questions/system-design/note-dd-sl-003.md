...
**30 秒电梯演讲：** 就像图书馆更新目录卡——你先换新书，再扔掉旧目录卡，下次有人查时自然去拿新卡。

**展开框架：**
1. **Cache** — Aside（先DB后删缓存）
2. **延迟双删** — 延迟双删
3. **消息队列保证最终** — 一致

**收尾：** 如果先删缓存再更新DB，会有什么问题？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【滴滴面经】怎么保证 Redis 里的短链数据和 DB 一致 | "就像图书馆更新目录卡——你先换新书，再扔掉旧目录卡，下次有人查时自然去拿新卡。" | 引入 |
| 0:20 | 概念图解 | "Aside（先DB后删缓存）" | Cache |
| 0:45 | 对比表格 | "延迟双删" | 延迟双删 |
| 1:15 | 代码截图 | "一致" | 消息队列保证最终 |
| 2:15 | 总结卡 | "记住三个词：Cache、延迟双删、消息队列保证最终" | 收尾 |
