...
**30 秒电梯演讲：** 寄快递——生产者寄件（要确认快递收了），快递中转站（包裹不能丢），收件人签收（不能说没收到）。任何一环疏忽包裹就丢了。

**展开框架：**
1. **生产者端** — 同步发送(sync)替代异步(oneway)，失败重试
2. **Broker端** — 同步刷盘(SYNC_FLUSH)替代异步刷盘，主从同步复制
3. **消费者端** — 手动提交offset(非自动)，消费失败重试+死信队列

**收尾：** RocketMQ的同步刷盘和异步刷盘性能差多少？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【大厂二面】RocketMQ 消息丢失怎么处理？ | "寄快递——生产者寄件（要确认快递收了），快递中转站（包裹不能丢），收件人签收（不能说没收到）。任何一" | 引入 |
| 0:20 | 概念图解 | "同步发送(sync)替代异步(oneway)，失败重试" | 生产者端 |
| 0:45 | 对比表格 | "同步刷盘(SYNC_FLUSH)替代异步刷盘，主从同步复制" | Broker端 |
| 1:15 | 代码截图 | "手动提交offset(非自动)，消费失败重试+死信队列" | 消费者端 |
| 1:45 | 总结卡 | "记住三个词：生产者端、Broker端、消费者端" | 收尾 |
