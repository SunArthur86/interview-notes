...
**30 秒电梯演讲：** 传统语义检索像用"关键词匹配"抓小偷——搜"买了手机"会召回所有买手机的人(大部分是正常用户)。风控检索像用"行为画像匹配"——不只看买了什么，还看购买时间、频率、设备指纹、支付方式组合，找到"行为模式像已知黑产"的人。

**展开框架：**
1. **核心转变** — 从"语义相似度"到"行为模式相似度"
2. **特征工程** — 交易序列编码 + 设备/网络/时间多维特征 + 频率/金额统计
3. **检索策略** — 行为向量检索 + 规则引擎过滤 + 异常分数排序
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：设计信贷风控场景的检索系统 | "传统语义检索像用"关键词匹配"抓小偷——搜"买了手机"会召回所有买手机的人(大部分是正常用户)。风控" | 引入 |
| 0:20 | 概念图解 | "从"语义相似度"到"行为模式相似度"" | 核心转变 |
| 0:45 | 对比表格 | "交易序列编码 + 设备/网络/时间多维特征 + 频率/金额统计" | 特征工程 |
| 1:15 | 代码截图 | "行为向量检索 + 规则引擎过滤 + 异常分数排序" | 检索策略 |
| 2:15 | 总结卡 | "记住三个词：核心转变、特征工程、检索策略" | 收尾 |
