...
**30 秒电梯演讲：** 像报社发新闻——记者写好稿子（数据源变更），编辑发到通讯社（CDC+MQ），报纸、网站、APP各自从通讯社取稿子（下游消费），格式各不相同但源头只有一个。

**展开框架：**
1. **CDC核心** — 监听Binlog而非定时轮询，实现毫秒级延迟
2. **架构** — 数据源 → CDC工具(Canal/Debezium) → MQ → 多下游消费者
3. **每个下游维护自己** — 的消费offset，互不影响
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节一面】基于 CDC 思想设计一个数据同步系统：单一数据 | "像报社发新闻——记者写好稿子（数据源变更），编辑发到通讯社（CDC+MQ），报纸、网站、APP各自从" | 引入 |
| 0:20 | 概念图解 | "监听Binlog而非定时轮询，实现毫秒级延迟" | CDC核心 |
| 0:45 | 对比表格 | "数据源 → CDC工具(Canal/Debezium) → MQ → 多下游消费者" | 架构 |
| 1:15 | 代码截图 | "的消费offset，互不影响" | 每个下游维护自己 |
| 2:15 | 总结卡 | "记住三个词：CDC核心、架构、每个下游维护自己" | 收尾 |
