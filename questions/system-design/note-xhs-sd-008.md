...
**30 秒电梯演讲：** AI网关像国际快递公司。你只需要说「寄到国外」，快递公司（网关）自动选择走空运(OpenAI快)、海运(通义便宜)还是陆运(文心)。你不用关心每家航空公司的规定，快递公司帮你处理所有差异。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何设计多 AI 厂商接口统一适配方案？（入职Java复盘） | "AI网关像国际快递公司。你只需要说「寄到国外」，快递公司（网关）自动选择走空运(OpenAI快)、海" | 引入 |
| 0:20 | 概念图解 | "统一接口+各厂商Adapter实现" | 适配器模式 |
| 0:45 | 对比表格 | "按任务类型/成本/质量/延迟选模型" | 路由策略 |
| 1:15 | 代码截图 | "failover chain自动切换厂商" | 降级容错 |
| 2:15 | 总结卡 | "记住三个词：适配器模式、路由策略、降级容错" | 收尾 |
