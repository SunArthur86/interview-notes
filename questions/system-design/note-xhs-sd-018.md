...
**30 秒电梯演讲：** 拼团就像组织团购——团长开团（创建拼团），成员加入（锁定库存），人齐了才算成功（扣减库存），有人退出要释放库存。防黄牛（防刷）、控制排队（限流）、保证不多卖不少卖（库存一致性）是三大核心。

**展开框架：**
1. **拼团状态机** — 开团→待成团→已成团/已失败（超时）
2. **库存分两层** — 团维度库存（限制开团数量）+ 商品维度库存（限制总销量）
3. **防刷** — 用户ID+设备指纹+IP三维度限流 + 风控规则

**收尾：** 拼团超时自动失败的定时任务，如果任务执行慢导致库存长时间不释放，怎么办？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多二面】设计一个拼团活动的抢购系统（防刷、限流、库存一 | "拼团就像组织团购——团长开团（创建拼团），成员加入（锁定库存），人齐了才算成功（扣减库存），有人退出" | 引入 |
| 0:20 | 概念图解 | "开团→待成团→已成团/已失败（超时）" | 拼团状态机 |
| 0:45 | 对比表格 | "团维度库存（限制开团数量）+ 商品维度库存（限制总销量）" | 库存分两层 |
| 1:15 | 代码截图 | "用户ID+设备指纹+IP三维度限流 + 风控规则" | 防刷 |
| 2:15 | 总结卡 | "记住三个词：拼团状态机、库存分两层、防刷" | 收尾 |
