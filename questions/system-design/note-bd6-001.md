...
**30 秒电梯演讲：** 像搬家——不可能一次性把所有东西塞一辆车（OOM），要分批次打包（分片），多辆车同时搬（并行），每车到了签收（校验），中途歇了知道从哪继续（续传）。

**展开框架：**
1. **数据切分** — 按ID范围或Hash分片，每片10-50万行
2. **并行处理** — 线程池并发处理多个分片，控制并发度防OOM
3. **流式写入** — SXSSF或EasyExcel流式写，不全加载到内存
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节一面】使用普通数据库和Java原生手段，设计一个千万级 | "像搬家——不可能一次性把所有东西塞一辆车（OOM），要分批次打包（分片），多辆车同时搬（并行），每车" | 引入 |
| 0:20 | 概念图解 | "按ID范围或Hash分片，每片10-50万行" | 数据切分 |
| 0:45 | 对比表格 | "线程池并发处理多个分片，控制并发度防OOM" | 并行处理 |
| 1:15 | 代码截图 | "SXSSF或EasyExcel流式写，不全加载到内存" | 流式写入 |
| 2:15 | 总结卡 | "记住三个词：数据切分、并行处理、流式写入" | 收尾 |
