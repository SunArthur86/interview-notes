...
**30 秒电梯演讲：** 评论系统就像一个万人演唱会的大屏幕互动——(1)分片存储=多个大屏幕(分库分表)各管一个区域；(2)读写分离=观众看屏幕(读)和发消息(写)走不同通道；(3)消息队列削峰=先收集到中转站(Kafka)，再一批批显示到大屏幕；(4)多级...

**展开框架：**
1. **写入挑战** — 高并发评论写入（热门视频每秒数千条）
2. **读取挑战** — 实时展示最新评论+分页加载历史评论
3. **分片策略** — 按video_id哈希分片，保证同一视频的评论在同一分片
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【快手Java一面】如何设计快手评论系统，确保高并发写入和实 | "评论系统就像一个万人演唱会的大屏幕互动——(1)分片存储=多个大屏幕(分库分表)各管一个区域；(2)" | 引入 |
| 0:20 | 概念图解 | "高并发评论写入（热门视频每秒数千条）" | 写入挑战 |
| 0:45 | 对比表格 | "实时展示最新评论+分页加载历史评论" | 读取挑战 |
| 1:15 | 代码截图 | "按video_id哈希分片，保证同一视频的评论在同一分片" | 分片策略 |
| 2:15 | 总结卡 | "记住三个词：写入挑战、读取挑战、分片策略" | 收尾 |
