...
**30 秒电梯演讲：** 像一家超大型翻译公司——仓库（S3）堆满了待翻译的文件，项目经理（调度引擎）把文件分给翻译员（标注员），质检员（质量校验）抽查翻译质量，不合格的打回重做。

**展开框架：**
1. **S3/HDFS对** — 象存储管理PB级原始数据
2. **工作流引擎调度标** — 注任务(分配/回收)
3. **自动标注(模型预** — 标注)+人工修正
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：PB级路测数据需人工+自动标注，如何设计后端架构，支持标注任 | "像一家超大型翻译公司——仓库（S3）堆满了待翻译的文件，项目经理（调度引擎）把文件分给翻译员（标注员" | 引入 |
| 0:20 | 概念图解 | "象存储管理PB级原始数据" | S3/HDFS对 |
| 0:45 | 对比表格 | "注任务(分配/回收)" | 工作流引擎调度标 |
| 1:15 | 代码截图 | "标注)+人工修正" | 自动标注(模型预 |
| 2:15 | 总结卡 | "记住三个词：S3/HDFS对、工作流引擎调度标、自动标注(模型预" | 收尾 |
