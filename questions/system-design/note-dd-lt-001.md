...
**30 秒电梯演讲：** 就像超市最后一件商品——如果10个人同时抢，你必须在仓库门上加一把锁，保证只有第一个人能拿到。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【滴滴面经】抽奖场景里，奖品库存超卖是怎么控制的？ | "就像超市最后一件商品——如果10个人同时抢，你必须在仓库门上加一把锁，保证只有第一个人能拿到。" | 引入 |
| 0:20 | 概念图解 | "Lua原子扣减" | Redis |
| 0:45 | 对比表格 | "DB乐观锁兜底" | DB乐观锁兜底 |
| 1:15 | 代码截图 | "isson）" | 分布式锁（Red |
| 2:15 | 总结卡 | "记住三个词：Redis、DB乐观锁兜底、分布式锁（Red" | 收尾 |
