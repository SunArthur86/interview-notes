...
**30 秒电梯演讲：** 就像超市扫码结账——同一件商品你扫多少次码，系统都只收一次钱。条形码就是"业务流水号"，收银系统检查"这个码扫过了没"就是去重。

**展开框架：**
1. **幂等三方案** — 去重表(通用) / 业务状态机(有状态流转) / 数据库唯一约束(简单)
2. **去重表设计** — idempotent_key(PK) + status + created_at + 业务字段
3. **并发处理** — 数据库行锁(SELECT FOR UPDATE) / 乐观锁(版本号) / Redis分布式锁

**收尾：** 如果Redis宕机导致分布式锁失效怎么办？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：设计MQ幂等消费函数 | "就像超市扫码结账——同一件商品你扫多少次码，系统都只收一次钱。条形码就是"业务流水号"，收银系统检查" | 引入 |
| 0:20 | 概念图解 | "去重表(通用) / 业务状态机(有状态流转) / 数据库唯一约束(简单)" | 幂等三方案 |
| 0:45 | 对比表格 | "idempotent_key(PK) + status + created_at + 业务字段" | 去重表设计 |
| 1:15 | 代码截图 | "数据库行锁(SELECT FOR UPDATE) / 乐观锁(版本号) / Redis分布式锁" | 并发处理 |
| 1:45 | 总结卡 | "记住三个词：幂等三方案、去重表设计、并发处理" | 收尾 |
