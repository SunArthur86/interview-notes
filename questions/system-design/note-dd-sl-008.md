...
**30 秒电梯演讲：** 就像商场分区导览图——你知道店名但不知道在哪个楼层，路由表就是那个导览图。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【滴滴面经】短链接数据分片之后，你又加了路由表，这个路由表具 | "就像商场分区导览图——你知道店名但不知道在哪个楼层，路由表就是那个导览图。" | 引入 |
| 0:20 | 概念图解 | "h%N）" | 哈希路由（has |
| 0:45 | 对比表格 | "一致性哈希" | 一致性哈希 |
| 1:15 | 代码截图 | "射）" | 路由表（元数据映 |
| 2:15 | 总结卡 | "记住三个词：哈希路由（has、一致性哈希、路由表（元数据映" | 收尾 |
