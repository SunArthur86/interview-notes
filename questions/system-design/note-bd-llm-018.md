...
**30 秒电梯演讲：** 就像大型医院——不同科室(产品线)有独立药房(知识库隔离)，不同病人有不同就医权限(多租户)，还有医疗质量监控部(可观测性)。

**展开框架：**
1. **多产品线** — 独立向量库+路由层分发
2. **多租户** — RBAC+数据行级隔离
3. **知识库隔离** — 租户级Collection/namespace
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】设计一个企业内部的智能客服系统，支持多产品线知识 | "就像大型医院——不同科室(产品线)有独立药房(知识库隔离)，不同病人有不同就医权限(多租户)，还有医" | 引入 |
| 0:20 | 概念图解 | "独立向量库+路由层分发" | 多产品线 |
| 0:45 | 对比表格 | "RBAC+数据行级隔离" | 多租户 |
| 1:15 | 代码截图 | "租户级Collection/namespace" | 知识库隔离 |
| 2:15 | 总结卡 | "记住三个词：多产品线、多租户、知识库隔离" | 收尾 |
