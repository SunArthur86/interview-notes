...
**30 秒电梯演讲：** 就像装修时把所有电线焊死在墙里——要加一个插座就得砸墙。规则引擎就是可插拔的电线管道。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【滴滴面经】你会把付费用户加权这种规则直接写死在抽奖流程里吗 | "就像装修时把所有电线焊死在墙里——要加一个插座就得砸墙。规则引擎就是可插拔的电线管道。" | 引入 |
| 0:20 | 概念图解 | "违反开闭原则" | 违反开闭原则 |
| 0:45 | 对比表格 | "难以维护和扩展" | 难以维护和扩展 |
| 1:15 | 代码截图 | "责任链模式解耦" | 责任链模式解耦 |
| 1:45 | 总结卡 | "记住三个词：违反开闭原则、难以维护和扩展、责任链模式解耦" | 收尾 |
