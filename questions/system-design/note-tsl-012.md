...
**30 秒电梯演讲：** 像智能门铃的安防功能——检测到陌生人在门口徘徊（异常检测），立刻手机弹窗告警（实时推送），同时录制视频保存证据（轨迹记录）。

**展开框架：**
1. **异常检测规则引擎** — (启动授权/移动模式)
2. **实时流处理(Ka** — fka+Flink秒级响应)
3. **多通道告警推送(** — App推送/短信/电话)

**收尾：** 如何区分"误报"（车主自己开走）和"真盗"？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：车辆出现异常启动、移动时触发预警，如何设计后端架构，支持预警 | "像智能门铃的安防功能——检测到陌生人在门口徘徊（异常检测），立刻手机弹窗告警（实时推送），同时录制视" | 引入 |
| 0:20 | 概念图解 | "(启动授权/移动模式)" | 异常检测规则引擎 |
| 0:45 | 对比表格 | "fka+Flink秒级响应)" | 实时流处理(Ka |
| 1:15 | 代码截图 | "App推送/短信/电话)" | 多通道告警推送( |
| 1:45 | 总结卡 | "记住三个词：异常检测规则引擎、实时流处理(Ka、多通道告警推送(" | 收尾 |
