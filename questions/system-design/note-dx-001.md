...
**30 秒电梯演讲：** 就像去超市买东西刷信用卡——POS机超时重新刷了一次，但你只应被扣一次钱。幂等性就是保证"不管刷几次，只扣一次"。

**展开框架：**
1. **幂等定义** — f(f(x)) = f(x)，执行多次结果相同
2. **方案一** — Token机制（防重令牌），请求前先获取Token
3. **方案二** — 数据库唯一约束（业务流水号做唯一索引）
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【中国电信面试】微服务架构下，如何保证服务之间调用的幂等性？ | "就像去超市买东西刷信用卡——POS机超时重新刷了一次，但你只应被扣一次钱。幂等性就是保证"不管刷几次" | 引入 |
| 0:20 | 概念图解 | "f(f(x)) = f(x)，执行多次结果相同" | 幂等定义 |
| 0:45 | 对比表格 | "Token机制（防重令牌），请求前先获取Token" | 方案一 |
| 1:15 | 代码截图 | "数据库唯一约束（业务流水号做唯一索引）" | 方案二 |
| 1:45 | 总结卡 | "记住三个词：幂等定义、方案一、方案二" | 收尾 |
