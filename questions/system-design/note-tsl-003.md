...
**30 秒电梯演讲：** 像大型商场的会员积分卡——收银台（Redis）实时扣减积分保证不超扣，每天晚上后台（DB）统一对账同步。这样收银快又不丢数据。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：亿级车主积分获取、兑换、过期管理，如何设计后端架构，保证积分 | "像大型商场的会员积分卡——收银台（Redis）实时扣减积分保证不超扣，每天晚上后台（DB）统一对账同" | 引入 |
| 0:20 | 概念图解 | "作保证积分不超扣" | Redis原子操 |
| 0:45 | 对比表格 | "积分记录" | 分库分表存储亿级 |
| 1:15 | 代码截图 | "dis→DB" | MQ异步同步Re |
| 2:15 | 总结卡 | "记住三个词：Redis原子操、分库分表存储亿级、MQ异步同步Re" | 收尾 |
