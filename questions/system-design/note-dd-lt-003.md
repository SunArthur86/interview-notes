...
**30 秒电梯演讲：** 就像彩票箱——你提前按1000张票放进去，抽奖时只随机摸一张就行。

**展开框架：**
1. **概率展开为数组** — 概率展开为数组
2. **随机数取模O(1** — )
3. **内存占用=N*s** — izeof(PrizeID)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【滴滴面经】你说用了空间换时间，把抽奖算法优化到了 O(1) | "就像彩票箱——你提前按1000张票放进去，抽奖时只随机摸一张就行。" | 引入 |
| 0:20 | 概念图解 | "概率展开为数组" | 概率展开为数组 |
| 0:45 | 对比表格 | ")" | 随机数取模O(1 |
| 1:15 | 代码截图 | "izeof(PrizeID)" | 内存占用=N*s |
| 2:15 | 总结卡 | "记住三个词：概率展开为数组、随机数取模O(1、内存占用=N*s" | 收尾 |
