...
**30 秒电梯演讲：** 就像城市交通堵塞——路口A→B→C→D，你说从A到D的时间从5分钟变成40分钟，但D停车场不挤(MySQL正常)。那堵在哪？(1)先看导航(SkyWalking)找出堵在哪段路；(2)看每个路口的红绿灯状态(线程状态)；(3)看是不是...

**展开框架：**
1. **排查顺序** — 先看链路追踪定位慢节点→再看线程dump→然后连接池→最后限流保护
2. **MySQL** — CPU正常排除数据库瓶颈，重点排查应用层（线程阻塞、连接池、GC、锁竞争）
3. **SkyWalki** — ng定位RT高的具体Dubbo接口或模块

**收尾：** 如果链路追踪发现是某个下游RPC服务变慢，如何进一步排查？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【快手Java一面】促销活动时订单服务TP99从50ms飙到 | "就像城市交通堵塞——路口A→B→C→D，你说从A到D的时间从5分钟变成40分钟，但D停车场不挤(My" | 引入 |
| 0:20 | 概念图解 | "先看链路追踪定位慢节点→再看线程dump→然后连接池→最后限流保护" | 排查顺序 |
| 0:45 | 对比表格 | "CPU正常排除数据库瓶颈，重点排查应用层（线程阻塞、连接池、GC、锁竞争）" | MySQL |
| 1:15 | 代码截图 | "ng定位RT高的具体Dubbo接口或模块" | SkyWalki |
| 1:45 | 总结卡 | "记住三个词：排查顺序、MySQL、SkyWalki" | 收尾 |
