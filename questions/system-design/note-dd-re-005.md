...
**30 秒电梯演讲：** 就像乐高积木——你不重新开模，只需按说明书拼装不同积木就能搭出各种造型。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【滴滴面经】如果规则引擎要做成动态可配置的，你觉得应该怎么设 | "就像乐高积木——你不重新开模，只需按说明书拼装不同积木就能搭出各种造型。" | 引入 |
| 0:20 | 概念图解 | "DSL" | JSON/YAML |
| 0:45 | 对比表格 | "规则解释器" | 规则解释器 |
| 1:15 | 代码截图 | "热更新（不重启）" | 热更新（不重启） |
| 2:15 | 总结卡 | "记住三个词：JSON/YAML、规则解释器、热更新（不重启）" | 收尾 |
