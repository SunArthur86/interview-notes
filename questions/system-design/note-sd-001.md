...
**30 秒电梯演讲：** 秒杀像春运抢火车票——先让购票app卡一下分散流量（前端限流），余票信息提前到Redis（不查数据库），抢到后排队出票（MQ异步），出完票更新数据库。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：设计一个秒杀系统，核心要点是什么？ | "秒杀像春运抢火车票——先让购票app卡一下分散流量（前端限流），余票信息提前到Redis（不查数据库" | 引入 |
| 0:20 | 概念图解 | "流" | 前端+网关双层限 |
| 0:45 | 对比表格 | "减防超卖" | Redis原子扣 |
| 1:15 | 代码截图 | "MQ异步削峰" | MQ异步削峰 |
| 2:15 | 总结卡 | "记住三个词：前端+网关双层限、Redis原子扣、MQ异步削峰" | 收尾 |
