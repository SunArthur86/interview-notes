...
**30 秒电梯演讲：** 最终一致性就像快递物流系统——你下单后，你的页面立刻显示"已下单"，但仓库的库存扣减、物流的发货安排可能要几分钟后才同步。最终所有系统都会达到一致状态，只是不是瞬间完成的。

**展开框架：**
1. **强一致性要求所有副本同时可见** — 代价是可用性下降
2. **最终一致性接受短暂不一致** — 通过异步消息最终达成一致
3. **BASE理论** — Basically Available + Soft state + Eventually consistent

**收尾：** 如果消息队列在扣款成功后宕机了，库存没扣，怎么补偿？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多一面】什么是最终一致性？在电商下单流程中如何体现？ | "最终一致性就像快递物流系统——你下单后，你的页面立刻显示"已下单"，但仓库的库存扣减、物流的发货安排" | 引入 |
| 0:20 | 概念图解 | "代价是可用性下降" | 强一致性要求所有副本同时可见 |
| 0:45 | 对比表格 | "通过异步消息最终达成一致" | 最终一致性接受短暂不一致 |
| 1:15 | 代码截图 | "Basically Available + Soft state + Eventually consistent" | BASE理论 |
| 1:45 | 总结卡 | "记住三个词：强一致性要求所有副本同时可见、最终一致性接受短暂不一致、BASE理论" | 收尾 |
