...
**30 秒电梯演讲：** 就像水管系统——水流不出来，要逐段排查：水龙头→管道→蓄水池→水源。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【滴滴面经】压测过程中，实际性能瓶颈在哪里？ | "就像水管系统——水流不出来，要逐段排查：水龙头→管道→蓄水池→水源。" | 引入 |
| 0:20 | 概念图解 | "O（单线程瓶颈）" | Redis网络I |
| 0:45 | 对比表格 | "DB连接池耗尽" | DB连接池耗尽 |
| 1:15 | 代码截图 | "GC停顿" | GC停顿 |
| 2:15 | 总结卡 | "记住三个词：Redis网络I、DB连接池耗尽、GC停顿" | 收尾 |
