...
**30 秒电梯演讲：** 僈星级酒店会员——住够N晚升级金卡（等级状态机），金卡享受免费早餐和延迟退房（权益映射），结账时自动打折（实时计费）。

**展开框架：**
1. **会员等级状态机(** — Silver→Gold→Platinum)
2. **权益规则引擎(等** — 级→权益映射)
3. **实时计费(充电费** — ×会员折扣)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：会员享受充电折扣、优先预约等权益，如何设计后端架构，支持会员 | "僈星级酒店会员——住够N晚升级金卡（等级状态机），金卡享受免费早餐和延迟退房（权益映射），结账时自动" | 引入 |
| 0:20 | 概念图解 | "Silver→Gold→Platinum)" | 会员等级状态机( |
| 0:45 | 对比表格 | "级→权益映射)" | 权益规则引擎(等 |
| 1:15 | 代码截图 | "×会员折扣)" | 实时计费(充电费 |
| 1:45 | 总结卡 | "记住三个词：会员等级状态机(、权益规则引擎(等、实时计费(充电费" | 收尾 |
