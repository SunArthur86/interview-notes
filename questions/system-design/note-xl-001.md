...
**30 秒电梯演讲：** 像一个实时股票行情系统——全网博文是交易数据，热度计算是股价计算，榜单是涨跌幅排行，突发热点就是涨停

**展开框架：**
1. **数据源** — 全网博文实时入库 → Kafka
2. **计算层** — Go消费Kafka → 分词/打标签 → 热度加权
3. **存储层** — Redis(实时榜单) + MySQL(历史) + ClickHouse(分析)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：从零设计微博热搜实时计算服务 | "像一个实时股票行情系统——全网博文是交易数据，热度计算是股价计算，榜单是涨跌幅排行，突发热点就是涨停" | 引入 |
| 0:20 | 概念图解 | "全网博文实时入库 → Kafka" | 数据源 |
| 0:45 | 对比表格 | "Go消费Kafka → 分词/打标签 → 热度加权" | 计算层 |
| 1:15 | 代码截图 | "Redis(实时榜单) + MySQL(历史) + ClickHouse(分析)" | 存储层 |
| 2:15 | 总结卡 | "记住三个词：数据源、计算层、存储层" | 收尾 |
