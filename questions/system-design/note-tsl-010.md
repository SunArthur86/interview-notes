...
**30 秒电梯演讲：** 像大型医院的分诊台——病人来了先由分诊护士（AI分类）判断看什么科，然后分到对应诊室（工单流转），看完病还要回访是否治好（闭环跟踪）。

**展开框架：**
1. **NLP自动分类(** — Bug/建议/投诉/咨询)
2. **工单状态机(提交** — →分类→处理→验证→关闭)
3. **SLA分级(紧急** — /P1/P2/P3)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：亿级车主提交APP、车载系统反馈，如何设计后端架构，实现反馈 | "像大型医院的分诊台——病人来了先由分诊护士（AI分类）判断看什么科，然后分到对应诊室（工单流转），看" | 引入 |
| 0:20 | 概念图解 | "Bug/建议/投诉/咨询)" | NLP自动分类( |
| 0:45 | 对比表格 | "→分类→处理→验证→关闭)" | 工单状态机(提交 |
| 1:15 | 代码截图 | "/P1/P2/P3)" | SLA分级(紧急 |
| 1:45 | 总结卡 | "记住三个词：NLP自动分类(、工单状态机(提交、SLA分级(紧急" | 收尾 |
