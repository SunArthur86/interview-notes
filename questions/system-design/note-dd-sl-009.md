...
**30 秒电梯演讲：** 就像在图书馆找自己借的所有书——书按书名分散在不同楼层，你需要一个个人借阅清单来快速定位。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【滴滴面经】如果一个用户创建了很多短链，他要查看自己的短链列 | "就像在图书馆找自己借的所有书——书按书名分散在不同楼层，你需要一个个人借阅清单来快速定位。" | 引入 |
| 0:20 | 概念图解 | "表" | 建用户维度的索引 |
| 0:45 | 对比表格 | "副表" | 按用户ID分片的 |
| 1:15 | 代码截图 | "ES倒排索引" | ES倒排索引 |
| 2:15 | 总结卡 | "记住三个词：建用户维度的索引、按用户ID分片的、ES倒排索引" | 收尾 |
