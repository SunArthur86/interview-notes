...
**30 秒电梯演讲：** 三人合伙开店——2PC是一个人统一指挥"大家同时转账"（必须等所有人准备好，慢但同步）；TCC是每人提前冻结资金（预留资源，灵活但代码复杂）；本地消息表是A先转，然后发微信通知B和C（最终都能收到，有延迟但最实用）。

**展开框架：**
1. **2PC** — 协调者统一指挥，prepare→commit两阶段，强一致但阻塞
2. **TCC** — Try(预留)→Confirm(确认)→Cancel(回滚)，业务侵入大
3. **本地消息表** — 业务表+消息表同库事务写入，异步投递MQ，最终一致

**收尾：** Seata的AT模式和TCC模式有什么区别？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【美云智数终面】分布式事务场景你遇到过几种？2PC、TCC、 | "三人合伙开店——2PC是一个人统一指挥"大家同时转账"（必须等所有人准备好，慢但同步）；TCC是每人" | 引入 |
| 0:20 | 概念图解 | "协调者统一指挥，prepare→commit两阶段，强一致但阻塞" | 2PC |
| 0:45 | 对比表格 | "Try(预留)→Confirm(确认)→Cancel(回滚)，业务侵入大" | TCC |
| 1:15 | 代码截图 | "业务表+消息表同库事务写入，异步投递MQ，最终一致" | 本地消息表 |
| 2:15 | 总结卡 | "记住三个词：2PC、TCC、本地消息表" | 收尾 |
