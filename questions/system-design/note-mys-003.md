...
**30 秒电梯演讲：** 就像最后一个演唱会门票——如果10个人同时看到"还剩1张"，都点击购买，就超卖了。解决方法：要么加锁（数据库层），要么先到先得（Redis层），要么排队（消息队列层）。

**展开框架：**
1. **数据库层** — UPDATE stock=stock-1 WHERE stock>0 利用行锁保证原子性
2. **Redis层** — DECR/Lua脚本原子扣减，库存预热到Redis
3. **消息队列层** — 请求入队串行消费，削峰填谷
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【美云智数终面】高并发下商品库存扣减，如何防止超卖？从数据库 | "就像最后一个演唱会门票——如果10个人同时看到"还剩1张"，都点击购买，就超卖了。解决方法：要么加锁" | 引入 |
| 0:20 | 概念图解 | "UPDATE stock=stock-1 WHERE stock>0 利用行锁保证原子性" | 数据库层 |
| 0:45 | 对比表格 | "DECR/Lua脚本原子扣减，库存预热到Redis" | Redis层 |
| 1:15 | 代码截图 | "请求入队串行消费，削峰填谷" | 消息队列层 |
| 1:45 | 总结卡 | "记住三个词：数据库层、Redis层、消息队列层" | 收尾 |
