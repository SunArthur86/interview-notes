...
**30 秒电梯演讲：** 雪花算法就像一个工厂给产品打编号——时间戳是'日期'(41位精确到毫秒)，机器ID是'车间号'(10位)，序列号是'当天产线序号'(12位)。冲突原因：(1)车间时钟走慢了(时钟回拨)→和别的车间撞了日期；(2)两个车间挂了同一个车牌...

**展开框架：**
1. **Snowflake结构** — 1+41+10+12=64位，每毫秒每机器最多4096个ID
2. **冲突根因1** — 时钟回拨(NTP同步导致时间回退)
3. **冲突根因2** — WorkerID重复(手动配置相同机器号)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【快手Java一面】分库分表后，雪花算法生成的ID出现冲突， | "雪花算法就像一个工厂给产品打编号——时间戳是'日期'(41位精确到毫秒)，机器ID是'车间号'(10" | 引入 |
| 0:20 | 概念图解 | "1+41+10+12=64位，每毫秒每机器最多4096个ID" | Snowflake结构 |
| 0:45 | 对比表格 | "时钟回拨(NTP同步导致时间回退)" | 冲突根因1 |
| 1:15 | 代码截图 | "WorkerID重复(手动配置相同机器号)" | 冲突根因2 |
| 1:45 | 总结卡 | "记住三个词：Snowflake结构、冲突根因1、冲突根因2" | 收尾 |
