...
**30 秒电梯演讲：** 缓存就像超市的促销商品货架。缓存击穿=特价商品卖完了(缓存失效)，所有顾客都涌向仓库(数据库)抢货。防御：(1)在货架旁放个小推车备货(Caffeine本地缓存)；(2)让理货员在后台悄悄补货(逻辑过期+异步刷新)；(3)提前预测哪些...

**展开框架：**
1. **缓存击穿=单个热** — 点Key失效导致大量请求穿透到DB
2. **互斥锁方案** — 只让一个线程查DB重建缓存，其他等待（已有方案）
3. **一级防御** — Caffeine本地缓存，短TTL，挡住大部分瞬时请求

**收尾：** 缓存击穿、缓存穿透、缓存雪崩三者的区别和各自防御方案？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【快手Java一面】热点Key突然失效，数据库被打到限流，除 | "缓存就像超市的促销商品货架。缓存击穿=特价商品卖完了(缓存失效)，所有顾客都涌向仓库(数据库)抢货。" | 引入 |
| 0:20 | 概念图解 | "点Key失效导致大量请求穿透到DB" | 缓存击穿=单个热 |
| 0:45 | 对比表格 | "只让一个线程查DB重建缓存，其他等待（已有方案）" | 互斥锁方案 |
| 1:15 | 代码截图 | "Caffeine本地缓存，短TTL，挡住大部分瞬时请求" | 一级防御 |
| 1:45 | 总结卡 | "记住三个词：缓存击穿=单个热、互斥锁方案、一级防御" | 收尾 |
