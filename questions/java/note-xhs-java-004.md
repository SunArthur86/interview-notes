...
**30 秒电梯演讲：** IOC像住酒店不用自己打扫（服务员=容器帮你搞定）；AOP像给所有方法都装了监控摄像头（不用改每个方法，统一拦截）。三级缓存解决循环依赖就像：A和B互相等对方（循环依赖），酒店先给A一个临时房卡（半成品），等B办好了再换正式卡。

**展开框架：**
1. **IOC=对象创建权交给容器** — DI=容器自动注入依赖
2. **构造器注入最推荐** — （可final、不可变、强制依赖）
3. **三级缓存解决si** — ngleton循环依赖+AOP代理问题

**收尾：** Spring 三级缓存为什么不能是两级缓存？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Spring IOC、AOP 原理及依赖注入实现方式？（华为 | "IOC像住酒店不用自己打扫（服务员=容器帮你搞定）；AOP像给所有方法都装了监控摄像头（不用改每个方" | 引入 |
| 0:20 | 概念图解 | "DI=容器自动注入依赖" | IOC=对象创建权交给容器 |
| 0:45 | 对比表格 | "（可final、不可变、强制依赖）" | 构造器注入最推荐 |
| 1:15 | 代码截图 | "ngleton循环依赖+AOP代理问题" | 三级缓存解决si |
| 2:15 | 总结卡 | "记住三个词：IOC=对象创建权交给容器、构造器注入最推荐、三级缓存解决si" | 收尾 |
