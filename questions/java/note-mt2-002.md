...
**30 秒电梯演讲：** 像医院ICU监控——心跳(活跃线程)、床位(队列容量)、拒诊率(拒绝策略)，一旦指标异常就自动报警+启动应急方案

**展开框架：**
1. **三大监控指标** — 队列积压、活跃线程比、拒绝次数
2. **血崩场景** — 所有线程被慢调用阻塞 → 新请求全进队列 → OOM
3. **防护** — 熔断(隔离慢调用) + 超时(快速失败) + 背压(CallerRuns)

**收尾：** CompletableFuture默认线程池有什么坑？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：用 Future/线程池做并发，怎么监控线程池状态？怎么防止 | "像医院ICU监控——心跳(活跃线程)、床位(队列容量)、拒诊率(拒绝策略)，一旦指标异常就自动报警+" | 引入 |
| 0:20 | 概念图解 | "队列积压、活跃线程比、拒绝次数" | 三大监控指标 |
| 0:45 | 对比表格 | "所有线程被慢调用阻塞 → 新请求全进队列 → OOM" | 血崩场景 |
| 1:15 | 代码截图 | "熔断(隔离慢调用) + 超时(快速失败) + 背压(CallerRuns)" | 防护 |
| 1:45 | 总结卡 | "记住三个词：三大监控指标、血崩场景、防护" | 收尾 |
