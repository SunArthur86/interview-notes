...
**30 秒电梯演讲：** 就像厨房只有一个灶台——IO密集是等外卖送达(等的时候别人可以用灶台)，CPU密集是持续炒菜(别人完全没机会)。

**展开框架：**
1. **GIL保护引用计** — 数
2. **IO操作释放GI** — L(time.sleep/network/file)
3. **CPU密集型用m** — ultiprocessing绕过
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】讲一下 Python 的 GIL 机制，在 I/ | "就像厨房只有一个灶台——IO密集是等外卖送达(等的时候别人可以用灶台)，CPU密集是持续炒菜(别人完" | 引入 |
| 0:20 | 概念图解 | "数" | GIL保护引用计 |
| 0:45 | 对比表格 | "L(time.sleep/network/file)" | IO操作释放GI |
| 1:15 | 代码截图 | "ultiprocessing绕过" | CPU密集型用m |
| 1:45 | 总结卡 | "记住三个词：GIL保护引用计、IO操作释放GI、CPU密集型用m" | 收尾 |
