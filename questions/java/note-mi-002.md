...
**30 秒电梯演讲：** 线程池像餐厅后厨——核心线程是常驻厨师，队列是等单架，最大线程是高峰临时工，拒绝策略是客流爆满时的处理方式(拒绝接单/老板亲自上/转其他店)

**展开框架：**
1. **CPU密集型** — 核心线程=CPU核数+1(+1防偶发page fault)
2. **IO密集型** — 核心线程=CPU核数×2或更多(线程等待IO时让出CPU)
3. **禁用无界队列Li** — nkedBlockingQueue → 极易OOM
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：线程池怎么配置？线上拒绝策略怎么选？ | "线程池像餐厅后厨——核心线程是常驻厨师，队列是等单架，最大线程是高峰临时工，拒绝策略是客流爆满时的处" | 引入 |
| 0:20 | 概念图解 | "核心线程=CPU核数+1(+1防偶发page fault)" | CPU密集型 |
| 0:45 | 对比表格 | "核心线程=CPU核数×2或更多(线程等待IO时让出CPU)" | IO密集型 |
| 1:15 | 代码截图 | "nkedBlockingQueue → 极易OOM" | 禁用无界队列Li |
| 1:45 | 总结卡 | "记住三个词：CPU密集型、IO密集型、禁用无界队列Li" | 收尾 |
