...
**30 秒电梯演讲：** 就像公厕门——先看门牌"有人吗"（第一次检查，无锁快），没人就推门进去锁上（加锁），进去了再看一眼确实没人（第二次检查，防两人同时进来），然后安心使用。

**展开框架：**
1. **第一次检查** — 无锁快速路径，对象已创建则直接返回
2. **第二次检查** — 加锁后再次确认，防止多线程同时通过第一次检查
3. **volatile必须加** — 防止new操作的指令重排

**收尾：** 为什么不加volatile会有问题？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节三面手撕】实现双重检查锁（DCL）单例模式 | "就像公厕门——先看门牌"有人吗"（第一次检查，无锁快），没人就推门进去锁上（加锁），进去了再看一眼确" | 引入 |
| 0:20 | 概念图解 | "无锁快速路径，对象已创建则直接返回" | 第一次检查 |
| 0:45 | 对比表格 | "加锁后再次确认，防止多线程同时通过第一次检查" | 第二次检查 |
| 1:15 | 总结卡 | "记住三个词：第一次检查、第二次检查、volatile必须加" | 收尾 |
