...
**30 秒电梯演讲：** 想象排队买奶茶：ArrayList 像固定座位的影院（第5排第3座直接走过去），LinkedList 像手拉手的人群（找第5个人要从第1个开始数）。

**展开框架：**
1. **ArrayLis** — t=数组+1.5倍扩容，随机访问O(1)
2. **LinkedList=双向链表** — 头尾插入O(1)
3. **实际开发95%用** — ArrayList（CPU缓存友好）

**收尾：** ArrayList 扩容 1.5 倍的原因是什么？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：ArrayList 和 LinkedList 的区别与使用场 | "想象排队买奶茶：ArrayList 像固定座位的影院（第5排第3座直接走过去），LinkedList" | 引入 |
| 0:20 | 概念图解 | "t=数组+1.5倍扩容，随机访问O(1)" | ArrayLis |
| 0:45 | 对比表格 | "头尾插入O(1)" | LinkedList=双向链表 |
| 1:15 | 总结卡 | "记住三个词：ArrayLis、LinkedList=双向链表、实际开发95%用" | 收尾 |
