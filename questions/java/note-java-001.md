...
**30 秒电梯演讲：** synchronized 像自动门（进去自动关门出来自动开门），ReentrantLock 像手动门（自己锁自己开，忘开了就死锁）。

**展开框架：**
1. **synchron** — ized=JVM内置+自动释放
2. **Reentran** — tLock=API层+手动释放
3. **Reentran** — tLock支持公平/中断/超时/多Condition
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：synchronized 和 ReentrantLock 的 | "synchronized 像自动门（进去自动关门出来自动开门），ReentrantLock 像手动门" | 引入 |
| 0:20 | 概念图解 | "ized=JVM内置+自动释放" | synchron |
| 0:45 | 对比表格 | "tLock=API层+手动释放" | Reentran |
| 1:15 | 代码截图 | "tLock支持公平/中断/超时/多Condition" | Reentran |
| 1:45 | 总结卡 | "记住三个词：synchron、Reentran、Reentran" | 收尾 |
