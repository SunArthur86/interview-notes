...
**30 秒电梯演讲：** 就像公共厕所的门锁进化：一开始不锁门（无锁），后来装个弹簧锁谁推谁进（偏向锁），再后来加个旋钮锁有人等就排队自旋（轻量级锁），最后升级成密码锁由管理员调度（重量级锁）

**展开框架：**
1. **对象头Mark** — Word存储锁状态（32位/64位）
2. **无锁→偏向锁** — 首次有线程进入
3. **偏向锁→轻量级锁** — 出现第二个线程竞争
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多 Java服务端】synchronized锁升级过程 | "就像公共厕所的门锁进化：一开始不锁门（无锁），后来装个弹簧锁谁推谁进（偏向锁），再后来加个旋钮锁有人" | 引入 |
| 0:20 | 概念图解 | "Word存储锁状态（32位/64位）" | 对象头Mark |
| 0:45 | 对比表格 | "首次有线程进入" | 无锁→偏向锁 |
| 1:15 | 代码截图 | "出现第二个线程竞争" | 偏向锁→轻量级锁 |
| 1:45 | 总结卡 | "记住三个词：对象头Mark、无锁→偏向锁、偏向锁→轻量级锁" | 收尾 |
