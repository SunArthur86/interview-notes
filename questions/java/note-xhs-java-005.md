...
**30 秒电梯演讲：** 临时实例像外卖骑手——随时上线下线（AP优先，宁可短暂看到已下线的骑手也不能让系统卡住）。持久实例像实体店——数据必须准确一致（CP优先，宁可短暂不可用也不能数据错乱）。用外卖骑手的方式管理实体店地址=灾难

**展开框架：**
1. **临时实例=客户端心跳维持** — 断连自动摘除→Distro协议(AP)
2. **持久实例=服务端主动探测** — 永久存储→Raft协议(CP)
3. **Distro** — AP模式，各节点独立处理写入，异步同步，允许短暂不一致

**收尾：** Distro协议和Raft协议的具体区别是什么？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【Java微服务】Nacos临时实例vs持久实例的区别？各自 | "临时实例像外卖骑手——随时上线下线（AP优先，宁可短暂看到已下线的骑手也不能让系统卡住）。持久实例像" | 引入 |
| 0:20 | 概念图解 | "断连自动摘除→Distro协议(AP)" | 临时实例=客户端心跳维持 |
| 0:45 | 对比表格 | "永久存储→Raft协议(CP)" | 持久实例=服务端主动探测 |
| 1:15 | 总结卡 | "记住三个词：临时实例=客户端心跳维持、持久实例=服务端主动探测、Distro" | 收尾 |
