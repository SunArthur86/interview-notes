...
**30 秒电梯演讲：** volatile就像办公室的白板——你写上去所有人立刻能看到（可见性），写的时候必须按顺序写（有序性），但两个人同时抢着写就会乱（不保证原子性）。

**展开框架：**
1. **保证可见性** — 修改后立即刷新到主内存，读取时强制从主内存加载
2. **保证有序性** — 通过内存屏障禁止指令重排序
3. **不保证原子性** — volatile int的i++不是原子操作

**收尾：** volatile能替代synchronized吗？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多一面】volatile 关键字的作用和底层实现 | "volatile就像办公室的白板——你写上去所有人立刻能看到（可见性），写的时候必须按顺序写（有序性" | 引入 |
| 0:20 | 概念图解 | "修改后立即刷新到主内存，读取时强制从主内存加载" | 保证可见性 |
| 0:45 | 对比表格 | "通过内存屏障禁止指令重排序" | 保证有序性 |
| 1:15 | 代码截图 | "volatile int的i++不是原子操作" | 不保证原子性 |
| 1:45 | 总结卡 | "记住三个词：保证可见性、保证有序性、不保证原子性" | 收尾 |
