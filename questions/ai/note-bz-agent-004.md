...
**30 秒电梯演讲：** 像组装一台能干活的机器人——大脑(LLM)、手(工具)、笔记本(记忆)、操作系统(循环)、保险丝(安全)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何从 0 搭建一个 AI Agent？整体流程是怎样的？ | "像组装一台能干活的机器人——大脑(LLM)、手(工具)、笔记本(记忆)、操作系统(循环)、保险丝(安" | 引入 |
| 0:20 | 概念图解 | "选LLM→定工具→写Prompt→做记忆→加循环→上安全→测迭代" | 七步 |
| 0:45 | 对比表格 | "先用最简单组件跑通闭环" | MVP原则 |
| 1:15 | 代码截图 | "接口层/编排层/能力层/安全层" | 架构分层 |
| 1:45 | 总结卡 | "记住三个词：七步、MVP原则、架构分层" | 收尾 |
