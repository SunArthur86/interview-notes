...
**30 秒电梯演讲：** 像医生开处方——医生(LLM)写药方(调用指令)，药师(程序)配药执行，病人吃药后反馈(结果)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：LLM 工具调用（Tool Use）机制是什么？如何实践？ | "像医生开处方——医生(LLM)写药方(调用指令)，药师(程序)配药执行，病人吃药后反馈(结果)。" | 引入 |
| 0:20 | 概念图解 | "LLM生成tool_call → 程序执行 → 结果回传 → LLM继续" | 流程 |
| 0:45 | 对比表格 | "原生Function Calling / Prompt工程" | 两种实现 |
| 1:15 | 代码截图 | "工具描述(JSON Schema)+参数校验+错误处理" | 关键 |
| 1:45 | 总结卡 | "记住三个词：流程、两种实现、关键" | 收尾 |
