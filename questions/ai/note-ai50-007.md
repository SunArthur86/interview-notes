...
2. **Faithful** — 答案是否忠于检索到的上下文，直接衡量幻觉
3. **Context** — 检索到的上下文中有多少是真正相关的
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Ragas框架如何评估RAG系统中的幻觉？ | "就像考试评分——不光看最终答案对不对，还要看参考书找得准不准(检索)、答题有没有超出参考书范围(忠实" | 引入 |
| 0:20 | 概念图解 | "Faithfulness, Answer Relevancy, Context Precision, Contex..." | Ragas四大指标 |
| 0:45 | 对比表格 | "答案是否忠于检索到的上下文，直接衡量幻觉" | Faithful |
| 1:15 | 代码截图 | "检索到的上下文中有多少是真正相关的" | Context |
| 1:45 | 总结卡 | "记住三个词：Ragas四大指标、Faithful、Context" | 收尾 |
