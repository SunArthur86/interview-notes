...
**30 秒电梯演讲：** 像旅行计划——先做完整攻略(Day1去哪Day2去哪)，再按计划执行。比ReAct的"走一步看一步"更有全局观，遇到封路再调整。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Plan-and-Execute（计划-执行）认知框架怎么工 | "像旅行计划——先做完整攻略(Day1去哪Day2去哪)，再按计划执行。比ReAct的"走一步看一步"" | 引入 |
| 0:20 | 概念图解 | "Planner全局规划 + Executor逐步执行" | 两阶段 |
| 0:45 | 对比表格 | "局部视角问题" | 解决ReAct的 |
| 1:15 | 代码截图 | "可用不同模型" | 规划与执行解耦 |
| 1:45 | 总结卡 | "记住三个词：两阶段、解决ReAct的、规划与执行解耦" | 收尾 |
