...
**30 秒电梯演讲：** 像出差收拾行李——只带必需品(任务拆解)、重要文件随身记忆分层)、换洗衣物按天数带(滑动窗口)，而不是把整个家搬走。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：多轮对话越聊越贵，如何优化 Token 成本？ | "像出差收拾行李——只带必需品(任务拆解)、重要文件随身记忆分层)、换洗衣物按天数带(滑动窗口)，而不" | 引入 |
| 0:20 | 概念图解 | "任务拆解+记忆分层+滑动窗口" | 三招 |
| 0:45 | 对比表格 | "按信息价值裁剪上下文" | 核心 |
| 1:15 | 代码截图 | "增量计算+缓存复用+模型路由" | 进阶 |
| 1:45 | 总结卡 | "记住三个词：三招、核心、进阶" | 收尾 |
