...
**30 秒电梯演讲：** 就像考试做选择题——hit@1是"第一个选的必须对"，hit@3是"给你三次机会，选到对的就算过"。如果考题本身有歧义(用户表述模糊)，给三次机会显然更公平。而且如果后续有澄清机制，从前3个里找到正确意图就能继续。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：意图识别准确率为什么统计hit@3而不是hit@1？ | "就像考试做选择题——hit@1是"第一个选的必须对"，hit@3是"给你三次机会，选到对的就算过"。" | 引入 |
| 0:20 | 概念图解 | "用户表达模糊时，模型可能把正确意图排到第2-3位" | hit@1过于严格 |
| 0:45 | 对比表格 | "配合澄清机制，Top-3命中即可补救" | hit@3反映实际可用性 |
| 1:15 | 代码截图 | "无澄清→hit@1，有澄清→hit@3" | 指标选择取决于下游设计 |
| 1:45 | 总结卡 | "记住三个词：hit@1过于严格、hit@3反映实际可用性、指标选择取决于下游设计" | 收尾 |
