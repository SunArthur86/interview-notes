...
**30 秒电梯演讲：** 像搬家公司——一家搬不下的大件家具（大模型），DeepSpeed 拆散分到多辆车（多 GPU）上：优化器状态放车1（ZeRO-1）、梯度放车2（ZeRO-2）、连模型参数都分到各车（ZeRO-3）。激活检查点像"中途把不用的家具暂存仓...

**展开框架：**
1. **ZeRO 三阶段** — 优化器状态分片(Z1)+梯度分片(Z2)+参数分片(Z3)
2. **Z1省4倍显存** — Z2再省一点，Z3让单卡只存1/N参数
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【阶跃星辰面经】对 DeepSpeed 框架有没有了解 | "像搬家公司——一家搬不下的大件家具（大模型），DeepSpeed 拆散分到多辆车（多 GPU）上：优" | 引入 |
| 0:20 | 概念图解 | "优化器状态分片(Z1)+梯度分片(Z2)+参数分片(Z3)" | ZeRO 三阶段 |
| 0:45 | 对比表格 | "Z2再省一点，Z3让单卡只存1/N参数" | Z1省4倍显存 |
| 1:15 | 代码截图 | "用算力换显存(只存部分激活，反向时重算)" | 激活检查点 |
| 1:45 | 总结卡 | "记住三个词：ZeRO 三阶段、Z1省4倍显存、激活检查点" | 收尾 |
