...
**30 秒电梯演讲：** 像四个人互相传东西必须同时给同时收——如果 A 先给 B 再收 C，但 B 先收 A 再给 C，顺序不一致就互相干等。或者一个人突然手抖（报错），其他三个人拿着东西等他，永远传不下去。

**展开框架：**
1. **死锁=多个GPU** — 互相等待对方释放资源，全部卡住
2. **原因1** — NCCL collective 顺序不一致（部分先发后收）
3. **原因2** — 负载不均导致同步点干等

**收尾：** NCCL 的 all-reduce 怎么实现？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【阶跃星辰面经】GPU 死锁是什么情况 | "像四个人互相传东西必须同时给同时收——如果 A 先给 B 再收 C，但 B 先收 A 再给 C，顺序" | 引入 |
| 0:20 | 概念图解 | "互相等待对方释放资源，全部卡住" | 死锁=多个GPU |
| 0:45 | 对比表格 | "NCCL collective 顺序不一致（部分先发后收）" | 原因1 |
| 1:15 | 代码截图 | "负载不均导致同步点干等" | 原因2 |
| 1:45 | 总结卡 | "记住三个词：死锁=多个GPU、原因1、原因2" | 收尾 |
