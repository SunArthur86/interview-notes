...
**30 秒电梯演讲：** 像有个翻译员——你用大白话说"查上月销量"，翻译员(Text2SQL)转成SQL去数据库查，再把结果翻译回你能懂的话。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Text2SQL 在 Agent 里怎么用？ | "像有个翻译员——你用大白话说"查上月销量"，翻译员(Text2SQL)转成SQL去数据库查，再把结果" | 引入 |
| 0:20 | 概念图解 | "Text2SQL作为Agent的数据查询工具" | 定位 |
| 0:45 | 对比表格 | "意图理解→SQL生成→执行→结果解读" | 流程 |
| 1:15 | 代码截图 | "复杂SQL/多表/业务语义" | 难点 |
| 1:45 | 总结卡 | "记住三个词：定位、流程、难点" | 收尾 |
