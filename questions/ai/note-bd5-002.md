...
**30 秒电梯演讲：** Rules像交通法规(红灯停绿灯行，永远生效)；Skills像开车技能(需要时才用)。你不会把'红灯停'写进'如何开车'的说明书里——法规和技能是两个层次
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Rules 和 Skills 有什么区别？为什么不把 Ski | "Rules像交通法规(红灯停绿灯行，永远生效)；Skills像开车技能(需要时才用)。你不会把'红灯" | 引入 |
| 0:20 | 概念图解 | "系统级约束，不可违反(安全/合规/格式)" | Rules |
| 0:45 | 对比表格 | "工具能力，按需调用(搜索/计算/生成)" | Skills |
| 1:15 | 代码截图 | "标准化工具协议，统一Skills的注册和调用" | MCP |
| 1:45 | 总结卡 | "记住三个词：Rules、Skills、MCP" | 收尾 |
