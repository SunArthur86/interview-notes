...
**30 秒电梯演讲：** Continuous Batching像拼车——不停有人上下车但车一直在跑；Cascade Attention像共享笔记——前面讲的共同内容大家共用一份，只有各自不同的部分单独记
...
**收尾：** PagedAttention和操作系统虚拟内存有什么关系？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：VLM 的 Continuous Batching 是什么？ | "Continuous Batching像拼车——不停有人上下车但车一直在跑；Cascade Atte" | 引入 |
| 0:20 | 概念图解 | "iteration级别动态拼batch，消除队头阻塞" | Continuous |
| 0:45 | 对比表格 | "KV Cache分页管理，减少显存碎片" | PagedAttention |
| 1:15 | 代码截图 | "共享前缀复用KV Cache(few-shot/系统prompt场景)" | Cascade |
| 2:15 | 总结卡 | "记住三个词：Continuous、PagedAttention、Cascade" | 收尾 |
