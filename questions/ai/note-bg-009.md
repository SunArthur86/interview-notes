...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【八股总结】vLLM 和 SGLang 的核心原理与适用场景 | "vLLM像高效的"出租车调度"——用PagedAttention把显存当虚拟内存管理，不浪费碎片空间" | 引入 |
| 0:20 | 概念图解 | "PagedAttention(分页KV) + Continuous Batching(动态批)" | vLLM核心 |
| 0:45 | 对比表格 | "RadixAttention(前缀缓存树) + 结构化生成" | SGLang核心 |
| 1:15 | 代码截图 | "高并发通用推理服务" | vLLM适合 |
| 1:45 | 总结卡 | "记住三个词：vLLM核心、SGLang核心、vLLM适合" | 收尾 |
