...
**30 秒电梯演讲：** 像超市结账——1000 个顾客里 999 个买几件东西，1 个买了整车。如果按"顾客ID"分到不同收银台，那个买整车的收银台排队1小时，其他收银台闲死。解法：把那个大客户的商品拆到多个收银台（加盐打散），最后再合并。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【神州专车面经】数据倾斜怎么处理？ | "像超市结账——1000 个顾客里 999 个买几件东西，1 个买了整车。如果按"顾客ID"分到不同收" | 引入 |
| 0:20 | 概念图解 | "Shuffle 时某 key 数据量远超其他，少数 task 成为瓶颈" | 倾斜本质 |
| 0:45 | 对比表格 | "空值/null、热点业务key(大V用户ID)、GroupBy高频key" | 三类常见key |
| 1:15 | 代码截图 | "过滤掉 或 用随机值打散" | 空值解法 |
| 2:15 | 总结卡 | "记住三个词：倾斜本质、三类常见key、空值解法" | 收尾 |
