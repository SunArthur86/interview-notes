...
│  └──────────────────────────────────────────────────┘    │
│  ┌──────────────────────────────────────────────────┐    │
│  │ ④ Few-shot 示例（Input → Output 样本）            │    │
│  │    示例1 / 示例2 / 示例3                          │    │
│  └──────────────────────────────────────────────────┘    │
│  ┌──────────────────────────────────────────────────┐    │
│  │ ⑤ 输出格式约束（JSON Schema / Markdown 规范）     │    │
│  │    "严格按以下JSON格式输出：{...}"                │    │
│  └──────────────────────────────────────────────────┘    │
└──────────────────────────────────────────────────────────┘
         ▲                                         ▲
         │                                         │
    版本 v1.2.3                              A/B Test → 评估指标
```
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【美团面经】提示词模板是怎么构建的？ | "就像写SOP——你是谁做什么有哪些参考材料结果长什么样。" | 引入 |
| 0:20 | 概念图解 | "Prompt" | 角色设定System |
| 0:45 | 对比表格 | "ja2" | 变量化模板Jin |
| 1:15 | 代码截图 | "Schema" | 输出格式约束JSON |
| 1:45 | 总结卡 | "记住三个词：角色设定System、变量化模板Jin、输出格式约束JSON" | 收尾 |
