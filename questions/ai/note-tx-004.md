...
**30 秒电梯演讲：** CC像金鱼记忆——只记得眼前的事，但有个笔记本（CLAUDE.md）随时翻。OpenClaw像大象记忆——能记住很久以前的事，通过外部大脑（向量DB）检索。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【腾讯面经】Claude Code 的 memory 是怎么 | "CC像金鱼记忆——只记得眼前的事，但有个笔记本（CLAUDE.md）随时翻。OpenClaw像大象记" | 引入 |
| 0:20 | 概念图解 | "上下文内+CLAUDE.md+Auto-Compact" | CC |
| 0:45 | 对比表格 | "分层记忆+外部向量DB" | OpenClaw |
| 1:15 | 代码截图 | "觉" | CC简单可靠无幻 |
| 2:15 | 总结卡 | "记住三个词：CC、OpenClaw、CC简单可靠无幻" | 收尾 |
