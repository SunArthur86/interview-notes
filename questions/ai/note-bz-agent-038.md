...
**30 秒电梯演讲：** Skill像菜谱(做什么菜+怎么做)，MCP像厨房电器标准接口(所有厨具统一插座)。菜谱用电器的标准接口调用厨具。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Agent Skill 与 MCP 的区别？ | "Skill像菜谱(做什么菜+怎么做)，MCP像厨房电器标准接口(所有厨具统一插座)。菜谱用电器的标准" | 引入 |
| 0:20 | 概念图解 | "能力封装(prompt+tools+flow)，面向任务" | Skill |
| 0:45 | 对比表格 | "通信协议(标准化工具接口)，面向连接" | MCP |
| 1:15 | 代码截图 | "Skill通过MCP调用工具" | 关系 |
| 1:45 | 总结卡 | "记住三个词：Skill、MCP、关系" | 收尾 |
