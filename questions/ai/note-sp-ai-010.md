...
**30 秒电梯演讲：** Encoder像阅读理解——通读全文做笔记。Decoder像写作文——看着笔记一个字一个字写出来

**展开框架：**
1. **Encoder读完整输入** — 每个词融合全句上下文
2. **Decoder逐词自回归生成** — 多一层交叉注意力
3. **每层结构** — 自注意力+前馈+残差+LayerNorm

**收尾：** 为什么Encoder可以并行而Decoder不能？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Transformer的整体结构是怎样的？Encoder和D | "Encoder像阅读理解——通读全文做笔记。Decoder像写作文——看着笔记一个字一个字写出来" | 引入 |
| 0:20 | 概念图解 | "每个词融合全句上下文" | Encoder读完整输入 |
| 0:45 | 对比表格 | "多一层交叉注意力" | Decoder逐词自回归生成 |
| 1:15 | 总结卡 | "记住三个词：Encoder读完整输入、Decoder逐词自回归生成、每层结构" | 收尾 |
