...
**30 秒电梯演讲：** 像开会做笔记——会议太长笔记本写满时，你会：丢掉闲聊（截断）、把长讨论总结成要点（摘要）、但绝不能丢掉"会议目标"和"待办事项"（关键信息）。工具返回的长报告，你会提炼关键结论而非全文记录。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【八股总结】Agent 上下文窗口溢出怎么办？工具结果太长如 | "像开会做笔记——会议太长笔记本写满时，你会：丢掉闲聊（截断）、把长讨论总结成要点（摘要）、但绝不能丢" | 引入 |
| 0:20 | 概念图解 | "截断/摘要/选择性保留/外部记忆" | 溢出处理 |
| 0:45 | 对比表格 | "裁剪关键部分 or 摘要 or 存外部按需检索" | 工具结果太长 |
| 1:15 | 代码截图 | "系统指令、当前任务、关键决策、工具调用契约" | 不能压缩 |
| 2:15 | 总结卡 | "记住三个词：溢出处理、工具结果太长、不能压缩" | 收尾 |
