...
**30 秒电梯演讲：** 像考试限时——学生可能2分钟交卷（短轨迹），也可能死磕到打铃（长尾）。如果等最慢的学生，全班效率被拖垮。解法是设时限（max_turns）、慢的先交部分卷（截断）、并行批改（异步）。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【八股总结】多轮 Agentic RL 何时停止 rollo | "像考试限时——学生可能2分钟交卷（短轨迹），也可能死磕到打铃（长尾）。如果等最慢的学生，全班效率被拖" | 引入 |
| 0:20 | 概念图解 | "final answer / max_turns / 死循环检测" | 停止条件 |
| 0:45 | 对比表格 | "少数超长轨迹拖慢整体同步训练" | 长尾问题 |
| 1:15 | 代码截图 | "异步rollout（不等慢的）" | 解法1 |
| 2:15 | 总结卡 | "记住三个词：停止条件、长尾问题、解法1" | 收尾 |
