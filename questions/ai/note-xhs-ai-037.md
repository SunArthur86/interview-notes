...
**30 秒电梯演讲：** 你在跟客服多轮聊天。短对话=把之前的聊天记录都带上（全量拼接）；长对话=让客服先看个摘要（摘要压缩）；最聪明的方式=把'它多少钱'自动改成'iPhone 15多少钱'（query改写补全指代）
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【RAG多轮对话】多轮对话怎么处理上下文？ | "你在跟客服多轮聊天。短对话=把之前的聊天记录都带上（全量拼接）；长对话=让客服先看个摘要（摘要压缩）" | 引入 |
| 0:20 | 概念图解 | "历史对话拼到当前query里一起检索（适合短对话）" | 方式一 |
| 0:45 | 对比表格 | "LLM对历史做摘要作为额外上下文（适合长对话，省token）" | 方式二 |
| 1:15 | 总结卡 | "记住三个词：方式一、方式二、方式三" | 收尾 |
