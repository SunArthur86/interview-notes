...
**30 秒电梯演讲：** 写文章时先用AI生成初稿(draft模型快)，再人工审核修改(target模型准)——比逐字手写快得多，质量有保证
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：什么是投机解码(Speculative Decoding)？ | "写文章时先用AI生成初稿(draft模型快)，再人工审核修改(target模型准)——比逐字手写快得" | 引入 |
| 0:20 | 概念图解 | "小模型，快速生成k个候选token" | Draft模型 |
| 0:45 | 对比表格 | "大模型，并行验证k个token" | Target模型 |
| 1:15 | 代码截图 | "拒绝采样，保证输出分布与target模型完全一致" | 接受规则 |
| 2:15 | 总结卡 | "记住三个词：Draft模型、Target模型、接受规则" | 收尾 |
