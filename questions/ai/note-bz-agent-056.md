...
**30 秒电梯演讲：** Naive RAG像自动售货机(投币出货固定流程)，Agentic RAG像导购(听需求→找商品→不够再找→推荐)。

**展开框架：**
1. **Agentic** — RAG是检索作为Agent工具，动态决策
2. **Naive RAG是固定流程** — 一次性检索
3. **区别** — 智能程度（动态vs固定）

**收尾：** Agentic RAG什么时候比Naive好？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：什么是 Agentic RAG？和传统 RAG 的区别？ | "Naive RAG像自动售货机(投币出货固定流程)，Agentic RAG像导购(听需求→找商品→不" | 引入 |
| 0:20 | 概念图解 | "RAG是检索作为Agent工具，动态决策" | Agentic |
| 0:45 | 对比表格 | "一次性检索" | Naive RAG是固定流程 |
| 1:15 | 代码截图 | "智能程度（动态vs固定）" | 区别 |
| 2:15 | 总结卡 | "记住三个词：Agentic、Naive RAG是固定流程、区别" | 收尾 |
