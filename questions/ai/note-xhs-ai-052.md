...
**30 秒电梯演讲：** 普通RAG像一个公共图书馆——所有人看同样的书，书更新很慢，只能找文字内容。企业级RAG像一个企业内部档案室——不同部门只能看自己的文件（权限），政策一变马上更新（增量），能查表格和数据库（结构化），还能理解你的上下文语境（多轮对话）。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【高德AI面试】企业级RAG和普通RAG的核心区别？ | "普通RAG像一个公共图书馆——所有人看同样的书，书更新很慢，只能找文字内容。企业级RAG像一个企业内" | 引入 |
| 0:20 | 概念图解 | "不同角色/部门只能检索到权限范围内的知识" | 权限管控 |
| 0:45 | 对比表格 | "政策规则时效变更时实时更新向量库" | 增量索引 |
| 1:15 | 代码截图 | "不仅查文本向量，还能解析表格/数据库/结构化文档" | 结构化检索 |
| 1:45 | 总结卡 | "记住三个词：权限管控、增量索引、结构化检索" | 收尾 |
