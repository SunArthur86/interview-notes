...
**30 秒电梯演讲：** 像快递分拣系统——先检查包裹信息是否完整(校验)，分拣出错就重新扫描(重试)，实在不行就走人工通道(降级)
...
**收尾：** 除了Function Calling还有哪些结构化输出方法？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Function Calling工程实现：如何设计容错机制？ | "像快递分拣系统——先检查包裹信息是否完整(校验)，分拣出错就重新扫描(重试)，实在不行就走人工通道(" | 引入 |
| 0:20 | 概念图解 | "JSON Schema校验 + 参数类型检查" | 输入层 |
| 0:45 | 对比表格 | "自动重试 + Few-shot修复 + 格式约束Prompt" | 模型层 |
| 1:15 | 代码截图 | "降级方案 + 默认值 + 人工介入触发" | 输出层 |
| 1:45 | 总结卡 | "记住三个词：输入层、模型层、输出层" | 收尾 |
