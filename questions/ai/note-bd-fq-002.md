...
**30 秒电梯演讲：** 就像翻译——中文和英文各有一套词汇（不同空间），但"猫"和"cat"指同一个东西。对比学习就是让翻译正确的配对在语义空间里靠得更近
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：多模态检索中，图像和文本向量不在同一空间时，如何实现对齐？ | "就像翻译——中文和英文各有一套词汇（不同空间），但"猫"和"cat"指同一个东西。对比学习就是让翻译" | 引入 |
| 0:20 | 概念图解 | "对比学习+InfoNCE损失，最大化匹配图文对的余弦相似度" | CLIP范式 |
| 0:45 | 对比表格 | "在预训练模型上加投影头，无需重训整个模型" | 投影层映射 |
| 1:15 | 代码截图 | "Cross-Modal Attention让文本query动态关注图像区域" | 跨模态注意力 |
| 2:15 | 总结卡 | "记住三个词：CLIP范式、投影层映射、跨模态注意力" | 收尾 |
