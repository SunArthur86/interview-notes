...
**30 秒电梯演讲：** Prompt像便签纸上的菜谱(写完就扔)，Skill像装订成册的菜谱书(有目录/食材表/步骤图/营养信息)，可保存可分享可复用。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Prompt 不等于 Skill，怎么把 Prompt 升级 | "Prompt像便签纸上的菜谱(写完就扔)，Skill像装订成册的菜谱书(有目录/食材表/步骤图/营养" | 引入 |
| 0:20 | 概念图解 | "加元数据/加工具/加流程/加Schema/加示例" | Prompt→Skill五步 |
| 0:45 | 对比表格 | "Skill是结构化可管理的，Prompt是扁平的" | 核心区别 |
| 1:15 | 代码截图 | "可复用/可分享/可迭代/可测试" | 升级价值 |
| 1:45 | 总结卡 | "记住三个词：Prompt→Skill五步、核心区别、升级价值" | 收尾 |
