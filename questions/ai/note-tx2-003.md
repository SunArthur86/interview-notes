...
**30 秒电梯演讲：** 像教小孩填表——先给他看很多"问题+空白表+填好的表"的例子(SFT 监督学习)，他学会怎么填；再告诉他"这种填法比那种好"(RLHF 偏好优化)，他学会填得更准。最后看到新问题，他能自动填出正确的表。
...
**收尾：** Structured Output（response_format json_schema）和 Function Calling 区别？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【某讯面经】Function Calling 底层原理：模型 | "像教小孩填表——先给他看很多"问题+空白表+填好的表"的例子(SFT 监督学习)，他学会怎么填；再告" | 引入 |
| 0:20 | 概念图解 | "(指令+工具schema, 正确调用JSON) 监督微调" | SFT 阶段 |
| 0:45 | 对比表格 | "偏好数据(正确调用>错误调用)优化选最优" | RLHF 辅助 |
| 1:15 | 代码截图 | "自然语言→结构化意图 的映射，通过SFT+RLHF固化进权重" | 本质 |
| 2:15 | 总结卡 | "记住三个词：SFT 阶段、RLHF 辅助、本质" | 收尾 |
