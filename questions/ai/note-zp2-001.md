...
**30 秒电梯演讲：** SFT像健身练基础体能——当你力量不再增长但柔韧性/爆发力还能提升时，说明基础训练够了，该转专项训练(RL)了

**展开框架：**
1. **不能只看trai** — n/validation loss下降
2. **关键信号** — OOD/hard set不涨 + 输出模板化 = SFT进入低收益区
3. **pass@k高但** — pass@1低 → 应优先RL而非继续堆SFT
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何判断 SFT 已经做到头了？ | "SFT像健身练基础体能——当你力量不再增长但柔韧性/爆发力还能提升时，说明基础训练够了，该转专项训练" | 引入 |
| 0:20 | 概念图解 | "n/validation loss下降" | 不能只看trai |
| 0:45 | 对比表格 | "OOD/hard set不涨 + 输出模板化 = SFT进入低收益区" | 关键信号 |
| 1:15 | 代码截图 | "pass@1低 → 应优先RL而非继续堆SFT" | pass@k高但 |
| 2:15 | 总结卡 | "记住三个词：不能只看trai、关键信号、pass@k高但" | 收尾 |
