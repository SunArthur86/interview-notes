...
**30 秒电梯演讲：** 就像心理咨询师的工作手册——规定了什么时候该共情、什么时候该转介、遇到自杀倾向必须触发应急流程。LangGraph就是这份手册的代码实现
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：心理咨询Agent如何用LangGraph控制对话走向？ | "就像心理咨询师的工作手册——规定了什么时候该共情、什么时候该转介、遇到自杀倾向必须触发应急流程。La" | 引入 |
| 0:20 | 概念图解 | "共情→评估→干预→总结" | 状态图定义对话阶段 |
| 0:45 | 对比表格 | "根据情绪检测结果决定下一步" | 条件路由 |
| 1:15 | 代码截图 | "自杀/自伤关键词触发应急流程" | 安全门控 |
| 2:15 | 总结卡 | "记住三个词：状态图定义对话阶段、条件路由、安全门控" | 收尾 |
