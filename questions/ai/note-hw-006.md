...
**30 秒电梯演讲：** 像给数据中心配一个24小时不休息的"首席运维官+SRE团队"——它同时盯住所有服务器的指标（眼睛），用积累的运维知识判断哪里要出问题（大脑），还能自动执行预案（手），并且每次故障后复盘改进（学习）。

**展开框架：**
1. **三层架构** — 感知层(采集) + 认知层(LLM推理) + 执行层(自动恢复)
2. **核心挑战** — 误报率控制、幻觉抑制、安全执行边界
3. **RAG注入历史故障知识库** — 避免从零推理

**收尾：** 如何降低LLM运维Agent的幻觉和误报？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【华为面经】基于大模型设计服务器自动故障预警、排查和恢复系统 | "像给数据中心配一个24小时不休息的"首席运维官+SRE团队"——它同时盯住所有服务器的指标（眼睛），" | 引入 |
| 0:20 | 概念图解 | "感知层(采集) + 认知层(LLM推理) + 执行层(自动恢复)" | 三层架构 |
| 0:45 | 对比表格 | "误报率控制、幻觉抑制、安全执行边界" | 核心挑战 |
| 1:15 | 代码截图 | "避免从零推理" | RAG注入历史故障知识库 |
| 2:15 | 总结卡 | "记住三个词：三层架构、核心挑战、RAG注入历史故障知识库" | 收尾 |
