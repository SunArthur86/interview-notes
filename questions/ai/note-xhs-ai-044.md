...
**30 秒电梯演讲：** RAG像给AI配了一个参考书架（检索），但AI可能看了书还瞎编。Prompt约束=告诉AI'只参考书架上的内容'；后校验=交卷前对答案；溯源=每句话标注引用来源。三层都不能少
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【快手AI大模型】RAG缓解幻觉的边界在哪里？三层管控怎么做 | "RAG像给AI配了一个参考书架（检索），但AI可能看了书还瞎编。Prompt约束=告诉AI'只参考书" | 引入 |
| 0:20 | 概念图解 | "减少但无法消除，模型仍可能编造检索结果中不存在的信息" | RAG缓解幻觉的边界 |
| 0:45 | 对比表格 | "Prompt约束（预防）+ 内容后校验（拦截）+ 素材库溯源（验证）" | 三层管控 |
| 1:15 | 代码截图 | "明确要求"只基于检索内容回答，不确定时说不知道"" | Prompt约束 |
| 1:45 | 总结卡 | "记住三个词：RAG缓解幻觉的边界、三层管控、Prompt约束" | 收尾 |
