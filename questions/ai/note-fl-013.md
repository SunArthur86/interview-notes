...
**30 秒电梯演讲：** Redis Stream 像小区快递柜（够用、就近、偶尔丢件），Kafka 像专业物流中心（吞吐大、可追溯、跨城市）。小社区用快递柜够了，跨国电商必须物流中心。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节飞连面经】Redis Stream vs Kafka： | "Redis Stream 像小区快递柜（够用、就近、偶尔丢件），Kafka 像专业物流中心（吞吐大、" | 引入 |
| 0:20 | 概念图解 | "Redis看模式(AOF everysec可能丢1s)，Kafka磁盘顺序写+副本更强" | 持久化 |
| 0:45 | 对比表格 | "Redis单机十万级，Kafka单机百万级" | 吞吐 |
| 1:15 | 代码截图 | "都有；ACK用XACK vs offset commit" | 消费组 |
| 1:45 | 总结卡 | "记住三个词：持久化、吞吐、消费组" | 收尾 |
