...
**30 秒电梯演讲：** 就像新闻编辑——不能只靠记者自觉(Prompt约束)，还要有事实核查(RAG)、编辑审稿(后处理)、多源交叉验证(多路生成)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】除了在 Prompt 中加约束，还有哪些工程手段 | "就像新闻编辑——不能只靠记者自觉(Prompt约束)，还要有事实核查(RAG)、编辑审稿(后处理)、" | 引入 |
| 0:20 | 概念图解 | "提供事实依据约束生成" | RAG |
| 0:45 | 对比表格 | "限制token候选(上下文约束)" | 约束解码 |
| 1:15 | 代码截图 | "NLI校验/事实核查API" | 后处理 |
| 2:15 | 总结卡 | "记住三个词：RAG、约束解码、后处理" | 收尾 |
