...
**30 秒电梯演讲：** SFT像跟读课本（模仿标准答案），RLHF像老师打分+学生改进（先训练打分器再优化），DPO像直接给学生看好作文和差作文对比（跳过打分器一步到位）
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：详细说明SFT、RLHF和DPO三种后训练方法的核心流程、适 | "SFT像跟读课本（模仿标准答案），RLHF像老师打分+学生改进（先训练打分器再优化），DPO像直接给" | 引入 |
| 0:20 | 概念图解 | "监督学习，用标准答案做交叉熵损失" | SFT |
| 0:45 | 对比表格 | "训练RM → PPO优化策略，流程复杂但效果好" | RLHF |
| 1:15 | 代码截图 | "直接用偏好对(pair)优化，无需RM和PPO，更稳定" | DPO |
| 2:15 | 总结卡 | "记住三个词：SFT、RLHF、DPO" | 收尾 |
