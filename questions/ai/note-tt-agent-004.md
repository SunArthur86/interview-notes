...
**30 秒电梯演讲：** 就像装修工程——设计师出图（Planner）→工人施工（Executor）→质检员验收（Reflector）→不合格返工（回到Planner调整）→验收通过交付

**展开框架：**
1. **Planner拆分任务为子步骤** — 输出结构化执行计划
2. **Executor按计划逐步执行** — 调用工具/API
3. **Reflector检查结果质量** — 决定通过/修复/放弃
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：简述Agent的Planner-Executor-Refle | "就像装修工程——设计师出图（Planner）→工人施工（Executor）→质检员验收（Reflec" | 引入 |
| 0:20 | 概念图解 | "输出结构化执行计划" | Planner拆分任务为子步骤 |
| 0:45 | 对比表格 | "调用工具/API" | Executor按计划逐步执行 |
| 1:15 | 代码截图 | "决定通过/修复/放弃" | Reflector检查结果质量 |
| 2:15 | 总结卡 | "记住三个词：Planner拆分任务为子步骤、Executor按计划逐步执行、Reflector检查结果质量" | 收尾 |
