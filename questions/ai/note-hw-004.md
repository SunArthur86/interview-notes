...
**30 秒电梯演讲：** 像建一座"拍照学院"——先从各场景招学生（采集），按教学大纲分班（场景分类），请专业老师打分（专家标注），定期考核质检（一致性检验），毕业的学生（标注样本）送进工厂（训练）。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【华为面经】华为手机拍照场景训练数据采集与标注方案 | "像建一座"拍照学院"——先从各场景招学生（采集），按教学大纲分班（场景分类），请专业老师打分（专家标" | 引入 |
| 0:20 | 概念图解 | "真实用户场景为主，合成数据为辅，确保分布匹配" | 数据采集层 |
| 0:45 | 对比表格 | "按场景类型（人像/风景/夜景/美食）× 拍摄条件（光照/距离/抖动）正交划分" | 场景分层 |
| 1:15 | 代码截图 | "自动标注(模型预标) + 人工标注(专家) + 一致性检验" | 标注分层 |
| 2:15 | 总结卡 | "记住三个词：数据采集层、场景分层、标注分层" | 收尾 |
