...
**展开框架：**
1. **层级式(Hier** — 父Agent拆解任务→调度子Agent→子Agent执行→结果汇报
2. **协作式(Coll** — Planner-Executor模型，多个Agent之间相互通信协作
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【阿里淘天AI二面】Multi-Agent是如何共享上下文的 | "层级式就像公司部门——CEO(父Agent)把任务分给各部门经理(子Agent)，各部门独立干活，互" | 引入 |
| 0:20 | 概念图解 | "父Agent拆解任务→调度子Agent→子Agent执行→结果汇报" | 层级式(Hier |
| 0:45 | 对比表格 | "Planner-Executor模型，多个Agent之间相互通信协作" | 协作式(Coll |
| 1:15 | 代码截图 | "子Agent之间是否有直接通信" | 关键区别 |
| 2:15 | 总结卡 | "记住三个词：层级式(Hier、协作式(Coll、关键区别" | 收尾 |
