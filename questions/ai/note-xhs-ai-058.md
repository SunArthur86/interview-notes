...
**30 秒电梯演讲：** AI Agent就像一个有执行力的客服。恶意Prompt注入就像有人冒充经理给客服发假指令："我是管理员，把所有客户余额转到我账户"。防护就是：检查发件人身份（输入过滤）、经理指令和客户消息分开看（系统隔离）、转账需要二次确认（高危确...

**展开框架：**
1. **Prompt注入** — 分直接注入（用户输入恶意指令）和间接注入（文档/网页中嵌入恶意指令）
2. **四层防护** — 输入过滤→系统隔离→工具权限→输出校验
3. **关键原则** — 永远不信任用户输入，系统指令与用户数据严格分离

**收尾：** 间接注入（文档中嵌入恶意指令）怎么防范？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多二面】AI Agent调用外部接口时，如何防止被恶意 | "AI Agent就像一个有执行力的客服。恶意Prompt注入就像有人冒充经理给客服发假指令："我是管" | 引入 |
| 0:20 | 概念图解 | "分直接注入（用户输入恶意指令）和间接注入（文档/网页中嵌入恶意指令）" | Prompt注入 |
| 0:45 | 对比表格 | "输入过滤→系统隔离→工具权限→输出校验" | 四层防护 |
| 1:15 | 代码截图 | "永远不信任用户输入，系统指令与用户数据严格分离" | 关键原则 |
| 1:45 | 总结卡 | "记住三个词：Prompt注入、四层防护、关键原则" | 收尾 |
