...
**30 秒电梯演讲：** 就像开卷考试——索引阶段是提前做好目录和书签，检索阶段是考试时快速翻到对应页面，生成阶段是看着书上的内容写答案。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】RAG 系统的整体流程是什么？索引阶段和检索阶段 | "就像开卷考试——索引阶段是提前做好目录和书签，检索阶段是考试时快速翻到对应页面，生成阶段是看着书上的" | 引入 |
| 0:20 | 概念图解 | "文档解析-切片-Embedding-入库" | 索引阶段 |
| 0:45 | 对比表格 | "Query向量化-相似度搜索-重排序" | 检索阶段 |
| 1:15 | 代码截图 | "检索结果注入Prompt-LLM生成" | 生成阶段 |
| 1:45 | 总结卡 | "记住三个词：索引阶段、检索阶段、生成阶段" | 收尾 |
