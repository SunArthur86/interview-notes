...
**30 秒电梯演讲：** 像一个只能装10页的文件夹——新文件来了，文件夹满了，就把最旧的几页拍照压缩成一张摘要页放回去，腾出空间放新文件。这样既不会爆，又保留了历史要点。

**展开框架：**
1. **维护消息deque** — tokenizer算每条token数
2. **超限时摘除最旧几条** — + LLM摘要 + 插入摘要消息
3. **保留近期原文(细节)** — 旧的摘要(压缩)

**收尾：** 摘要本身也占 token，怎么避免"摘要越积越多"？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【某讯面经】算法题：滑动窗口实现长对话自动摘要截断，控制总  | "像一个只能装10页的文件夹——新文件来了，文件夹满了，就把最旧的几页拍照压缩成一张摘要页放回去，腾出" | 引入 |
| 0:20 | 概念图解 | "tokenizer算每条token数" | 维护消息deque |
| 0:45 | 对比表格 | "+ LLM摘要 + 插入摘要消息" | 超限时摘除最旧几条 |
| 1:15 | 总结卡 | "记住三个词：维护消息deque、超限时摘除最旧几条、保留近期原文(细节)" | 收尾 |
