...
**展开框架：**
1. **自动重试(指数退避** — 3次)
2. **工具替换(同类工** — 具fallback)
3. **状态回滚(che** — ckpoint恢复)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】如果 Agent 在多步推理过程中某一步调用工具 | "就像导航软件——一条路堵了(工具失败)先等一会儿(重试)，不行就换路(替代工具)，再不行就改目的地(" | 引入 |
| 0:20 | 概念图解 | "3次)" | 自动重试(指数退避 |
| 0:45 | 对比表格 | "具fallback)" | 工具替换(同类工 |
| 1:15 | 代码截图 | "ckpoint恢复)" | 状态回滚(che |
| 2:15 | 总结卡 | "记住三个词：自动重试(指数退避、工具替换(同类工、状态回滚(che" | 收尾 |
