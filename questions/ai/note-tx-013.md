...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【腾讯面经】QLoRA 调参怎么做？核心原理是什么？ | "就像用一部老手机（量化后的模型）跑新系统——手机硬件不变（冻结参数），只装一个轻量插件（LoRA）来" | 引入 |
| 0:20 | 概念图解 | "NFQLoRA 量化" | 4-bit |
| 0:45 | 对比表格 | "低秩适配" | LoRA |
| 1:15 | 代码截图 | "双重量化" | 双重量化 |
| 1:45 | 总结卡 | "记住三个词：4-bit、LoRA、双重量化" | 收尾 |
