...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】不同的 Embedding 模型对检索质量影响很 | "就像选相机镜头——不能只看参数表，要在实际拍摄场景中对比样片才能知道哪个最适合你的需求。" | 引入 |
| 0:20 | 概念图解 | "参考" | MTEB榜单排名 |
| 0:45 | 对比表格 | "m3等)" | 中英文双语支持(bge |
| 1:15 | 代码截图 | "衡" | 维度与存储成本权 |
| 1:45 | 总结卡 | "记住三个词：MTEB榜单排名、中英文双语支持(bge、维度与存储成本权" | 收尾 |
