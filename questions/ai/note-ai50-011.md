...
2. **观测(Obser** — Trace每一步的Prompt/Response/Tool I/O
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：没有Agent Harness大模型会遇到什么瓶颈？ | "就像飞机的黑匣子+自动驾驶仪——不光让飞机飞起来(LLM生成)，还要记录每一步操作(Trace)、监" | 引入 |
| 0:20 | 概念图解 | "任务完成率、工具调用成功率、成本效率" | 评测(Eval) |
| 0:45 | 对比表格 | "Trace每一步的Prompt/Response/Tool I/O" | 观测(Obser |
| 1:15 | 代码截图 | "精确复现某次执行的完整链路" | 回放(Replay) |
| 2:15 | 总结卡 | "记住三个词：评测(Eval)、观测(Obser、回放(Replay)" | 收尾 |
