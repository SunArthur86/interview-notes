...
**30 秒电梯演讲：** 像一个迷路的人不停兜圈——你要先告诉他别走回头路(Prompt)，再设个GPS步数上限(运行时)，最后装个检测器发现他在兜圈就拦住他(监控)
...
**收尾：** 怎么检测Agent在'隐性循环'(每步动作不同但语义相同)？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Agent 出现无限循环或规划混乱怎么防？ | "像一个迷路的人不停兜圈——你要先告诉他别走回头路(Prompt)，再设个GPS步数上限(运行时)，最" | 引入 |
| 0:20 | 概念图解 | "规则约束、Few-shot反面案例、强制进度校验" | Prompt层 |
| 0:45 | 对比表格 | "最大轮数、调用历史去重、指数退避" | 运行时层 |
| 1:15 | 代码截图 | "语义相似度检测、目标漂移校验、异常告警" | 监控层 |
| 1:45 | 总结卡 | "记住三个词：Prompt层、运行时层、监控层" | 收尾 |
