...
**30 秒电梯演讲：** SFT冷启动像教小孩"说话的基本规矩"（先学会清晰表达），RL像"通过奖惩培养好习惯"（学会说什么更好）。如果小孩还不会说话就直接奖惩，他根本不知道为什么被奖/罚，学习效率极低。

**展开框架：**
1. **SFT提供RL的** — 初始化策略π_0，决定了探索起点
2. **没有SFT直接RL** — reward方差大、训练不稳定、容易reward hacking
3. **SFT教"格式和基本能力"** — RL优化"偏好和质量"
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【八股总结】SFT 冷启动和后续 RL 的关系是什么？为什么 | "SFT冷启动像教小孩"说话的基本规矩"（先学会清晰表达），RL像"通过奖惩培养好习惯"（学会说什么更" | 引入 |
| 0:20 | 概念图解 | "初始化策略π_0，决定了探索起点" | SFT提供RL的 |
| 0:45 | 对比表格 | "reward方差大、训练不稳定、容易reward hacking" | 没有SFT直接RL |
| 1:15 | 代码截图 | "RL优化"偏好和质量"" | SFT教"格式和基本能力" |
| 1:45 | 总结卡 | "记住三个词：SFT提供RL的、没有SFT直接RL、SFT教"格式和基本能力"" | 收尾 |
