...
**30 秒电梯演讲：** 想象你是一个开卷考试的学生。如果你翻到的参考页码不对（检索失败），你可能凭记忆瞎编答案（幻觉）。限制幻觉就是：确保翻到正确的页（高召回检索）、只允许你引用书上的话（生成约束）、最后老师检查答案是否真的来自书本（后置校验）。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何限制RAG幻觉？ | "想象你是一个开卷考试的学生。如果你翻到的参考页码不对（检索失败），你可能凭记忆瞎编答案（幻觉）。限制" | 引入 |
| 0:20 | 概念图解 | "提升召回率(混合检索+重排序)、过滤低相关chunk、查询改写" | 检索层 |
| 0:45 | 对比表格 | "Prompt约束(只基于context回答)、温度调低、结构化输出" | 生成层 |
| 1:15 | 代码截图 | "事实一致性校验(NLI)、引用溯源、置信度阈值过滤" | 后置层 |
| 1:45 | 总结卡 | "记住三个词：检索层、生成层、后置层" | 收尾 |
