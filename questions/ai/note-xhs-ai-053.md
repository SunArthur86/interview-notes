...
**30 秒电梯演讲：** 固定切片像用裁纸刀按固定长度切报纸——一句话可能被拦腰截断。语义切片像按段落和语义切——保证每个片段意思完整。滑动窗口像切的时候每刀和上一刀重叠一小段——防止跨段信息被遗漏。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【高德AI面试】做文档切片的时候，怎么避免关键信息丢失？ | "固定切片像用裁纸刀按固定长度切报纸——一句话可能被拦腰截断。语义切片像按段落和语义切——保证每个片段" | 引入 |
| 0:20 | 概念图解 | "割裂语义、丢失跨段落关联、规则类文档被拆碎" | 固定分块问题 |
| 0:45 | 对比表格 | "按章节/段落/句子自然边界切分" | 语义切片 |
| 1:15 | 代码截图 | "10%-15%窗口避免关键语句被切割" | 滑动窗口重叠 |
| 1:45 | 总结卡 | "记住三个词：固定分块问题、语义切片、滑动窗口重叠" | 收尾 |
