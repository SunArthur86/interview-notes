...
**30 秒电梯演讲：** String 像便签纸（一条信息），Hash 像表格（一行多列可单独改），List 像排队队列（先进先出），Set 像标签云（去重），ZSet 像带分数的排行榜（自动排序）。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节飞连面经】Redis 数据结构：String/Hash | "String 像便签纸（一条信息），Hash 像表格（一行多列可单独改），List 像排队队列（先进" | 引入 |
| 0:20 | 概念图解 | "计数器(INCR)/缓存对象JSON/分布式锁" | String |
| 0:45 | 对比表格 | "对象字段级读写，Agent 会话状态首选" | Hash |
| 1:15 | 总结卡 | "记住三个词：String、Hash、List" | 收尾 |
