...
**30 秒电梯演讲：** 像切蛋糕——均匀切（固定长度）简单但可能切开草莓；按花纹切（按段落）美观但大小不一；按草莓位置切（语义分块）每个都有完整草莓但形状不规则。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【宇树科技二面】RAG 的 Chunking 策略如何设计？ | "像切蛋糕——均匀切（固定长度）简单但可能切开草莓；按花纹切（按段落）美观但大小不一；按草莓位置切（语" | 引入 |
| 0:20 | 概念图解 | "简单粗暴，可能切断句子" | 固定长度分块 |
| 0:45 | 对比表格 | "语义完整但块大小不均" | 按段落/标题分块 |
| 1:15 | 代码截图 | "先按大标题再按段落再按句子，层次化切分" | 递归分块 |
| 1:45 | 总结卡 | "记住三个词：固定长度分块、按段落/标题分块、递归分块" | 收尾 |
