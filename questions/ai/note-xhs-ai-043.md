...
**30 秒电梯演讲：** 训练一个模仿脱口秀演员风格的AI。如果只喂他的段子（同质化数据），AI只会背段子不会聊天（过拟合）。如果数据太多太杂，又学不到风格（欠拟合）。最佳方案：80%风格数据+20%通用数据混合训练，像唱歌时保持自己的声线又能驾驭不同曲风
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【快手AI大模型】LoRA/QLoRA微调如何保证人设不崩坏 | "训练一个模仿脱口秀演员风格的AI。如果只喂他的段子（同质化数据），AI只会背段子不会聊天（过拟合）。" | 引入 |
| 0:20 | 概念图解 | "从达人历史内容提取风格特征（语气/用词/结构）" | 数据集构建 |
| 0:45 | 对比表格 | "去重（相似度>0.9的删）+ 去低质（短文本/无信息量）+ 去同质化" | 清洗 |
| 1:15 | 代码截图 | "混合通用数据（10-20%）防灾难性遗忘" | 人设不崩坏 |
| 1:45 | 总结卡 | "记住三个词：数据集构建、清洗、人设不崩坏" | 收尾 |
