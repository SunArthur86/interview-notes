...
**30 秒电梯演讲：** 就像看直播做饭——不是等菜全做好才端上来，而是从切菜、炒菜到装盘全程实时展示，用户不用干等

**展开框架：**
1. **SSE是Agen** — t流式传输的主流方案(单向推送, 基于HTTP)
2. **WebSocke** — t适合需要双向交互的场景
3. **流式内容** — Thought流(逐token) + Action事件 + Observation事件 + Final Answer流

**收尾：** SSE和WebSocket在Agent场景下怎么选？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Agent多步任务的流式传输怎么实现？ | "就像看直播做饭——不是等菜全做好才端上来，而是从切菜、炒菜到装盘全程实时展示，用户不用干等" | 引入 |
| 0:20 | 概念图解 | "t流式传输的主流方案(单向推送, 基于HTTP)" | SSE是Agen |
| 0:45 | 对比表格 | "t适合需要双向交互的场景" | WebSocke |
| 1:15 | 代码截图 | "Thought流(逐token) + Action事件 + Observation事件 + Final Answer流" | 流式内容 |
| 1:45 | 总结卡 | "记住三个词：SSE是Agen、WebSocke、流式内容" | 收尾 |
