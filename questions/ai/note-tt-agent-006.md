...
**30 秒电梯演讲：** 就像餐厅服务员——熟客档案说"不吃香菜"（长期画像），但今天客人说"给我加香菜"（当前会话），当然听当前的，同时记住"可能口味变了"

**展开框架：**
1. **当前会话信息优先** — 于长期画像（时间就近原则）
2. **冲突时触发画像更** — 新流程（异步不阻塞回答）
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：用户长期画像和当前会话信息冲突时，系统怎么处理？如何防止RA | "就像餐厅服务员——熟客档案说"不吃香菜"（长期画像），但今天客人说"给我加香菜"（当前会话），当然听" | 引入 |
| 0:20 | 概念图解 | "于长期画像（时间就近原则）" | 当前会话信息优先 |
| 0:45 | 对比表格 | "新流程（异步不阻塞回答）" | 冲突时触发画像更 |
| 1:15 | 代码截图 | "检索内容是素材而非模板" | RAG做参考不做约束 |
| 1:45 | 总结卡 | "记住三个词：当前会话信息优先、冲突时触发画像更、RAG做参考不做约束" | 收尾 |
