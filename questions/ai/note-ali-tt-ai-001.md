...
**30 秒电梯演讲：** ReAct就像探险家——走到哪看到哪，根据眼前情况决定下一步（发现岔路→想→选一条走→看到新线索→再想→再选）。Plan & Execute就像建筑工程师——开工前先画完整施工图，然后按图纸一步步施工。现实中两者常配合：工程师先做总体...

**展开框架：**
1. **ReAct** — = Observation-Thought-Action 循环，单步推理+执行，灵活但上下文易爆炸
2. **Plan** — & Execute = 先全局规划再分步执行，稳定但灵活性差，重新规划成本高
3. **ReAct适合环** — 境动态变化、需要边执行边决策的场景（如数据分析查询）

**收尾：** ReAct的Thought步骤能否用Chain-of-Thought(CoT)替代？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【阿里淘天AI二面】介绍ReAct范式和Plan & Exe | "ReAct就像探险家——走到哪看到哪，根据眼前情况决定下一步（发现岔路→想→选一条走→看到新线索→再" | 引入 |
| 0:20 | 概念图解 | "= Observation-Thought-Action 循环，单步推理+执行，灵活但上下文易爆炸" | ReAct |
| 0:45 | 对比表格 | "& Execute = 先全局规划再分步执行，稳定但灵活性差，重新规划成本高" | Plan |
| 1:15 | 代码截图 | "境动态变化、需要边执行边决策的场景（如数据分析查询）" | ReAct适合环 |
| 1:45 | 总结卡 | "记住三个词：ReAct、Plan、ReAct适合环" | 收尾 |
