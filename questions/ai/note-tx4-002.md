...
**30 秒电梯演讲：** 像一个酒店管家服务——住客昨天说要素食今天说要牛排(冲突)，管家不能擅自覆盖，得确认；同时不能把301房的偏好给302房看(隔离)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：设计多用户在线 Agent 长期记忆系统，用户前后说法矛盾产 | "像一个酒店管家服务——住客昨天说要素食今天说要牛排(冲突)，管家不能擅自覆盖，得确认；同时不能把30" | 引入 |
| 0:20 | 概念图解 | "不直接覆盖，加时间戳+置信度，冲突时问用户" | 冲突处理 |
| 0:45 | 对比表格 | "tenant_id+user_id双层过滤，物理分库+权限网关" | 多租户隔离 |
| 1:15 | 代码截图 | "创建→去重合并→置信衰减→过期淘汰" | 记忆生命周期 |
| 2:15 | 总结卡 | "记住三个词：冲突处理、多租户隔离、记忆生命周期" | 收尾 |
