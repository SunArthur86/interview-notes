...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】你在项目里遇到过大模型接口调用超时或限流的问题吗 | "就像飞机出行——航班取消(超时)时先改签(重试)，改签不了换高铁(模型降级)，高铁也没有就住酒店等明" | 引入 |
| 0:20 | 概念图解 | "试(3次上限)" | 指数退避+抖动重 |
| 0:45 | 对比表格 | "4→GPT-3.5→本地模型)" | 模型降级(GPT |
| 1:15 | 代码截图 | "uery命中缓存)" | 缓存降级(相似Q |
| 2:15 | 总结卡 | "记住三个词：指数退避+抖动重、模型降级(GPT、缓存降级(相似Q" | 收尾 |
