...
**30 秒电梯演讲：** 像导航软件不停说"前方掉头"然后又"前方掉头"——一直在转圈。解决方法是导航检测到你在原地打转，强制重新规划路线或停下来问你。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【宇树科技二面】ReAct 循环中，如何检测并跳出无效的"思 | "像导航软件不停说"前方掉头"然后又"前方掉头"——一直在转圈。解决方法是导航检测到你在原地打转，强制" | 引入 |
| 0:20 | 概念图解 | "工具调用序列去重、推理内容相似度、步数上限" | 循环检测三要素 |
| 0:45 | 对比表格 | "滑动窗口看最近N步是否有重复工具调用" | 轻量检测 |
| 1:15 | 代码截图 | "embedding相似度比较推理内容" | 深度检测 |
| 1:45 | 总结卡 | "记住三个词：循环检测三要素、轻量检测、深度检测" | 收尾 |
