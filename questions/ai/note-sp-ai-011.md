...
**30 秒电梯演讲：** 自注意力像会议室讨论——每个人(Q)提出自己关心的点，和其他人的标签(K)匹配，匹配度高的就听谁说(V)

**展开框架：**
1. **Q=查询向量(我要找什么)** — K=键向量(我有什么标签)，V=值向量(我的内容)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：自注意力机制的计算过程是怎样的？Q、K、V分别是什么？ | "自注意力像会议室讨论——每个人(Q)提出自己关心的点，和其他人的标签(K)匹配，匹配度高的就听谁说(" | 引入 |
| 0:20 | 概念图解 | "K=键向量(我有什么标签)，V=值向量(我的内容)" | Q=查询向量(我要找什么) |
| 0:45 | 对比表格 | "Q·K^T→scale→softmax→×V" | 计算流程 |
| 1:15 | 总结卡 | "记住三个词：Q=查询向量(我要找什么)、计算流程、多头=并行8组" | 收尾 |
