...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Self-Attention 为什么除以根号 d_k？ | "就像考试分数标准化——100道题的总分（方差大）除以根号100=10，变成标准分（方差1）。" | 引入 |
| 0:20 | 概念图解 | "点积方差=d_k" | 点积方差=d_k |
| 0:45 | 对比表格 | "方差=1" | 除以根号d_k后 |
| 1:15 | 代码截图 | "x饱和" | 防止softma |
| 1:45 | 总结卡 | "记住三个词：点积方差=d_k、除以根号d_k后、防止softma" | 收尾 |
