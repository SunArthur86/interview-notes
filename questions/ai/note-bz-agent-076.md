...
**30 秒电梯演讲：** 像大坝防洪——主闸门(网关限流)、分洪道(会话限流)、应急泄洪(熔断)、备用电源(降级)，层层保护。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Agent 高并发如何做防护？（网关限流+会话限流+模型熔断 | "像大坝防洪——主闸门(网关限流)、分洪道(会话限流)、应急泄洪(熔断)、备用电源(降级)，层层保护。" | 引入 |
| 0:20 | 概念图解 | "网关限流/会话限流/模型熔断/工具降级" | 四层 |
| 0:45 | 对比表格 | "保护脆弱的LLM调用" | 核心 |
| 1:15 | 代码截图 | "限流/熔断/降级/队列/缓存" | 手段 |
| 2:15 | 总结卡 | "记住三个词：四层、核心、手段" | 收尾 |
