...
**30 秒电梯演讲：** 像汽车仪表盘：资源层=油量表（显存），性能层=发动机转速（命中率/重算率），业务层=时速表（P99延迟），异常层=故障灯（OOM/evict告警）。只盯油表（显存利用率）就像只看油表开车——发动机过热都不知道

**展开框架：**
1. **四层监控金字塔** — 资源层→性能层→业务层→异常层
2. **KV** — Cache利用率<15%触发compaction
3. **Prefix** — cache命中率<50%检查prompt标准化

**收尾：** Prometheus+Grafana的具体dashboard怎么配置？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【AI Infra推理优化】如何构建KV Cache可观测体 | "像汽车仪表盘：资源层=油量表（显存），性能层=发动机转速（命中率/重算率），业务层=时速表（P99延" | 引入 |
| 0:20 | 概念图解 | "资源层→性能层→业务层→异常层" | 四层监控金字塔 |
| 0:45 | 对比表格 | "Cache利用率<15%触发compaction" | KV |
| 1:15 | 代码截图 | "cache命中率<50%检查prompt标准化" | Prefix |
| 1:45 | 总结卡 | "记住三个词：四层监控金字塔、KV、Prefix" | 收尾 |
