...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Multi-Agent 之间的连接（通信）方式有哪几种？ | "像同事沟通——直接喊话(直接调用)、用共享文档(黑板)、用即时通讯群(消息总线)、通过项目经理协调(" | 引入 |
| 0:20 | 概念图解 | "紧耦合，简单但不灵活" | 直接调用 |
| 0:45 | 对比表格 | "通过共享状态通信" | 共享黑板 |
| 1:15 | 代码截图 | "发布订阅，事件驱动" | 消息总线 |
| 1:45 | 总结卡 | "记住三个词：直接调用、共享黑板、消息总线" | 收尾 |
