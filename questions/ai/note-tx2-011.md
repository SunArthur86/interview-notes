...
**30 秒电梯演讲：** 像酒店管理——每个客人住独立房间(会话隔离)，大堂/餐厅是共享资源要排队(并发控制)，客人的物品绝不能进别人房间(数据隔离)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【某讯面经】高并发多用户会话隔离设计 | "像酒店管理——每个客人住独立房间(会话隔离)，大堂/餐厅是共享资源要排队(并发控制)，客人的物品绝不" | 引入 |
| 0:20 | 概念图解 | "session_id维度Redis key + 每请求独立LLM上下文" | 会话隔离 |
| 0:45 | 对比表格 | "令牌桶限流(每用户QPS) + 信号量(LLM并发槽) + 队列" | 并发控制 |
| 1:15 | 代码截图 | "user_id做row-level权限，绝不交叉" | 数据隔离 |
| 1:45 | 总结卡 | "记住三个词：会话隔离、并发控制、数据隔离" | 收尾 |
