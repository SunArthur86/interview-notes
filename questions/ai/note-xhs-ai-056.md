...
**30 秒电梯演讲：** 就像餐厅菜单推荐——如果现做推荐菜（调LLM）要等1分钟，顾客会不耐烦。所以准备三手：已做好的招牌菜（缓存）、先上凉菜再上热菜（流式返回）、万一厨房忙不过来就推荐当日套餐（兜底文案）。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多一面】商品详情页接入"智能推荐文案"AI接口，如何设 | "就像餐厅菜单推荐——如果现做推荐菜（调LLM）要等1分钟，顾客会不耐烦。所以准备三手：已做好的招牌菜" | 引入 |
| 0:20 | 概念图解 | "LLM生成第一个token就返回，用户体感延迟降至200ms" | 流式返回（SSE） |
| 0:45 | 对比表格 | "商品特征哈希→缓存命中文案，避免重复调用LLM" | 多级缓存 |
| 1:15 | 代码截图 | "300ms超时→返回模板文案，500ms→返回静态文案" | 超时降级 |
| 1:45 | 总结卡 | "记住三个词：流式返回（SSE）、多级缓存、超时降级" | 收尾 |
