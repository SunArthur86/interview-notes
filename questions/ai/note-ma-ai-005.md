...
**30 秒电梯演讲：** 多Agent并行开发就像多人协作装修——Task Lock是'先到先得锁定房间'，自动校验是'装修完验收(水电/墙面)'，语义冲突检测是'检查两个人的装修方案在功能上是否冲突(比如都改了同一根水管)'
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【后端开发二面】多个Agent并行开发如何保证代码一致性？ | "多Agent并行开发就像多人协作装修——Task Lock是'先到先得锁定房间'，自动校验是'装修完" | 引入 |
| 0:20 | 概念图解 | "防止多Agent同时修改同一公共模块" | Task Lock(逻辑锁) |
| 0:45 | 对比表格 | "TypeScript编译、ESLint、单元测试、E2E" | 代码层面校验 |
| 1:15 | 代码截图 | "Git Merge能通过但语义矛盾(如两个Agent对同一接口定义不一致)" | 语义冲突 |
| 2:15 | 总结卡 | "记住三个词：Task Lock(逻辑锁)、代码层面校验、语义冲突" | 收尾 |
