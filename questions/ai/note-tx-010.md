...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【腾讯面经】在分层上下文管理机制中，各层级分别负责什么内容？ | "就像人脑的记忆系统——工作记忆（当前任务）→短期记忆（本次会话）→长期记忆（项目知识库）。" | 引入 |
| 0:20 | 概念图解 | "）" | 即时层（当前对话 |
| 0:45 | 对比表格 | "下文）" | 会话层（工作区上 |
| 1:15 | 代码截图 | "文档）" | 项目层（代码库+ |
| 2:15 | 总结卡 | "记住三个词：即时层（当前对话、会话层（工作区上、项目层（代码库+" | 收尾 |
