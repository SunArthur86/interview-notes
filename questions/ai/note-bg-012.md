...
**30 秒电梯演讲：** Function Call是"打电话的能力"（基础机制）。MCP是"统一的电话协议标准"（让所有电话能互通）。Skill是"预设的快捷拨号+通话脚本"（打包好的复杂任务流程）。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【八股总结】MCP、Skill、Function Call  | "Function Call是"打电话的能力"（基础机制）。MCP是"统一的电话协议标准"（让所有电话" | 引入 |
| 0:20 | 概念图解 | "LLM输出结构化{tool_name, arguments}的能力（底层）" | Function Call |
| 0:45 | 对比表格 | "标准化的工具/数据源接入协议（中间层，解决碎片化）" | MCP |
| 1:15 | 代码截图 | "封装好的任务能力包（应用层，解决复杂编排）" | Skill |
| 1:45 | 总结卡 | "记住三个词：Function Call、MCP、Skill" | 收尾 |
