...
**30 秒电梯演讲：** 像在书里找最长的不重复段落——用两个手指夹住一段（窗口），右手指往右滑，遇到重复的字就把左手指直接跳到上次这个字之后，继续找。不用每次左手指一格一格挪，直接跳过去最快。

**展开框架：**
1. **滑动窗口[left** — right]+哈希表存字符最新下标
2. **right右扩遇重复** — left跳到重复字符上次位置+1
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】手撕：求字符串最长无重复字符子串的长度 | "像在书里找最长的不重复段落——用两个手指夹住一段（窗口），右手指往右滑，遇到重复的字就把左手指直接跳" | 引入 |
| 0:20 | 概念图解 | "right]+哈希表存字符最新下标" | 滑动窗口[left |
| 0:45 | 对比表格 | "left跳到重复字符上次位置+1" | right右扩遇重复 |
| 1:15 | 总结卡 | "记住三个词：滑动窗口[left、right右扩遇重复、关键优化" | 收尾 |
