...
**30 秒电梯演讲：** 就像备课——老师第一次讲新课需要认真备课（Prefill计算），但如果三个班上同一门课，备课内容可以复用（Prefix Cache），只需要为每个班补充不同的习题（Decode阶段）。

**展开框架：**
1. **Prefix** — Cache保存公共前缀的KV状态，跳过重复Prefill计算
2. **普通KV Cache是单序列的** — Prefix Cache跨请求共享
3. **缓存键需绑定to** — kenizer、模型版本、Adapter等参数

**收尾：** Prefix Cache的命中率受什么影响？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【AI Infra面经】Prefix Cache 的工作原理 | "就像备课——老师第一次讲新课需要认真备课（Prefill计算），但如果三个班上同一门课，备课内容可以" | 引入 |
| 0:20 | 概念图解 | "Cache保存公共前缀的KV状态，跳过重复Prefill计算" | Prefix |
| 0:45 | 对比表格 | "Prefix Cache跨请求共享" | 普通KV Cache是单序列的 |
| 1:15 | 代码截图 | "kenizer、模型版本、Adapter等参数" | 缓存键需绑定to |
| 1:45 | 总结卡 | "记住三个词：Prefix、普通KV Cache是单序列的、缓存键需绑定to" | 收尾 |
