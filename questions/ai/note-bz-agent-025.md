...
**30 秒电梯演讲：** 像餐厅服务——当前桌的订单(热/Redis)、今晚的所有桌(温/内存)、历史所有订单(冷/DB)，按访问频率分层。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：实时对话系统的状态管理方案？ | "像餐厅服务——当前桌的订单(热/Redis)、今晚的所有桌(温/内存)、历史所有订单(冷/DB)，按" | 引入 |
| 0:20 | 概念图解 | "会话(Redis)/上下文(LLM)/业务(DB)" | 三层状态 |
| 0:45 | 对比表格 | "热数据快、温数据全、冷数据省" | 核心原则 |
| 1:15 | 代码截图 | "最终一致性+冲突解决" | 状态同步 |
| 1:45 | 总结卡 | "记住三个词：三层状态、核心原则、状态同步" | 收尾 |
