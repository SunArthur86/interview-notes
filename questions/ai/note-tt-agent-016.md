...
**30 秒电梯演讲：** 就像训练新员工打电话——给他清晰的话术模板（Prompt），标准的操作手册格式（Schema），几个成功案例参考（Few-shot），以及犯了错怎么纠正的机制（异常重试）
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：怎么提升Function Call准确率？Prompt、Sc | "就像训练新员工打电话——给他清晰的话术模板（Prompt），标准的操作手册格式（Schema），几个" | 引入 |
| 0:20 | 概念图解 | "工具选择决策树、使用时机约束、负面示例" | Prompt维度 |
| 0:45 | 对比表格 | "参数描述精确化、枚举值约束、必填校验" | Schema维度 |
| 1:15 | 代码截图 | "正例+反例+边界case的示范" | Few-shot维度 |
| 1:45 | 总结卡 | "记住三个词：Prompt维度、Schema维度、Few-shot维度" | 收尾 |
