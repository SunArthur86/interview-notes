...
**30 秒电梯演讲：** 就像医院的实习医生——可以自己开常规检查单(低风险自主)，但手术方案必须主治医生签字(高风险需审批)，所有操作都记录在病历中(审计日志)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：告警Agent如何控制风险，人工在哪一步介入？ | "就像医院的实习医生——可以自己开常规检查单(低风险自主)，但手术方案必须主治医生签字(高风险需审批)" | 引入 |
| 0:20 | 概念图解 | "只读(自主) → 低危写(自主+日志) → 高危写(人工审批) → 不可逆(禁止)" | 风险分级 |
| 0:45 | 对比表格 | "高风险操作前的审批门 + 执行失败后的升级 + 定期审计" | 人工介入点 |
| 1:15 | 代码截图 | "操作白名单 + 参数范围限制 + 回滚机制" | 安全护栏 |
| 2:15 | 总结卡 | "记住三个词：风险分级、人工介入点、安全护栏" | 收尾 |
