...
**30 秒电梯演讲：** 就像建购物中心——先建好商铺标准（产物协议：统一面积/水电接口/消防规范）、公共区域（产物面板：走廊/电梯/卫生间）、管理系统（版本/权限/导出），之后招商（新Agent）只需要按标准装修入驻，不用自己从地基开始盖。

**展开框架：**
1. **统一产物对象模型** — 是最先该平台化的
2. **产物渲染/编辑/** — 导出做成可扩展的渲染器注册表
3. **新Agent接入** — 只需注册产物类型和适配器
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】如果产品要扩到更多桌面能力，哪层前端架构最该 | "就像建购物中心——先建好商铺标准（产物协议：统一面积/水电接口/消防规范）、公共区域（产物面板：走廊" | 引入 |
| 0:20 | 概念图解 | "是最先该平台化的" | 统一产物对象模型 |
| 0:45 | 对比表格 | "导出做成可扩展的渲染器注册表" | 产物渲染/编辑/ |
| 1:15 | 代码截图 | "只需注册产物类型和适配器" | 新Agent接入 |
| 2:15 | 总结卡 | "记住三个词：统一产物对象模型、产物渲染/编辑/、新Agent接入" | 收尾 |
