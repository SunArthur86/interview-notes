...
**30 秒电梯演讲：** 就像厨师做菜前先检查食材——如果发现鸡蛋过期了、盐用完了，会先告诉你"这些食材可能影响菜品质量"，而不是闷头做出一桌难吃的菜再让你排查原因。

**展开框架：**
1. **在执行前展示引用** — 文件摘要和关键字段
2. **对缺失权限、空文** — 件或异常格式做前置校验
3. **提示用户当前上下** — 文可能不完整或过期
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】如果用户给了错误的文件上下文，前端怎样尽早发 | "就像厨师做菜前先检查食材——如果发现鸡蛋过期了、盐用完了，会先告诉你"这些食材可能影响菜品质量"，而" | 引入 |
| 0:20 | 概念图解 | "文件摘要和关键字段" | 在执行前展示引用 |
| 0:45 | 对比表格 | "件或异常格式做前置校验" | 对缺失权限、空文 |
| 1:15 | 代码截图 | "文可能不完整或过期" | 提示用户当前上下 |
| 1:45 | 总结卡 | "记住三个词：在执行前展示引用、对缺失权限、空文、提示用户当前上下" | 收尾 |
