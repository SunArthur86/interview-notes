...
**30 秒电梯演讲：** 就像快递提速——用更快的车（模型层：小模型量化），优化配送路线（检索层：缓存+预检索），同时分拣打包（工程层：并行+流式输出）
...
**收尾：** Speculative Decoding在Agent场景的效果如何？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：自研Agent多步推理带来高延迟，从模型、检索、工程三层给出 | "就像快递提速——用更快的车（模型层：小模型量化），优化配送路线（检索层：缓存+预检索），同时分拣打包" | 引入 |
| 0:20 | 概念图解 | "7B蒸馏替代32B、INT4量化、KV Cache复用" | 模型层 |
| 0:45 | 对比表格 | "Query缓存、异步预检索、Embedding本地化" | 检索层 |
| 1:15 | 代码截图 | "无依赖步骤并行化、流式输出、Speculative Decoding" | 工程层 |
| 2:15 | 总结卡 | "记住三个词：模型层、检索层、工程层" | 收尾 |
