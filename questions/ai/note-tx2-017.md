...
**30 秒电梯演讲：** 像公司开会——人越多沟通越乱(N²)，解法是：减少会议人数(合并)、用文档协同而非开会(共享状态)、异步沟通不阻塞(消息总线)、用模板填表而非自由讨论(结构化)、按层级汇报(Manager-Worker)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【某讯面经】多 Agent 协作中通信成本高怎么办？ | "像公司开会——人越多沟通越乱(N²)，解法是：减少会议人数(合并)、用文档协同而非开会(共享状态)、" | 引入 |
| 0:20 | 概念图解 | "通信路径N²增长 + 每条消息要LLM理解" | 根源 |
| 0:45 | 对比表格 | "减少Agent数(合并职责相似)" | 解法1 |
| 1:15 | 代码截图 | "共享状态而非直接对话(避免N²)" | 解法2 |
| 2:15 | 总结卡 | "记住三个词：根源、解法1、解法2" | 收尾 |
