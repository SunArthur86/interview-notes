...
**30 秒电梯演讲：** 就像让学生填表格——Function Calling是给标准表格(强约束)，约束解码是限制学生只能填特定选项(token级约束)，后处理是交上来后老师帮改格式。

**展开框架：**
1. **Function** — 厂商原生支持，最稳定
2. **Constrai** — GBNF/Outlines，token级约束
3. **JSON Mode** — OpenAI response_format

**收尾：** Outlines的GBNF语法怎么写？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】让模型稳定输出符合 Schema 的 JSON  | "就像让学生填表格——Function Calling是给标准表格(强约束)，约束解码是限制学生只能填" | 引入 |
| 0:20 | 概念图解 | "厂商原生支持，最稳定" | Function |
| 0:45 | 对比表格 | "GBNF/Outlines，token级约束" | Constrai |
| 1:15 | 代码截图 | "OpenAI response_format" | JSON Mode |
| 1:45 | 总结卡 | "记住三个词：Function、Constrai、JSON Mode" | 收尾 |
