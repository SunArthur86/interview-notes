...
**30 秒电梯演讲：** 就像会议纪要——每轮对话不用重新记录所有历史发言(KV Cache缓存)，直接引用之前的纪要。换了话题(前缀变化)就要重新记(缓存未命中)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】KV Cache 的核心原理是什么？在多轮对话场 | "就像会议纪要——每轮对话不用重新记录所有历史发言(KV Cache缓存)，直接引用之前的纪要。换了话" | 引入 |
| 0:20 | 概念图解 | "缓存历史层的K和V矩阵" | KV Cache |
| 0:45 | 对比表格 | "前缀变化/对话轮次多" | 命中率低原因 |
| 1:15 | 代码截图 | "缓存系统提示等公共前缀" | Prefix Caching |
| 2:15 | 总结卡 | "记住三个词：KV Cache、命中率低原因、Prefix Caching" | 收尾 |
