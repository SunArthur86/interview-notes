...
**30 秒电梯演讲：** Function Calling像培训新员工用内部工具——光给说明书不够，还要演示几遍(Few-shot)，员工填错表单时让他重填(校验重试)。长对话压缩像会议纪要——核心决议常驻白板(常驻)，讨论细节定期摘要归档(压缩)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【快手AI大模型】Function Calling准确率怎么 | "Function Calling像培训新员工用内部工具——光给说明书不够，还要演示几遍(Few-sh" | 引入 |
| 0:20 | 概念图解 | "参数名/类型/描述/示例四要素齐全" | 工具描述结构化 |
| 0:45 | 对比表格 | "给出正确调用的input→output示例" | Few-shot示例 |
| 1:15 | 代码截图 | "校验失败→返回错误信息→LLM修正重试" | 参数校验+重试 |
| 1:45 | 总结卡 | "记住三个词：工具描述结构化、Few-shot示例、参数校验+重试" | 收尾 |
