...
**30 秒电梯演讲：** 普通RAG像查字典（找到相关词条），GraphRAG像查家谱（能找到表哥的岳父的母校这种多跳关系）。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【腾讯面经】GraphRAG 跟普通 RAG 的区别是什么？ | "普通RAG像查字典（找到相关词条），GraphRAG像查家谱（能找到表哥的岳父的母校这种多跳关系）。" | 引入 |
| 0:20 | 概念图解 | "向量相似度+局部片段" | 普通RAG |
| 0:45 | 对比表格 | "图遍历+多跳推理+全局摘要" | GraphRAG |
| 1:15 | 代码截图 | "多跳问答/关系推理/全局总结" | 适合 |
| 1:45 | 总结卡 | "记住三个词：普通RAG、GraphRAG、适合" | 收尾 |
