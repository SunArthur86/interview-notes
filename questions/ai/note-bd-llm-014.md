...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  └──────┬───────┘
         ...
         ▼
  ┌──────────────┐         ┌──────────────────┐
  │  CDC 监听层   │────────→│  消息队列 (Kafka)  │  文档变更事件流
  │  (Debezium)  │  事件    │  Topic: doc-events │
  └──────────────┘         └────────┬─────────┘
                                    │ ② 消费变更事件
                                    ▼
                           ┌──────────────────┐
                           │  变更处理器        │  ③ 去重/合并/批处理
                           │  (Change Processor)│  窗口聚合 (500ms)
                           └────────┬─────────┘
                                    │ ④ 仅处理变更文档
                                    ▼
              ┌─────────────────────────────────────┐
              │       增量 Embedding 服务            │
              │  ┌─────────┐    ┌──────────────┐    │
              │  │ Chunking │───→│ Embedding API │    │ ⑤ 只对变更文档做向量化
              │  │ Splitter │    │ (Batch Call)  │    │
              │  └─────────┘    └──────┬───────┘    │
              └────────────────────────┼─────────────┘
                                       │ ⑥ 写入新版本索引
...
**30 秒电梯演讲：** 就像超市补货——不是每天关门重新盘点(全量重建)，而是有新货到就实时上架(增量更新)，同时定期对账(一致性校验)。

**展开框架：**
1. **CDC(Change** — Data Capture)监听文档变更
2. **增量Embedd** — ing(只处理变更文档)
3. **双写策略(新索引** — +旧索引并行)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】业务知识库每天有大量文档新增和修改，如何设计一套 | "就像超市补货——不是每天关门重新盘点(全量重建)，而是有新货到就实时上架(增量更新)，同时定期对账(" | 引入 |
| 0:20 | 概念图解 | "Data Capture)监听文档变更" | CDC(Change |
| 0:45 | 对比表格 | "ing(只处理变更文档)" | 增量Embedd |
| 1:15 | 代码截图 | "+旧索引并行)" | 双写策略(新索引 |
| 2:15 | 总结卡 | "记住三个词：CDC(Change、增量Embedd、双写策略(新索引" | 收尾 |
