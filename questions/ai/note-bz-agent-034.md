...
**30 秒电梯演讲：** 像超市找商品——有清晰分区(分类)、商品标签(描述)、导购指引(检索)、你买过记住(反馈)，才能快速找到。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Agent Skill/工具过多，如何提升命中率？ | "像超市找商品——有清晰分区(分类)、商品标签(描述)、导购指引(检索)、你买过记住(反馈)，才能快速" | 引入 |
| 0:20 | 概念图解 | "选择空间大/描述不精/缺乏示例" | 原因 |
| 0:45 | 对比表格 | "分类索引+描述优化+RAG+few-shot+反馈学习" | 解法 |
| 1:15 | 代码截图 | "命中率=正确选择/总选择" | 评估 |
| 1:45 | 总结卡 | "记住三个词：原因、解法、评估" | 收尾 |
