...
**30 秒电梯演讲：** 通用基模像"通识教育的本科生"——什么都学一点，知识面广但都不深。业务基模像"定向培养的专业硕士"——在通识基础上，针对某行业/场景深度训练。评估本科生看综合成绩，评估专硕看专业对口就业。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【八股总结】业务基模和通用基模有什么区别？业务基模效果如何评 | "通用基模像"通识教育的本科生"——什么都学一点，知识面广但都不深。业务基模像"定向培养的专业硕士"—" | 引入 |
| 0:20 | 概念图解 | "垂直领域数据 + 业务定制能力 + 业务指标评估" | 业务基模 |
| 0:45 | 对比表格 | "开放领域数据 + 通用能力 + benchmark评估" | 通用基模 |
| 1:15 | 代码截图 | "业务指标(核心) + 通用benchmark(参考) + 在线A/B(终极)" | 评估三维度 |
| 2:15 | 总结卡 | "记住三个词：业务基模、通用基模、评估三维度" | 收尾 |
