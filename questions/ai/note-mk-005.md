...
**30 秒电梯演讲：** 就像公司管理——员工工位上的东西（页面层）随时可以清理，但项目档案和合同（任务层）必须存在公司档案室，不会因为换了工位就丢失。

**展开框架：**
1. **输入框** — 输入框、浮层和选中态留在页面层
2. **任务状态** — 任务状态、产物状态和文件索引状态上升到任务层
3. **长任务不要绑** — 长任务不要绑在单个组件生命周期上
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Vue 做桌面 AI 产品时，哪些状态应该在页面… | "就像公司管理——员工工位上的东西（页面层）随时可以清理，但项目档案和合同（任务层）必须存在…" | 开场钩子 |
| 0:20 | 核心概念图 | "桌面AI产品需要严格的状态分层——输入框、浮层、选中态等短暂交互状态留在页面层；任务状态、产物状态、文件索引状态上升到任…" | 核心定义 |
| 0:50 | 输入框、浮示意图 | "输入框、浮——输入框、浮层和选中态留在页面层" | 要点拆解1 |
| 1:30 | 任务状态示意图 | "任务状态——任务状态、产物状态和文件索引状态上升到任务层" | 要点拆解2 |
