...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】Vue 做桌面 AI 产品时，哪些状态应该在 | "就像公司管理——员工工位上的东西（页面层）随时可以清理，但项目档案和合同（任务层）必须存在公司档案室" | 引入 |
| 0:20 | 概念图解 | "中态留在页面层" | 输入框、浮层和选 |
| 0:45 | 对比表格 | "态和文件索引状态上升到任务层" | 任务状态、产物状 |
| 1:15 | 代码截图 | "个组件生命周期上" | 长任务不要绑在单 |
| 2:15 | 总结卡 | "记住三个词：输入框、浮层和选、任务状态、产物状、长任务不要绑在单" | 收尾 |
