...
3. **Overlap解** — 决的是"硬切分"问题，更好的方案是语义切分
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：文档切片为什么要有Overlap？它主要解决什么问题？ | "就像装修贴瓷砖留重叠缝——每块瓷砖边缘和下一块重叠一小段，这样即使某块有瑕疵，整体也不会出现明显的缝" | 引入 |
| 0:20 | 概念图解 | "chunk_size=512, overlap=50-100 tokens" | 标准配置 |
| 0:45 | 对比表格 | "—增加存储和计算成本" | Overlap不是越大越好 |
| 1:15 | 总结卡 | "记住三个词：标准配置、Overlap不是越大越好、Overlap解" | 收尾 |
