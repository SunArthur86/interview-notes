...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何确定Chunk的大小和切分策略？过大或过小分别会带来什么 | "像切蛋糕——切太大一坨吃不下（检索噪声多），切太小碎渣到处是（语义不完整）。要沿着蛋糕的"自然纹理"" | 引入 |
| 0:20 | 概念图解 | "简单但可能切断语义" | 固定切分 |
| 0:45 | 对比表格 | "按段落/句子边界，保留语义完整性" | 语义切分 |
| 1:15 | 代码截图 | "256-512 tokens，overlap 10-20%" | 常见范围 |
| 1:45 | 总结卡 | "记住三个词：固定切分、语义切分、常见范围" | 收尾 |
