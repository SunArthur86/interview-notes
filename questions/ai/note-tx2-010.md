...
**30 秒电梯演讲：** 像人的记忆系统——短期是工作记忆(正在想的事，容量小忘得快)，长期是经历沉淀(人生的经验)，向量是联想记忆(看到X想起相关的Y)。避免污染就是别把每个念头都记下来(摘要)、别让负面记忆干扰当下(scratchpad隔离)、忘掉没用的事...

**展开框架：**
1. **三层** — 短期(会话,Redis)/长期(画像,PG)/向量(知识,向量库)
2. **避免污染** — 摘要后写回/独立scratchpad/相关性召回/遗忘机制
3. **短期TTL短** — 长期持久，向量按相关性召回top-K
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【某讯面经】设计 Agent 的 Memory 系统（短期+ | "像人的记忆系统——短期是工作记忆(正在想的事，容量小忘得快)，长期是经历沉淀(人生的经验)，向量是联" | 引入 |
| 0:20 | 概念图解 | "短期(会话,Redis)/长期(画像,PG)/向量(知识,向量库)" | 三层 |
| 0:45 | 对比表格 | "摘要后写回/独立scratchpad/相关性召回/遗忘机制" | 避免污染 |
| 1:15 | 代码截图 | "长期持久，向量按相关性召回top-K" | 短期TTL短 |
| 2:15 | 总结卡 | "记住三个词：三层、避免污染、短期TTL短" | 收尾 |
