...
**30 秒电梯演讲：** Agent Harness就像飞机的黑匣子+自动驾驶测试台——记录每一步操作、实时监控状态、出事了能回放复现
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：从工程化角度看，Agent Harness主要解决哪些问题？ | "Agent Harness就像飞机的黑匣子+自动驾驶测试台——记录每一步操作、实时监控状态、出事了能" | 引入 |
| 0:20 | 概念图解 | "任务完成率、Tool成功率、延迟和成本度量" | 评测 |
| 0:45 | 对比表格 | "全链路Trace，记录每步Prompt和输出" | 观测 |
| 1:15 | 代码截图 | "从检查点恢复和复现执行过程" | 回放 |
| 1:45 | 总结卡 | "记住三个词：评测、观测、回放" | 收尾 |
