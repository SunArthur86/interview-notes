...
**30 秒电梯演讲：** 就像处理一封信和一张Excel表格——信按段落拆开读，表格按行列理解，不能用同一把剪刀。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】如果用户的文档同时包含大段正文和密集的表格数据， | "就像处理一封信和一张Excel表格——信按段落拆开读，表格按行列理解，不能用同一把剪刀。" | 引入 |
| 0:20 | 概念图解 | "语义切片(段落/标题/滑动窗口)" | 正文 |
| 0:45 | 对比表格 | "结构化保留(Markdown/HTML格式)" | 表格 |
| 1:15 | 代码截图 | "多模态解析(VLM识别)" | 图表 |
| 2:15 | 总结卡 | "记住三个词：正文、表格、图表" | 收尾 |
