...
**30 秒电梯演讲：** 像老师出题——如果全班都做对（题太简单）或全班都做错（题太难），这道题没有区分度，白出了。但只出"有区分度"的中等题，又会让差生永远学不会难题。解法是循序渐进地加难度、给差生多几次尝试机会、或把难题拆成步骤分步给分。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【八股总结】DAPO 动态采样提升效率 & 零奖励样本剔除的 | "像老师出题——如果全班都做对（题太简单）或全班都做错（题太难），这道题没有区分度，白出了。但只出"有" | 引入 |
| 0:20 | 概念图解 | "跳过advantage全为0的prompt，只训有信号的" | 动态采样 |
| 0:45 | 对比表格 | "困难样本（全错）学不到" | 零奖励样本剔除的代价 |
| 1:15 | 代码截图 | "课程学习，先易后难，让模型逐步能做对部分" | 解法1 |
| 2:15 | 总结卡 | "记住三个词：动态采样、零奖励样本剔除的代价、解法1" | 收尾 |
