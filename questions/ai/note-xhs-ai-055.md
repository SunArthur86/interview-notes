...
**30 秒电梯演讲：** MCP就像给大模型配了一个标准化的"遥控器协议"。不管你家里是格力还是美的的空调，遥控器格式都一样（协议标准化）。大模型只需要知道"调温度需要哪些参数"（schema），然后从你的话里提取这些参数，按下遥控器（调用接口），最后把结果显...

**展开框架：**
1. **MCP四步流程** — 意图识别→参数匹配→协议封装调用→结果整理
2. **LLM参数匹配靠** — 理解schema语义 + Few-shot示例约束格式
3. **MCP标准化了工** — 具描述、参数schema、调用格式、返回格式

**收尾：** MCP和OpenAI Function Calling有什么区别？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【高德AI面试】讲一下MCP调用全过程，LLM是怎么匹配工具 | "MCP就像给大模型配了一个标准化的"遥控器协议"。不管你家里是格力还是美的的空调，遥控器格式都一样（" | 引入 |
| 0:20 | 概念图解 | "意图识别→参数匹配→协议封装调用→结果整理" | MCP四步流程 |
| 0:45 | 对比表格 | "理解schema语义 + Few-shot示例约束格式" | LLM参数匹配靠 |
| 1:15 | 代码截图 | "具描述、参数schema、调用格式、返回格式" | MCP标准化了工 |
| 1:45 | 总结卡 | "记住三个词：MCP四步流程、LLM参数匹配靠、MCP标准化了工" | 收尾 |
