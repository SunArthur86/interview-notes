...
**30 秒电梯演讲：** 像水管爆裂排查——先关总阀(确认模型大小)，再查各支管(KV Cache/激活值/并发数)，最后找泄漏点(框架泄漏/碎片化)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：大模型推理时遇到OOM问题，你会从哪些方面入手排查和解决？ | "像水管爆裂排查——先关总阀(确认模型大小)，再查各支管(KV Cache/激活值/并发数)，最后找泄" | 引入 |
| 0:20 | 概念图解 | "确认模型大小是否匹配GPU" | 第一优先 |
| 0:45 | 对比表格 | "KV Cache随序列长度线性增长" | 第二优先 |
| 1:15 | 代码截图 | "并发请求 × 每请求KV Cache" | 第三优先 |
| 1:45 | 总结卡 | "记住三个词：第一优先、第二优先、第三优先" | 收尾 |
