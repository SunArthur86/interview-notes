...
**30 秒电梯演讲：** 就像找师傅修水管——你描述问题(LLM理解意图)，师傅选工具(扳手还是胶带)，按规格操作(构造参数)，看结果对不对(Observation)，不对换方法

**展开框架：**
1. **LLM本身不执行代码** — 只输出"调用意图"(函数名+参数JSON)
2. **判断是否需要调用工具** — 通过System Prompt中的工具描述让LLM自行决策
3. **执行环境(Runtime)负责** — 解析指令→执行函数→返回结果
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Agent工具调用的完整机制和判断逻辑 | "就像找师傅修水管——你描述问题(LLM理解意图)，师傅选工具(扳手还是胶带)，按规格操作(构造参数)" | 引入 |
| 0:20 | 概念图解 | "只输出"调用意图"(函数名+参数JSON)" | LLM本身不执行代码 |
| 0:45 | 对比表格 | "通过System Prompt中的工具描述让LLM自行决策" | 判断是否需要调用工具 |
| 1:15 | 代码截图 | "解析指令→执行函数→返回结果" | 执行环境(Runtime)负责 |
| 1:45 | 总结卡 | "记住三个词：LLM本身不执行代码、判断是否需要调用工具、执行环境(Runtime)负责" | 收尾 |
