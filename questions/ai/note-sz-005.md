...
**30 秒电梯演讲：** 普通聚合像把全班分数算个平均分（只出一个数），窗口函数像给每个学生旁边贴一列"班级平均分""班级第几名""比上次进步多少"——既保留每个学生明细，又能看到他在班级里的位置。
...
**收尾：** ROW_NUMBER/RANK/DENSE_RANK 怎么选？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【神州专车面经】开窗函数用过哪些？举个实际场景 | "普通聚合像把全班分数算个平均分（只出一个数），窗口函数像给每个学生旁边贴一列"班级平均分""班级第几" | 引入 |
| 0:20 | 概念图解 | "SUM/AVG/COUNT OVER(PARTITION BY) 保留明细算聚合" | 聚合类 |
| 0:45 | 对比表格 | "ROW_NUMBER(连续不重复)/RANK(重复跳号)/DENSE_RANK(重复不跳号)" | 排名类 |
| 1:15 | 总结卡 | "记住三个词：聚合类、排名类、偏移类" | 收尾 |
