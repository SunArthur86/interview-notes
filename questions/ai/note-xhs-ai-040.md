...
**30 秒电梯演讲：** LLM像一个过度自信的实习生——你说'帮我订机票'，他可能瞎编一个'超级订票API'来调用。防护措施：1) 工具说明书(白名单)——只有列表里的工具能用；2) 参数检查(schema校验)——调用参数必须合规；3) Plan B(降级...
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【AI Agent工程】模型幻觉调用不存在API如何拦截？工 | "LLM像一个过度自信的实习生——你说'帮我订机票'，他可能瞎编一个'超级订票API'来调用。防护措施" | 引入 |
| 0:20 | 概念图解 | "LLM输出的function call参数必须通过JSON Schema验证" | Schema校验 |
| 0:45 | 对比表格 | "只有预注册的工具能被调用，拒绝未注册的工具名" | 白名单机制 |
| 1:15 | 代码截图 | "LLM在训练数据中见过类似API名，会编造看似合理的调用" | 工具调用幻觉根因 |
| 1:45 | 总结卡 | "记住三个词：Schema校验、白名单机制、工具调用幻觉根因" | 收尾 |
