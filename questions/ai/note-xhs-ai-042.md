...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【AI Agent工程】复合需求怎么判断任务执行先后顺序？ | "做一顿年夜饭：红烧肉要炖2小时（先开始），蔬菜5分钟搞定（晚点做），汤要趁热喝（最后做）。你需要根据" | 引入 |
| 0:20 | 概念图解 | "→ 需要任务分解+排序" | 复合需求=多个子任务 |
| 0:45 | 对比表格 | "数据依赖（A的输出是B的输入）+ 时序依赖（必须先做A）" | 依赖分析 |
| 1:15 | 总结卡 | "记住三个词：复合需求=多个子任务、依赖分析、DAG拓扑排序" | 收尾 |
