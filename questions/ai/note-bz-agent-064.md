...
**30 秒电梯演讲：** 像画流程图——状态是节点，决策是分支，工具调用是动作，LangGraph把流程图变成可执行的Agent。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何基于 LangGraph 搭建一个 Agent 应用？ | "像画流程图——状态是节点，决策是分支，工具调用是动作，LangGraph把流程图变成可执行的Agen" | 引入 |
| 0:20 | 概念图解 | "定义State→建节点→连边→编译" | 四步 |
| 0:45 | 对比表格 | "Node(节点)/Edge(边)/State(状态)" | 核心概念 |
| 1:15 | 代码截图 | "循环/条件分支/人工节点/检查点" | 关键能力 |
| 1:45 | 总结卡 | "记住三个词：四步、核心概念、关键能力" | 收尾 |
