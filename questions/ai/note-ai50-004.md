...
**30 秒电梯演讲：** 就像招聘——HR先按简历关键词海选(向量检索)，然后技术面试官逐个深入评估(Reranker)。两步都不可少

**展开框架：**
1. **向量检索用Bi-Encoder** — Query和Doc分别编码，速度快可离线计算
2. **Reranker用Cross** — Query和Doc拼接后联合编码，精度高但只能在线计算
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：为什么要加Reranker重排？直接拿向量检索结果给模型会有 | "就像招聘——HR先按简历关键词海选(向量检索)，然后技术面试官逐个深入评估(Reranker)。两步" | 引入 |
| 0:20 | 概念图解 | "Query和Doc分别编码，速度快可离线计算" | 向量检索用Bi-Encoder |
| 0:45 | 对比表格 | "Query和Doc拼接后联合编码，精度高但只能在线计算" | Reranker用Cross |
| 1:15 | 代码截图 | "向量召回Top-50~100 → Reranker精排Top-5~10" | 工业标准 |
| 1:45 | 总结卡 | "记住三个词：向量检索用Bi-Encoder、Reranker用Cross、工业标准" | 收尾 |
