...
**30 秒电梯演讲：** 传统RAG像一个只会按菜谱做菜的厨师——给你什么食材就做什么菜。Agentic RAG像一个有判断力的厨师——发现食材不够会自己去买、尝味道不对会调整调料、一道菜需要多个步骤会按顺序完成。
...
**收尾：** Agentic RAG的延迟比传统RAG高很多，怎么平衡？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：为什么需要Agentic RAG？传统RAG加摘要召回不行吗 | "传统RAG像一个只会按菜谱做菜的厨师——给你什么食材就做什么菜。Agentic RAG像一个有判断力" | 引入 |
| 0:20 | 概念图解 | "query→检索→生成(单轮管道)" | 传统RAG |
| 0:45 | 对比表格 | "query→规划→迭代检索→判断→综合(多轮闭环)" | Agentic RAG |
| 1:15 | 代码截图 | "自主决定检索策略 + 多跳推理 + 动态终止判断" | 核心能力 |
| 2:15 | 总结卡 | "记住三个词：传统RAG、Agentic RAG、核心能力" | 收尾 |
