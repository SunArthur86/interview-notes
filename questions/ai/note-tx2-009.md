...
**30 秒电梯演讲：** 像客服中心运转——前台(路由Agent)分流，售后(工单Agent)查单，财务(退款Agent)退钱，主管(质检Agent)抽检。员工之间不直接喊话(走工单系统共享状态)，每个人超时了有备份接手(容错)。

**展开框架：**
1. **分工按角色** — 路由/工单/退款/质检 Agent
2. **通信走共享状态(** — 不直接对话)+消息总线异步
3. **状态管理** — 分布式存储+乐观锁(版本号)防冲突

**收尾：** 子 Agent 之间需要直接对话吗？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【某讯面经】设计一个多 Agent 协作系统（如客服中心）： | "像客服中心运转——前台(路由Agent)分流，售后(工单Agent)查单，财务(退款Agent)退钱" | 引入 |
| 0:20 | 概念图解 | "路由/工单/退款/质检 Agent" | 分工按角色 |
| 0:45 | 对比表格 | "不直接对话)+消息总线异步" | 通信走共享状态( |
| 1:15 | 代码截图 | "分布式存储+乐观锁(版本号)防冲突" | 状态管理 |
| 2:15 | 总结卡 | "记住三个词：分工按角色、通信走共享状态(、状态管理" | 收尾 |
