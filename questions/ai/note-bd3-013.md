...
**30 秒电梯演讲：** 像图书馆找书变难了——可能是新书没编目(数据缺失)、书架号变了(切片偏移)、目录索引太旧(索引退化)、或者你找的书换了个名字(Query漂移)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：RAG系统运行一段时间后召回效果变差，可能有哪些原因？如何设 | "像图书馆找书变难了——可能是新书没编目(数据缺失)、书架号变了(切片偏移)、目录索引太旧(索引退化)" | 引入 |
| 0:20 | 概念图解 | "新文档未索引、旧文档已删除但向量残留" | 数据层 |
| 0:45 | 对比表格 | "文档格式变化导致切分不一致" | 切片层 |
| 1:15 | 代码截图 | "Embedding模型版本不一致、向量精度损失" | 向量层 |
| 1:45 | 总结卡 | "记住三个词：数据层、切片层、向量层" | 收尾 |
