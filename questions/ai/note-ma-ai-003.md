...
**30 秒电梯演讲：** 短期记忆就像你的'便签纸'——记着当前正在做的事（当前任务的PRD、Design、Code）；长期记忆就像'笔记本'——记着项目结构、技术规范、过往经验，下次直接翻阅

**展开框架：**
1. **短期记忆** — 当前任务上下文，任务结束后清除或归档
2. **长期记忆** — 项目知识、用户偏好、历史决策，持久化存储
3. **短期存储在内存/Redis** — 长期存储在向量数据库/知识文件
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【后端开发二面】如何设计Agent Memory？长期记忆和 | "短期记忆就像你的'便签纸'——记着当前正在做的事（当前任务的PRD、Design、Code）；长期记" | 引入 |
| 0:20 | 概念图解 | "当前任务上下文，任务结束后清除或归档" | 短期记忆 |
| 0:45 | 对比表格 | "项目知识、用户偏好、历史决策，持久化存储" | 长期记忆 |
| 1:15 | 代码截图 | "长期存储在向量数据库/知识文件" | 短期存储在内存/Redis |
| 2:15 | 总结卡 | "记住三个词：短期记忆、长期记忆、短期存储在内存/Redis" | 收尾 |
