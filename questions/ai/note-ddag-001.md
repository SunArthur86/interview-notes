...
│                                                     │
│  多平台入口:                                        │
│  [Telegram] [Discord] [Feishu] [Terminal] [Web]    │
│         │        │        │          │         │    │
│         └────────┴────────┴──────────┴─────────┘    │
│                          │                          │
│                   ┌──────▼──────┐                   │
...
│                          │                          │
│              ┌───────────▼───────────┐              │
│              │   Dynamic Planner     │ ← 动态编排   │
│              │ (Plan→Delegate→Review)│              │
│              └───────────┬───────────┘              │
│                          │                          │
│    ┌─────────┬──────────┼──────────┬─────────┐     │
│    ▼         ▼          ▼          ▼         ▼     │
│ [Skills] [Tools]   [Sub-agents] [Plugins] [Cron]   │
│ (可扩展) (内置)    (委托)       (动态)   (定时)    │
│                                                     │
│  特点: 多场景 + 跨平台 + 动态编排 + 插件体系        │
└─────────────────────────────────────────────────────┘
```
...
**30 秒电梯演讲：** Claude Code像F1赛车的精密调校——在赛道(代码仓库)上追求极致性能；Hermes像多功能越野车——在不同地形(聊天平台/终端/浏览器)之间灵活切换，追求通用性和可扩展性。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：对比Claude Code和Hermes在设计模式上的区别 | "Claude Code像F1赛车的精密调校——在赛道(代码仓库)上追求极致性能；Hermes像多功能" | 引入 |
| 0:20 | 概念图解 | "单一场景(编码) + 深度集成(IDE/REPL) + 编排可控(Fixed Loop)" | Claude Code |
| 0:45 | 对比表格 | "多场景(通用) + 多平台(消息/终端/浏览器) + 动态编排(Planner-Executor)" | Hermes |
| 1:15 | 代码截图 | "专精 vs 通用 / 确定性 vs 灵活性" | 设计哲学差异 |
| 2:15 | 总结卡 | "记住三个词：Claude Code、Hermes、设计哲学差异" | 收尾 |
