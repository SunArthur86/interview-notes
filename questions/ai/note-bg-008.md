...
**30 秒电梯演讲：** TP像多人合作画一幅画的"同一部分"（每人画几笔颜色，画完拼一起）。PP像流水线工厂——每人负责一道工序，画传到下一人。DP像多个画师各画一幅相同的画，最后对比笔记统一风格。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【八股总结】TP/PP/DP 分布式策略 & TP 开大为什 | "TP像多人合作画一幅画的"同一部分"（每人画几笔颜色，画完拼一起）。PP像流水线工厂——每人负责一道" | 引入 |
| 0:20 | 概念图解 | "矩阵按列/行切分到多卡，层内AllReduce通信（频繁）" | TP |
| 0:45 | 对比表格 | "模型按层切分到多卡，层间P2P通信（流水线气泡）" | PP |
| 1:15 | 代码截图 | "完整模型副本，梯度AllReduce同步（批量）" | DP |
| 2:15 | 总结卡 | "记住三个词：TP、PP、DP" | 收尾 |
