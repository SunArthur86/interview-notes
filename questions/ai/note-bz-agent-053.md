...
**30 秒电梯演讲：** 像招聘——HR快速筛简历召回top-20(快但粗)，技术面试精挑top-5(慢但准)。

**展开框架：**
1. **两阶段** — 召回(Bi-Encoder快)→重排(Cross-Encoder准)
2. **Bi** — Encoder是query和doc分别编码
3. **Cross** — Encoder是query和doc拼接后一起编码
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：重排算法（Rerank）如何提升检索匹配精度？ | "像招聘——HR快速筛简历召回top-20(快但粗)，技术面试精挑top-5(慢但准)。" | 引入 |
| 0:20 | 概念图解 | "召回(Bi-Encoder快)→重排(Cross-Encoder准)" | 两阶段 |
| 0:45 | 对比表格 | "Encoder是query和doc分别编码" | Bi |
| 1:15 | 代码截图 | "Encoder是query和doc拼接后一起编码" | Cross |
| 1:45 | 总结卡 | "记住三个词：两阶段、Bi、Cross" | 收尾 |
