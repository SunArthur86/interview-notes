...
**30 秒电梯演讲：** AI Agent就像一个新来的客服——你给他一本工具手册（工具注册表），告诉他每个工具怎么用（schema描述），给他一个记事本记录和客户的对话（会话记忆），他就能根据客户问题自己决定用哪个工具、怎么用，最后给出回答。
...
**收尾：** Java实现Agent和Python（LangChain）比有什么优势和劣势？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多二面】用Java实现AI Agent：ToolCal | "AI Agent就像一个新来的客服——你给他一本工具手册（工具注册表），告诉他每个工具怎么用（sch" | 引入 |
| 0:20 | 概念图解 | "Java方法通过注解/接口声明为Tool，自动生成schema给LLM" | 工具注册 |
| 0:45 | 对比表格 | "LLM返回结构化JSON调用指令，Java反射执行对应方法" | Function |
| 1:15 | 代码截图 | "短期用对话列表（最近N轮），长期用向量数据库检索" | 会话记忆 |
| 2:15 | 总结卡 | "记住三个词：工具注册、Function、会话记忆" | 收尾 |
