...
**30 秒电梯演讲：** 就像公司规章——公司章程（CLAUDE.md，全公司遵守）、个人工作习惯笔记（~/.claude，只你用）、本次会议记录（会话 context，会开完就丢）。三层不混，各管各的。
...
**收尾：** Claude Code 的 memory/ 子目录（auto memory）怎么用 frontmatter 标 type？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节飞连面经】Claude Code 的 Memory 机 | "就像公司规章——公司章程（CLAUDE.md，全公司遵守）、个人工作习惯笔记（~/.claude，只" | 引入 |
| 0:20 | 概念图解 | "项目级 CLAUDE.md（进git）/ 用户级 ~/.claude/CLAUDE.md（不进git）/ 会话级 ..." | 三层 |
| 0:45 | 对比表格 | "团队约定 vs 个人习惯 vs 当前对话" | 职责隔离 |
| 1:15 | 代码截图 | "项目级 > 用户级（团队规矩压过个人喜好）" | 优先级 |
| 1:45 | 总结卡 | "记住三个词：三层、职责隔离、优先级" | 收尾 |
