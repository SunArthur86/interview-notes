...
**30 秒电梯演讲：** if-else调度像固定流程的工厂流水线——每个零件按固定路线走。ReAct像一个有判断力的工人——遇到问题先想想用什么工具、用完看看结果、不对再换一个。Plan-and-Solve像一个项目经理——先列出所有步骤和依赖关系，再按计划执行
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【AI Agent工程】多工具冲突除了if-else有什么动 | "if-else调度像固定流程的工厂流水线——每个零件按固定路线走。ReAct像一个有判断力的工人——" | 引入 |
| 0:20 | 概念图解 | "不灵活、无法应对新需求、维护成本高" | if-else硬编码的问题 |
| 0:45 | 对比表格 | "Thought→Action→Observation循环，LLM实时决策" | ReAct |
| 1:15 | 代码截图 | "先让LLM生成完整计划（DAG），再按拓扑序执行" | Plan-and-Solve |
| 1:45 | 总结卡 | "记住三个词：if-else硬编码的问题、ReAct、Plan-and-Solve" | 收尾 |
