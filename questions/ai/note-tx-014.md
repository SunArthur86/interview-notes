...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【腾讯面经】梯度检查点（Gradient Checkpoin | "就像考试时草稿纸不够——你只记下关键步骤（检查点），需要时再从头算一遍中间过程。" | 引入 |
| 0:20 | 概念图解 | "活" | 选择性保存中间激 |
| 0:45 | 对比表格 | "反向时重新计算" | 反向时重新计算 |
| 1:15 | 代码截图 | "显存减少~50%" | 显存减少~50% |
| 1:45 | 总结卡 | "记住三个词：选择性保存中间激、反向时重新计算、显存减少~50%" | 收尾 |
