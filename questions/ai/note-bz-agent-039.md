...
**30 秒电梯演讲：** 像开餐厅接入外卖平台——把菜品(能力)按平台标准(菜单格式)上架，所有平台的骑手(各种Agent)都能来取餐(调用)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何从 0 搭建一个专属 MCP 服务？ | "像开餐厅接入外卖平台——把菜品(能力)按平台标准(菜单格式)上架，所有平台的骑手(各种Agent)都" | 引入 |
| 0:20 | 概念图解 | "Server(工具提供方)+Client(Agent)" | MCP架构 |
| 0:45 | 对比表格 | "定义工具→实现Server→注册到Client" | 搭建步骤 |
| 1:15 | 代码截图 | "JSON-RPC标准协议" | 核心 |
| 1:45 | 总结卡 | "记住三个词：MCP架构、搭建步骤、核心" | 收尾 |
