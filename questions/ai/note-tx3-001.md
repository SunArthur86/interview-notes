...
**30 秒电梯演讲：** 就像写毕业论文——你不会把所有参考文献一字不差记在脑子里（塞满上下文），而是建一个研究笔记库（bible.md），每章只看需要的文献（按需检索），让同学帮你通读全文找问题（subagent）
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：用AI辅助写一本书，内容很长容易超过上下文长度，你怎么处理？ | "就像写毕业论文——你不会把所有参考文献一字不差记在脑子里（塞满上下文），而是建一个研究笔记库（bib" | 引入 |
| 0:20 | 概念图解 | "全书大纲+人物设定+世界观+文风要求" | 项目圣经（bible.md） |
| 0:45 | 对比表格 | "每次新会话自动读取项目配置" | CLAUDE.md自动加载 |
| 1:15 | 代码截图 | "写完生成200字摘要供后续章节参考" | 每章独立文件+摘要 |
| 2:15 | 总结卡 | "记住三个词：项目圣经（bible.md）、CLAUDE.md自动加载、每章独立文件+摘要" | 收尾 |
