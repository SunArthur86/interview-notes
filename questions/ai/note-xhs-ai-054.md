...
**30 秒电梯演讲：** 知识库就像客服的参考手册。如果手册里没有答案，绝对不能让客服自己编（幻觉），而应该：先仔细查查是不是翻漏了（调阈值重试），实在没有就诚实说"这个问题我暂时回答不了"（安全兜底），然后把这个记下来告诉主管补充进手册（闭环迭代）。

**展开框架：**
1. **绝对禁止模型在无** — —尤其涉及报价/规则/政策场景
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【高德AI面试】用户提问内容知识库完全没有覆盖，怎么处理？能 | "知识库就像客服的参考手册。如果手册里没有答案，绝对不能让客服自己编（幻觉），而应该：先仔细查查是不是" | 引入 |
| 0:20 | 概念图解 | "—尤其涉及报价/规则/政策场景" | 绝对禁止模型在无 |
| 0:45 | 对比表格 | "区分是真无数据还是检索失败（调阈值+多路召回验证）" | 第一步 |
| 1:15 | 代码截图 | "确认无匹配后明确告知用户，引导官方渠道" | 第二步 |
| 1:45 | 总结卡 | "记住三个词：绝对禁止模型在无、第一步、第二步" | 收尾 |
