...
**30 秒电梯演讲：** Agent像一个公司——编排引擎是CEO（决策），记忆是档案室，工具是各部门（执行），知识是公司知识库，安全是法务/审计。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【腾讯面经】你这个 Agent 项目怎么设计的？包含什么模块 | "Agent像一个公司——编排引擎是CEO（决策），记忆是档案室，工具是各部门（执行），知识是公司知识" | 引入 |
| 0:20 | 概念图解 | "编排/记忆/工具/知识/安全" | 5大模块 |
| 0:45 | 对比表格 | "路由到循环到聚合" | 编排=任务分解到 |
| 1:15 | 代码截图 | "权限控制+Trace+限流+人工确认" | 安全 |
| 1:45 | 总结卡 | "记住三个词：5大模块、编排=任务分解到、安全" | 收尾 |
