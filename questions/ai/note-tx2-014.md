...
**30 秒电梯演讲：** 像海关检查——先看护照是不是真的(json.loads格式)、再核对信息全不全(必填字段)、再看类型对不对(类型校验)、最后看有没有违禁品(业务规则)。不合格的退回去重新填。

**展开框架：**
1. **五类错误** — 字段缺失/类型错误/枚举越界/多余字段/格式错误
2. **Pydantic** — 定义schema自动校验，捕ValidationError分类返回
3. **校验流程** — json.loads解析 → Pydantic字段校验 → 业务校验

**收尾：** Pydantic v1 和 v2 的 ValidationError 区别？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【某讯面经】算法题：实现函数校验模型输出的 tool_cal | "像海关检查——先看护照是不是真的(json.loads格式)、再核对信息全不全(必填字段)、再看类型" | 引入 |
| 0:20 | 概念图解 | "字段缺失/类型错误/枚举越界/多余字段/格式错误" | 五类错误 |
| 0:45 | 对比表格 | "定义schema自动校验，捕ValidationError分类返回" | Pydantic |
| 1:15 | 总结卡 | "记住三个词：五类错误、Pydantic、校验流程" | 收尾 |
