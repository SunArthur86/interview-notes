...
**30 秒电梯演讲：** Multi-Agent就像软件开发团队——产品经理写PRD，架构师写设计文档，开发写代码，测试写测试用例。每个人只做自己的部分，通过文档(中间产物)传递信息。单Agent就像一个人从需求到上线全包了，容易出错
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【后端开发二面】为什么采用Multi-Agent而不是单Ag | "Multi-Agent就像软件开发团队——产品经理写PRD，架构师写设计文档，开发写代码，测试写测试" | 引入 |
| 0:20 | 概念图解 | "Prompt过长、上下文丢失、职责混乱、难以并行" | 单Agent瓶颈 |
| 0:45 | 对比表格 | "职责分离、并行执行、上下文隔离、可独立调试" | Multi-Agent优势 |
| 1:15 | 代码截图 | "PRD→Design→Task→Code→Test，用Markdown而非聊天记录" | 中间产物传递 |
| 2:15 | 总结卡 | "记住三个词：单Agent瓶颈、Multi-Agent优势、中间产物传递" | 收尾 |
