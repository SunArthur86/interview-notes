...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何对 Prompt 做版本控制与动态组装？ | "像管理法律条文——有版本(修订)、有模块(总则/分则)、按场景组合(民事用民法，刑事用刑法)。" | 引入 |
| 0:20 | 概念图解 | "git管理/语义版本/变更日志" | 版本控制 |
| 0:45 | 对比表格 | "Prompt片段独立管理" | 模块化 |
| 1:15 | 代码截图 | "按上下文拼接" | 动态组装 |
| 1:45 | 总结卡 | "记住三个词：版本控制、模块化、动态组装" | 收尾 |
