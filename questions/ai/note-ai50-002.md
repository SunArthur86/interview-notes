...
**30 秒电梯演讲：** 就像图书馆找书——找"封面图片特征"和找"书名文字"两条线索都要用，但不同类型的书侧重点不同

**展开框架：**
1. **图文检索本质是跨** — 模态对齐问题
2. **权重分配策略** — 静态阈值、动态路由、学习排序
3. **CLIP等模型可** — 以将图文映射到同一向量空间

**收尾：** CLIP模型在工业场景的局限性是什么？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：多模态检索中图文向量混合检索的权重分配 | "就像图书馆找书——找"封面图片特征"和找"书名文字"两条线索都要用，但不同类型的书侧重点不同" | 引入 |
| 0:20 | 概念图解 | "模态对齐问题" | 图文检索本质是跨 |
| 0:45 | 对比表格 | "静态阈值、动态路由、学习排序" | 权重分配策略 |
| 1:15 | 代码截图 | "以将图文映射到同一向量空间" | CLIP等模型可 |
| 1:45 | 总结卡 | "记住三个词：图文检索本质是跨、权重分配策略、CLIP等模型可" | 收尾 |
