...
**30 秒电梯演讲：** 就像评价一把厨刀——不是看它每天被拿起来多少次（DAU），而是看用它切菜是否一次成功（完成率）、切出来的菜是否合格（采纳率）、切歪了能不能修正（恢复率）。

**展开框架：**
1. **任务完成率、回到** — 任务现场的效率和产物采纳率是核心
2. **看用户是否经常重** — 新补上下文或重跑整单
3. **多窗口任务丢失率** — 和错误恢复率也重要
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】你会用哪些指标判断桌面 AI 产品前端做得好 | "就像评价一把厨刀——不是看它每天被拿起来多少次（DAU），而是看用它切菜是否一次成功（完成率）、切出" | 引入 |
| 0:20 | 概念图解 | "任务现场的效率和产物采纳率是核心" | 任务完成率、回到 |
| 0:45 | 对比表格 | "新补上下文或重跑整单" | 看用户是否经常重 |
| 1:15 | 代码截图 | "和错误恢复率也重要" | 多窗口任务丢失率 |
| 1:45 | 总结卡 | "记住三个词：任务完成率、回到、看用户是否经常重、多窗口任务丢失率" | 收尾 |
