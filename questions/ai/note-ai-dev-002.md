...
**30 秒电梯演讲：** 像提高考试分数——换更好的学生(换大模型)只能提几分，但改进学习方法(数据工程)能从60分跳到85分

**展开框架：**
1. **结构化语义切分+上下文桥接** — 沿语义边界切分，chunk间保留上下文
2. **Query扩写+语义相似度阈值** — 用户意图不够时自动扩展+质量过滤
3. **混合检索用Lam** — 向量+BM25分数统一排名
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：大厂P7：RAG准确率从60%提升到85%的4个关键优化 | "像提高考试分数——换更好的学生(换大模型)只能提几分，但改进学习方法(数据工程)能从60分跳到85分" | 引入 |
| 0:20 | 概念图解 | "沿语义边界切分，chunk间保留上下文" | 结构化语义切分+上下文桥接 |
| 0:45 | 对比表格 | "用户意图不够时自动扩展+质量过滤" | Query扩写+语义相似度阈值 |
| 1:15 | 代码截图 | "向量+BM25分数统一排名" | 混合检索用Lam |
| 1:45 | 总结卡 | "记住三个词：结构化语义切分+上下文桥接、Query扩写+语义相似度阈值、混合检索用Lam" | 收尾 |
