...
**30 秒电梯演讲：** 就像公司运作——产品经理(Orchestrator)拆任务，开发(Worker A)、设计(Worker B)各做各的，通过项目管理工具(共享状态)同步进度，不需要互相看对方代码(上下文隔离)
...
**收尾：** 多Agent和单个Agent用多个工具调用的本质区别是什么？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：多Agent协作、上下文管理和任务状态传递 | "就像公司运作——产品经理(Orchestrator)拆任务，开发(Worker A)、设计(Work" | 引入 |
| 0:20 | 概念图解 | "中心化(Orchestrator) vs 去中心化(P2P)" | 协作模式 |
| 0:45 | 对比表格 | "每个Agent有独立上下文，通过消息传递共享必要信息" | 上下文管理 |
| 1:15 | 代码截图 | "共享黑板模式 / 消息队列 / 分布式状态" | 状态传递 |
| 2:15 | 总结卡 | "记住三个词：协作模式、上下文管理、状态传递" | 收尾 |
