...
**30 秒电梯演讲：** KV Cache像读书时做笔记——翻过的页不用重新读，只看笔记。PagedAttention像图书馆——不必每人搬一整套书，按页借阅用完归还。Continuous Batching像超市收银——不等所有人到齐才开收银台，来一个结一个
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：KV Cache的工作原理是什么？vLLM的PagedAtt | "KV Cache像读书时做笔记——翻过的页不用重新读，只看笔记。PagedAttention像图书馆" | 引入 |
| 0:20 | 概念图解 | "存储历史token的K/V矩阵，推理时只计算新token" | KV Cache |
| 0:45 | 对比表格 | "分块存储KV Cache，消除碎片化，显存利用率从60%→97%" | PagedAttention |
| 1:15 | 代码截图 | "请求级别动态调度，不同请求可随时加入/退出batch" | Continuous |
| 1:45 | 总结卡 | "记住三个词：KV Cache、PagedAttention、Continuous" | 收尾 |
