...
**30 秒电梯演讲：** 普通RAG像自动售货机（投币出货，固定流程），ReAct RAG像导购员（听需求→找商品→不够再找→推荐）。

**展开框架：**
1. **把retriev** — al作为Agent的一个tool
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：ReAct RAG Agent 如何实现？ | "普通RAG像自动售货机（投币出货，固定流程），ReAct RAG像导购员（听需求→找商品→不够再找→" | 引入 |
| 0:20 | 概念图解 | "al作为Agent的一个tool" | 把retriev |
| 0:45 | 对比表格 | "检索时机/查询构造/是否再次检索" | Agent自主决定 |
| 1:15 | 代码截图 | "多次检索串联）" | 解决多跳问答（需 |
| 1:45 | 总结卡 | "记住三个词：把retriev、Agent自主决定、解决多跳问答（需" | 收尾 |
