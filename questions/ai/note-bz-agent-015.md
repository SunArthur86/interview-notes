...
**30 秒电梯演讲：** 像公司项目组——产品经理(规划)、开发(执行)、测试(验证)、运维(部署)，各司其职，通过文档/会议(消息)协作。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何设计一个多 Agent 协作系统架构？ | "像公司项目组——产品经理(规划)、开发(执行)、测试(验证)、运维(部署)，各司其职，通过文档/会议" | 引入 |
| 0:20 | 概念图解 | "角色定义+通信机制+协作流程" | 核心要素 |
| 0:45 | 对比表格 | "主管-工人/辩论/流水线/对等协作" | 常见模式 |
| 1:15 | 代码截图 | "消息总线/共享黑板/直接调用" | 通信 |
| 1:45 | 总结卡 | "记住三个词：核心要素、常见模式、通信" | 收尾 |
