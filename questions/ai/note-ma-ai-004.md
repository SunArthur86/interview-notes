...
**30 秒电梯演讲：** 降低幻觉就像防漏水——水龙头拧紧(Prompt约束减少幻觉)、接水盆兜底(RAG提供事实)、湿度报警器(自动测试检测错误)、自动修补(Auto Fix修复)、最后人工检查(Human-in-the-loop)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【后端开发二面】如何降低大模型幻觉导致的错误代码？ | "降低幻觉就像防漏水——水龙头拧紧(Prompt约束减少幻觉)、接水盆兜底(RAG提供事实)、湿度报警" | 引入 |
| 0:20 | 概念图解 | "训练数据不足、Prompt模糊、Context缺失、温度过高" | 幻觉来源 |
| 0:45 | 对比表格 | "明确约束、few-shot示例、思维链(CoT)" | Prompt层 |
| 1:15 | 代码截图 | "RAG提供事实、规则引擎校验、类型检查" | 验证层 |
| 2:15 | 总结卡 | "记住三个词：幻觉来源、Prompt层、验证层" | 收尾 |
