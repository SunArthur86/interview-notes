...
**30 秒电梯演讲：** 像查谁连续打卡7天——给每天的打卡编个流水号（行号），再把日期转成数字序号。如果一个人连续打卡，日期序号和行号的差值会一直不变（因为两个都每天+1）；一旦断了一天，差值就变了。相同差值的连续打卡归为一组，数每组有几天就行。
...
**收尾：** 如果要找"连续登录至少N天且期间不能有一天中断"，怎么改？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【神州专车面经】手写 SQL：找出连续7天都有登录的司机ID | "像查谁连续打卡7天——给每天的打卡编个流水号（行号），再把日期转成数字序号。如果一个人连续打卡，日期" | 引入 |
| 0:20 | 概念图解 | "连续日期的"日期序号 - 行号 = 恒定分组键"" | 核心 |
| 0:45 | 对比表格 | "用date减row_number得grp，group by grp having count>=7" | ROW_NUMBER差值法 |
| 1:15 | 代码截图 | "判断相邻日期差是否=1" | LEAD/LAG法 |
| 1:45 | 总结卡 | "记住三个词：核心、ROW_NUMBER差值法、LEAD/LAG法" | 收尾 |
