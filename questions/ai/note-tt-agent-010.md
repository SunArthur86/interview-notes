...
**30 秒电梯演讲：** 就像银行的自动审批系统——小额转账自动通过（全自动），大额转账需要短信验证（人工确认），异常交易直接冻结（人工介入）
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Human-in-the-loop人工断点怎么设计？哪些环节 | "就像银行的自动审批系统——小额转账自动通过（全自动），大额转账需要短信验证（人工确认），异常交易直接" | 引入 |
| 0:20 | 概念图解 | "不可逆操作（下单/退款/删除）执行前暂停" | 操作前确认 |
| 0:45 | 对比表格 | "模型置信度低于阈值时转人工" | 低置信度审核 |
| 1:15 | 代码截图 | "连续失败/超时/安全风险时人工介入" | 异常兜底 |
| 1:45 | 总结卡 | "记住三个词：操作前确认、低置信度审核、异常兜底" | 收尾 |
