...
**30 秒电梯演讲：** 像双11的物流系统：白天订单少时从容处理，半夜突然涌入10万个包裹（潮汐流量）。方案：快递柜暂存(MQ削峰)→分拣线分批处理(任务分片)→分拣线坏了有备选方案(降级)→同一个包裹不重复配送(幂等)
...
**收尾：** MQ选型：Kafka vs RocketMQ vs RabbitMQ？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【快手AI大模型】异步批量AIGC任务架构怎么设计？多级降级 | "像双11的物流系统：白天订单少时从容处理，半夜突然涌入10万个包裹（潮汐流量）。方案：快递柜暂存(M" | 引入 |
| 0:20 | 概念图解 | "Kafka/RocketMQ缓冲突发请求，消费者按GPU容量消费" | MQ削峰 |
| 0:45 | 对比表格 | "按优先级/创作者等级分队列，优质创作者优先" | 任务分片 |
| 1:15 | 代码截图 | "任务ID去重，防止重复生成扣费" | 幂等保证 |
| 2:15 | 总结卡 | "记住三个词：MQ削峰、任务分片、幂等保证" | 收尾 |
