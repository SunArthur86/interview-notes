...
│  ┌───────────────────▼───────────────────┐               │
│  │ Memory Store:                         │ ← 跨会话      │
│  │ - User Profile (用户偏好/习惯)         │               │
│  │ - Environment (OS/工具/项目结构)       │               │
│  │ - Lessons Learned (经验教训)           │               │
│  └───────────────────┬───────────────────┘               │
│                      │                                   │
│  Layer 3: 程序性知识 (Procedural Knowledge)               │
│  ┌───────────────────▼───────────────────┐               │
│  │ Skills (可复用的工作流)                │ ← 跨会话      │
│  │ - 编码工作流 (test→lint→commit)       │               │
│  │ - 部署流程                             │               │
│  │ - API调用模板                          │               │
│  └───────────────────┬───────────────────┘               │
│                      │                                   │
│  Layer 4: 历史检索 (Session History)                     │
│  ┌───────────────────▼───────────────────┐               │
│  │ Session DB: 过往对话的FTS5全文检索     │ ← 跨会话      │
│  └───────────────────────────────────────┘               │
│                                                          │
│  特点: 多层持久化 | 跨平台 | Profile隔离                  │
└──────────────────────────────────────────────────────────┘
```
...
**30 秒电梯演讲：** Claude Code像便利贴——贴在项目文件夹上，每次打开就看到，简单有效但只在这一个地方有用。Hermes像一本个人笔记本+云端同步——分为不同分区(用户偏好/环境配置/工作记忆/技能手册)，走到哪带到哪，但需要分类管理。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Claude Code和Hermes在记忆系统上的差别 | "Claude Code像便利贴——贴在项目文件夹上，每次打开就看到，简单有效但只在这一个地方有用。H" | 引入 |
| 0:20 | 概念图解 | "项目级CLAUDE.md + 会话上下文 → 轻量、文件驱动" | Claude Code |
| 0:45 | 对比表格 | "Memory(持久) + Skills(程序性) + Session(会话) + Profile(隔离) → 多层" | Hermes |
| 1:15 | 代码截图 | "生命周期(临时/持久) + 粒度(全局/项目/会话) + 触发(主动/被动)" | 设计维度 |
| 2:15 | 总结卡 | "记住三个词：Claude Code、Hermes、设计维度" | 收尾 |
