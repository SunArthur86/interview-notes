...
**30 秒电梯演讲：** 数据并行像10个人各自买了同一本教材（冗余）；ZeRO-1让大家共用一套教辅（优化器分片），ZeRO-2再共用笔记（梯度分片），ZeRO-3干脆把教材也拆成10份每人拿一部分（权重分片）
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：DeepSpeed ZeRO-1、ZeRO-2和ZeRO-3 | "数据并行像10个人各自买了同一本教材（冗余）；ZeRO-1让大家共用一套教辅（优化器分片），ZeRO" | 引入 |
| 0:20 | 概念图解 | "分片优化器状态 → 节省约4倍显存" | ZeRO-1 |
| 0:45 | 对比表格 | "分片优化器状态+梯度 → 节省约8倍显存" | ZeRO-2 |
| 1:15 | 代码截图 | "分片优化器状态+梯度+权重 → 理论无限扩展" | ZeRO-3 |
| 1:45 | 总结卡 | "记住三个词：ZeRO-1、ZeRO-2、ZeRO-3" | 收尾 |
