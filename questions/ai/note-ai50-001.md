...
**30 秒电梯演讲：** 就像印刷厂裁纸——遇到跨页海报必须先拼合再裁剪，不能按A4纸一刀切下去把海报拦腰截断
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：工业PDF跨页大表格的切片处理策略 | "就像印刷厂裁纸——遇到跨页海报必须先拼合再裁剪，不能按A4纸一刀切下去把海报拦腰截断" | 引入 |
| 0:20 | 概念图解 | "检测表格边框线和跨页标记" | 基于规则的前置检测 |
| 0:45 | 对比表格 | "用Table Transformer或PaddleOCR做表格区域识别" | 布局分析模型 |
| 1:15 | 代码截图 | "跨页表格的垂直拼接、去重表头、对齐列" | 合并策略 |
| 1:45 | 总结卡 | "记住三个词：基于规则的前置检测、布局分析模型、合并策略" | 收尾 |
