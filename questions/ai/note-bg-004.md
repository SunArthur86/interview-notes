...
**30 秒电梯演讲：** PPO像一个"小心翼翼的登山者"——每步都用ratio裁剪防止走太远。GRPO像"登山小组投票"——不用单独的向导(Critic)，组内成员比较谁高谁低决定方向。DPO像"直接学评委的品味"——不学打分模型，直接从偏好对学。DAPO像...
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【八股总结】PPO、GRPO、DPO、DAPO 的基本原理 | "PPO像一个"小心翼翼的登山者"——每步都用ratio裁剪防止走太远。GRPO像"登山小组投票"——" | 引入 |
| 0:20 | 概念图解 | "on-policy，ratio裁剪，需要Critic估计value" | PPO |
| 0:45 | 对比表格 | "无Critic，组内G个样本相对排名做优势" | GRPO |
| 1:15 | 代码截图 | "无RM无RL，直接用偏好对的对比loss" | DPO |
| 2:15 | 总结卡 | "记住三个词：PPO、GRPO、DPO" | 收尾 |
