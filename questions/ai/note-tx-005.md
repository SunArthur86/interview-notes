...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【腾讯面经】你的 Memory 是多用户的吗？有没有做用户隔 | "记忆系统像酒店——每个客人有独立房间（用户隔离），房间有不同清洁频率：常住客人的房间保留物品（高频访" | 引入 |
| 0:20 | 概念图解 | "离两种方案" | 命名空间/行级隔 |
| 0:45 | 对比表格 | "有记忆隔离" | 公共知识共享+私 |
| 1:15 | 代码截图 | "RU+时间衰减+主动删除" | 遗忘=TTL+L |
| 2:15 | 总结卡 | "记住三个词：命名空间/行级隔、公共知识共享+私、遗忘=TTL+L" | 收尾 |
