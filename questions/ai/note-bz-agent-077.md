...
**30 秒电梯演讲：** 像高速公路收费站——令牌桶是发卡机(控制进入速率)、滑动窗口是监控(防拥堵)、熔断是封路(事故时隔离)、降级是绕行(走国道)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：大模型 API 的限流熔断如何设计？ | "像高速公路收费站——令牌桶是发卡机(控制进入速率)、滑动窗口是监控(防拥堵)、熔断是封路(事故时隔离" | 引入 |
| 0:20 | 概念图解 | "令牌桶(QPS)+滑动窗口(突发)" | 限流 |
| 0:45 | 对比表格 | "错误率/延迟超阈值自动隔离" | 熔断 |
| 1:15 | 代码截图 | "主模型→备模型→规则→缓存" | 降级 |
| 1:45 | 总结卡 | "记住三个词：限流、熔断、降级" | 收尾 |
