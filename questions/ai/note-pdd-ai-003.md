...
**30 秒电梯演讲：** 模型轻量化就像搬家——量化是把每件东西压缩打包(32bit→8bit)，蒸馏是请搬家公司把大件换成功能相同的小件(大模型→小模型)，剪枝是扔掉不需要的东西(去掉不重要的参数)，batch合并是拼车一次运多家的货(批量推理)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多 AI大模型开发】CPU轻量化部署：INT8量化、模 | "模型轻量化就像搬家——量化是把每件东西压缩打包(32bit→8bit)，蒸馏是请搬家公司把大件换成功" | 引入 |
| 0:20 | 概念图解 | "FP32→INT8，体积缩4倍，推理快2-3倍，精度损失<1%" | INT8量化 |
| 0:45 | 对比表格 | "Teacher大模型→Student小模型，保留90%能力体积减10倍" | 模型蒸馏 |
| 1:15 | 代码截图 | "去掉权重接近0的参数（结构化剪枝+非结构化剪枝）" | 剪枝 |
| 2:15 | 总结卡 | "记住三个词：INT8量化、模型蒸馏、剪枝" | 收尾 |
