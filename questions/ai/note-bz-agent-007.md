...
**30 秒电梯演讲：** 像红绿灯控制系统——状态机定义灯的颜色（红/黄/绿），事件循环是时钟，根据传感器（观察）和规则（决策）切换状态。

**展开框架：**
1. **调度核心** — 状态机建模+事件循环驱动
2. **LLM在状态转换** — 点做决策
3. **工具在状态内执行** — 结果作为事件触发转换
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Agent 的底层调度逻辑是什么？ | "像红绿灯控制系统——状态机定义灯的颜色（红/黄/绿），事件循环是时钟，根据传感器（观察）和规则（决策" | 引入 |
| 0:20 | 概念图解 | "状态机建模+事件循环驱动" | 调度核心 |
| 0:45 | 对比表格 | "点做决策" | LLM在状态转换 |
| 1:15 | 代码截图 | "结果作为事件触发转换" | 工具在状态内执行 |
| 2:15 | 总结卡 | "记住三个词：调度核心、LLM在状态转换、工具在状态内执行" | 收尾 |
