...
**30 秒电梯演讲：** 想象时钟的指针——绝对位置是时分针的角度，两个指针的夹角差(相对位置)才是我们关心的。RoPE就是把token放到时钟表盘上，通过旋转角度差自然表达相对距离

**展开框架：**
1. **RoPE** — = 绝对位置编码 + 自然得到相对位置关系
2. **通过旋转矩阵实现** — 不引入额外参数
3. **支持长度外推(配合NTK** — aware scaling)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：详细讲讲 RoPE 旋转位置编码？还有哪些位置编码？为什么用 | "想象时钟的指针——绝对位置是时分针的角度，两个指针的夹角差(相对位置)才是我们关心的。RoPE就是把" | 引入 |
| 0:20 | 概念图解 | "= 绝对位置编码 + 自然得到相对位置关系" | RoPE |
| 0:45 | 对比表格 | "不引入额外参数" | 通过旋转矩阵实现 |
| 1:15 | 代码截图 | "aware scaling)" | 支持长度外推(配合NTK |
| 1:45 | 总结卡 | "记住三个词：RoPE、通过旋转矩阵实现、支持长度外推(配合NTK" | 收尾 |
