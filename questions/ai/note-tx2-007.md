...
**30 秒电梯演讲：** 像写论文防抄袭/造假——①每个观点必须标引用(溯源)②来源不可靠的资料不用(阈值过滤)③请导师审核论文内容是否真有出处(事实校验)④先把资料筛一遍质量再读(Rerank)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【某讯面经】缓解 RAG 幻觉手段：溯源引用、相似度阈值过滤 | "像写论文防抄袭/造假——①每个观点必须标引用(溯源)②来源不可靠的资料不用(阈值过滤)③请导师审核论" | 引入 |
| 0:20 | 概念图解 | "答案标注来源chunk，无来源不输出" | 溯源引用 |
| 0:45 | 对比表格 | "低分chunk不喂LLM，避免硬编" | 相似度阈值过滤 |
| 1:15 | 代码截图 | "独立验证答案是否被检索内容支持" | 事实校验子Agent |
| 2:15 | 总结卡 | "记住三个词：溯源引用、相似度阈值过滤、事实校验子Agent" | 收尾 |
