...
**30 秒电梯演讲：** 想象一排灯泡开关（每个开关=1个bit=1个位置）。setbit(3, 1)就是找到第3个开关把它打开。底层是先算出第几个字节（开关在哪排），再用位运算精确控制那一个开关。

**展开框架：**
1. **Bitmap不是独立类型** — 基于String的位操作
2. **setbit** — 定位byte→位运算置1/清0，O(1)
3. **getbit** — 定位byte→移位取值，O(1)

**收尾：** Bitmap 和 Set 的内存对比？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Redis Bitmap 是怎么实现的？set/get 一个 | "想象一排灯泡开关（每个开关=1个bit=1个位置）。setbit(3, 1)就是找到第3个开关把它打" | 引入 |
| 0:20 | 概念图解 | "基于String的位操作" | Bitmap不是独立类型 |
| 0:45 | 对比表格 | "定位byte→位运算置1/清0，O(1)" | setbit |
| 1:15 | 代码截图 | "定位byte→移位取值，O(1)" | getbit |
| 1:45 | 总结卡 | "记住三个词：Bitmap不是独立类型、setbit、getbit" | 收尾 |
