...
**30 秒电梯演讲：** MySQL像按字典序排列的词典——拼音索引直接翻到页就有解释(聚簇)。PgSQL像普通书的索引——索引页指向正文页码需要翻过去(非聚簇)

**展开框架：**
1. **MySQL(In** — noDB)是聚簇索引，数据存在主键B+树叶子节点
2. **PgSQL是非聚簇索引** — 叶子节点存物理地址(CTID)
3. **MySQL主键查** — 询快(一次IO)，PgSQL支持复杂查询和JSON/向量

**收尾：** MySQL的二级索引为什么需要回表？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：MySQL和PgSQL的区别？分别用什么数据结构实现索引？ | "MySQL像按字典序排列的词典——拼音索引直接翻到页就有解释(聚簇)。PgSQL像普通书的索引——索" | 引入 |
| 0:20 | 概念图解 | "noDB)是聚簇索引，数据存在主键B+树叶子节点" | MySQL(In |
| 0:45 | 对比表格 | "叶子节点存物理地址(CTID)" | PgSQL是非聚簇索引 |
| 1:15 | 总结卡 | "记住三个词：MySQL(In、PgSQL是非聚簇索引、MySQL主键查" | 收尾 |
