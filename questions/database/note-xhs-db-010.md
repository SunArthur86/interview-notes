...
**30 秒电梯演讲：** 想象一个工具箱：String是万能刀（啥都能干），Hash是收纳盒（分格放东西），List是传送带（有序进出），Set是筛子（去重+交集），ZSet是奖牌榜（排序+带分数）。

**展开框架：**
1. **5大基础类型** — String/Hash/List/Set/ZSet
2. **每种类型有小编码** — (ziplist/intset)和大编码(hashtable/skiplist)两种
3. **String=缓** — 存/计数/锁；Hash=对象；List=队列；Set=去重/集合运算；ZSet=排行榜/延迟队列

**收尾：** ZSet 为什么用跳表而不是红黑树？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Redis 数据类型有哪些？各自的应用场景？（华为od Ja | "想象一个工具箱：String是万能刀（啥都能干），Hash是收纳盒（分格放东西），List是传送带（" | 引入 |
| 0:20 | 概念图解 | "String/Hash/List/Set/ZSet" | 5大基础类型 |
| 0:45 | 对比表格 | "(ziplist/intset)和大编码(hashtable/skiplist)两种" | 每种类型有小编码 |
| 1:15 | 代码截图 | "存/计数/锁；Hash=对象；List=队列；Set=去重/集合运算；ZSet=排行榜/延迟队列" | String=缓 |
| 1:45 | 总结卡 | "记住三个词：5大基础类型、每种类型有小编码、String=缓" | 收尾 |
