...
**30 秒电梯演讲：** 就像新华字典——HashTable是拼音索引(快速查字)，SkipList是页码排序(按序浏览)，薄字典用压缩排版(ListPack省空间)

**展开框架：**
1. **HashTabl** — e负责member→score的O(1)查找
2. **SkipList** — 负责按score排序和范围查询O(logN)
3. **小数据量用Lis** — tPack(Redis 7.0+)压缩省内存

**收尾：** SkipList为什么用多级链表而不是平衡树？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Redis的ZSet底层是怎么实现的？ | "就像新华字典——HashTable是拼音索引(快速查字)，SkipList是页码排序(按序浏览)，薄" | 引入 |
| 0:20 | 概念图解 | "e负责member→score的O(1)查找" | HashTabl |
| 0:45 | 对比表格 | "负责按score排序和范围查询O(logN)" | SkipList |
| 1:15 | 总结卡 | "记住三个词：HashTabl、SkipList、小数据量用Lis" | 收尾 |
