...
**30 秒电梯演讲：** 就像物流配送——多表JOIN是合并多个仓库的货物，优化就是让合并的中间货物尽量少(小表驱动大表)，慢查询排查就是看运输路线图(执行计划)找堵点

**展开框架：**
1. **EXPLAIN看执行计划** — type/key/rows/Extra四列是关键
2. **多表JOIN** — 小表驱动大表，优先JOIN过滤性强的表
3. **索引优化** — WHERE/JOIN/ORDER BY的列要有索引
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：SQL多表查询优化和慢查询排查 | "就像物流配送——多表JOIN是合并多个仓库的货物，优化就是让合并的中间货物尽量少(小表驱动大表)，慢" | 引入 |
| 0:20 | 概念图解 | "type/key/rows/Extra四列是关键" | EXPLAIN看执行计划 |
| 0:45 | 对比表格 | "小表驱动大表，优先JOIN过滤性强的表" | 多表JOIN |
| 1:15 | 代码截图 | "WHERE/JOIN/ORDER BY的列要有索引" | 索引优化 |
| 1:45 | 总结卡 | "记住三个词：EXPLAIN看执行计划、多表JOIN、索引优化" | 收尾 |
