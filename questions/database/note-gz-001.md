...
**30 秒电梯演讲：** 像寄快递有签收确认——寄出后没丢(幂等生产者)，分拣中心有登记(Checkpoint)，最终签收才确认完成(两阶段提交)，三个环节缺一不可

**展开框架：**
1. **Kafka端** — 幂等性生产者(防重) + 事务(跨partition原子写入)
2. **Flink端** — Checkpoint机制(一致性快照) + Barrier对齐
3. **Sink端** — 两阶段提交(2PC)保证输出也精确一次

**收尾：** Flink Checkpoint的Barrier对齐有什么问题？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：如何保证 Kafka 到 Flink 的数据不丢失、不重复( | "像寄快递有签收确认——寄出后没丢(幂等生产者)，分拣中心有登记(Checkpoint)，最终签收才确" | 引入 |
| 0:20 | 概念图解 | "幂等性生产者(防重) + 事务(跨partition原子写入)" | Kafka端 |
| 0:45 | 对比表格 | "Checkpoint机制(一致性快照) + Barrier对齐" | Flink端 |
| 1:15 | 代码截图 | "两阶段提交(2PC)保证输出也精确一次" | Sink端 |
| 1:45 | 总结卡 | "记住三个词：Kafka端、Flink端、Sink端" | 收尾 |
