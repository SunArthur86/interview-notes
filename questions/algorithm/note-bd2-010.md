...
**30 秒电梯演讲：** 就像书架上找书——HashMap是书名目录(O(1)找到位置)，双向链表是按"最近阅读"排列的书架。每次看书就把它移到最前面，满了就丢最后那本

**展开框架：**
1. **HashMap** — key→链表节点，O(1)查找
2. **双向链表** — 维护访问顺序，最近访问在head，最久未访问在tail
3. **get** — 查HashMap→移到链表head→返回值
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：手撕：力扣146.LRU缓存（带输入输出版本） | "就像书架上找书——HashMap是书名目录(O(1)找到位置)，双向链表是按"最近阅读"排列的书架。" | 引入 |
| 0:20 | 概念图解 | "key→链表节点，O(1)查找" | HashMap |
| 0:45 | 对比表格 | "维护访问顺序，最近访问在head，最久未访问在tail" | 双向链表 |
| 1:15 | 代码截图 | "查HashMap→移到链表head→返回值" | get |
| 1:45 | 总结卡 | "记住三个词：HashMap、双向链表、get" | 收尾 |
