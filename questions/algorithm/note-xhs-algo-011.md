...
**30 秒电梯演讲：** 就像一排人手拉手面朝前站着——反转就是让每个人转过身来，松开前面人的手，去拉后面人的手。关键是转换过程中要临时记住下一个人的位置（next指针），否则手一松就找不到人了。

**展开框架：**
1. **迭代法** — 三指针 prev/curr/next，每轮翻转一个节点
2. **递归法** — 递归到链表尾部，回溯时翻转指针
3. **时间复杂度O(n)** — 空间复杂度迭代O(1)/递归O(n)
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多一面】手撕算法：反转单链表 | "就像一排人手拉手面朝前站着——反转就是让每个人转过身来，松开前面人的手，去拉后面人的手。关键是转换过" | 引入 |
| 0:20 | 概念图解 | "三指针 prev/curr/next，每轮翻转一个节点" | 迭代法 |
| 0:45 | 对比表格 | "递归到链表尾部，回溯时翻转指针" | 递归法 |
| 1:15 | 总结卡 | "记住三个词：迭代法、递归法、时间复杂度O(n)" | 收尾 |
