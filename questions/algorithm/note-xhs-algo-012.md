...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多二面】手撕算法：合并两个有序数组 | "就像合并两排已经按身高排好的队伍——如果从前往后排（最小的先），矮的要插到前面会挤走后面的。但从后往" | 引入 |
| 0:20 | 概念图解 | "p1指向nums1有效数据末尾，p2指向nums2末尾，p指向合并后末尾" | 双指针从后往前填充 |
| 0:45 | 对比表格 | "对应指针左移" | 每次取较大的放到p位置 |
| 1:15 | 总结卡 | "记住三个词：双指针从后往前填充、每次取较大的放到p位置、nums2剩余元素需要拷贝" | 收尾 |
