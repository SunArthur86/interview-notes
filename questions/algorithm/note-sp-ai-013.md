...
**30 秒电梯演讲：** 像走迷宫找宝藏——每到一个路口(节点)就记下走过的路并减掉消耗，到死胡同(叶子)发现刚好到达目标就记录路线，然后退回上一步试别的路

**展开框架：**
1. **DFS前序遍历访** — 问每个节点
2. **进入节点** — targetSum减去当前节点值
3. **叶子节点且剩余和为0** — 记录路径

**收尾：** 如果路径不需要从根到叶子，而是任意节点到任意节点呢？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：手撕二叉树目标和（LeetCode 113. 路径总和 II | "像走迷宫找宝藏——每到一个路口(节点)就记下走过的路并减掉消耗，到死胡同(叶子)发现刚好到达目标就记" | 引入 |
| 0:20 | 概念图解 | "问每个节点" | DFS前序遍历访 |
| 0:45 | 对比表格 | "targetSum减去当前节点值" | 进入节点 |
| 1:15 | 总结卡 | "记住三个词：DFS前序遍历访、进入节点、叶子节点且剩余和为0" | 收尾 |
