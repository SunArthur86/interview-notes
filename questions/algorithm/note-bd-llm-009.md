...
**30 秒电梯演讲：** 就像书架上最常用的书放最前面——查书时从书架上取下(hashmap查找)放到最前面(链表头部)，满了就从最后面扔掉(淘汰尾部)。

**展开框架：**
1. **HashMap** — key→Node(O(1)查找)
2. **双向链表** — 维护访问顺序(O(1)移动)
3. **get** — 查找+移到头部
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】实现一个 LRU Cache，要求 get/pu | "就像书架上最常用的书放最前面——查书时从书架上取下(hashmap查找)放到最前面(链表头部)，满了" | 引入 |
| 0:20 | 概念图解 | "key→Node(O(1)查找)" | HashMap |
| 0:45 | 对比表格 | "维护访问顺序(O(1)移动)" | 双向链表 |
| 1:15 | 代码截图 | "查找+移到头部" | get |
