...
**30 秒电梯演讲：** 想象一排人手拉手面朝右（1→2→3→4→5）。反转就是让每个人转过身来拉住前面的人：第一个人拉住null，第二个人拉住第一个，以此类推。最后整排人变成了面朝左（5→4→3→2→1）。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：反转单链表（O(n)时间 O(1)空间）（华为od Java | "想象一排人手拉手面朝右（1→2→3→4→5）。反转就是让每个人转过身来拉住前面的人：第一个人拉住nu" | 引入 |
| 0:20 | 概念图解 | "prev/curr/next三指针，O(n)时间O(1)空间" | 迭代法 |
| 0:45 | 对比表格 | "递归到末尾再逐层翻转指针，O(n)空间" | 递归法 |
| 1:15 | 总结卡 | "记住三个词：迭代法、递归法、核心操作" | 收尾 |
