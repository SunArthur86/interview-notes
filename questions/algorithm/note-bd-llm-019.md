...
**30 秒电梯演讲：** 就像排队买票——窗口是当前不重复的人，新来的人如果和队列里的人重号，就从重号的人后面截断队列。

**展开框架：**
1. **左右双指针维护窗** — 口
2. **HashMap记** — 录字符最后出现位置
3. **右指针扩展遇到重** — 复→左指针跳到重复字符+1

**收尾：** 如果是字符串流(无限输入)怎么处理？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【字节面经】给定一个字符串流，实现一个滑动窗口，返回当前窗口 | "就像排队买票——窗口是当前不重复的人，新来的人如果和队列里的人重号，就从重号的人后面截断队列。" | 引入 |
| 0:20 | 概念图解 | "口" | 左右双指针维护窗 |
| 0:45 | 对比表格 | "录字符最后出现位置" | HashMap记 |
| 1:15 | 总结卡 | "记住三个词：左右双指针维护窗、HashMap记、右指针扩展遇到重" | 收尾 |
