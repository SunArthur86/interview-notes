...
**30 秒电梯演讲：** 就像查字典的页码——左边所有页码必须比当前页小，右边所有页码必须比当前页大。不能只看相邻页，因为中间穿插的页也可能乱序。

**展开框架：**
1. **方法1** — 中序遍历必须严格递增 → 最直观
2. **方法2** — 递归传递合法值域(min, max) → 最高效
3. **陷阱** — 只比较节点和直接子节点是错误的
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：验证二叉搜索树（LeetCode 98） | "就像查字典的页码——左边所有页码必须比当前页小，右边所有页码必须比当前页大。不能只看相邻页，因为中间" | 引入 |
| 0:20 | 概念图解 | "中序遍历必须严格递增 → 最直观" | 方法1 |
| 0:45 | 对比表格 | "递归传递合法值域(min, max) → 最高效" | 方法2 |
| 1:15 | 总结卡 | "记住三个词：方法1、方法2、陷阱" | 收尾 |
