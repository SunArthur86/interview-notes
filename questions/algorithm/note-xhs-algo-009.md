...
**30 秒电梯演讲：** 想象两个部门的组织架构图要合并——同一个岗位有两个人，那合并后的岗位人数=两个部门人数之和；如果只有一个部门有这个岗位，就直接搬过来

**展开框架：**
1. **两棵树同一位置节点值相加** — t1.val + t2.val
2. **如果一边为null** — 直接返回另一边（递归终止条件）
3. **DFS递归** — merge(t1.left, t2.left) 和 merge(t1.right, t2.right)

**收尾：** 如果要求不修改原始树（创建新节点），代码怎么改？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多 Java服务端】力扣617：合并两棵二叉树 | "想象两个部门的组织架构图要合并——同一个岗位有两个人，那合并后的岗位人数=两个部门人数之和；如果只有" | 引入 |
| 0:20 | 概念图解 | "t1.val + t2.val" | 两棵树同一位置节点值相加 |
| 0:45 | 对比表格 | "直接返回另一边（递归终止条件）" | 如果一边为null |
| 1:15 | 总结卡 | "记住三个词：两棵树同一位置节点值相加、如果一边为null、DFS递归" | 收尾 |
