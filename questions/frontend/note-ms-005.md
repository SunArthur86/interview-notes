...
**30 秒电梯演讲：** 就像公司管理——日常事务(页面层)各组自己管，但项目进度/交付物/资源分配(任务层)必须统一管理。

**展开框架：**
1. **输入框/浮层/选** — 中态留页面层
2. **任务状态/产物状** — 态/文件索引上升任务层
3. **长任务不绑单组件** — 生命周期

**收尾：** Pinia和Vuex在桌面端怎么选？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】Vue 做桌面 AI 产品时，哪些状态应该在 | "就像公司管理——日常事务(页面层)各组自己管，但项目进度/交付物/资源分配(任务层)必须统一管理。" | 引入 |
| 0:20 | 概念图解 | "中态留页面层" | 输入框/浮层/选 |
| 0:45 | 对比表格 | "态/文件索引上升任务层" | 任务状态/产物状 |
| 1:15 | 代码截图 | "生命周期" | 长任务不绑单组件 |
| 2:15 | 总结卡 | "记住三个词：输入框/浮层/选、任务状态/产物状、长任务不绑单组件" | 收尾 |
