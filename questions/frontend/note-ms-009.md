...
**30 秒电梯演讲：** 就像项目管理系统的核心对象——任务(做什么)、资源(用什么)、交付物(产出什么)、权限(谁能做)、记录(做了什么)。

**展开框架：**
1. **Task** — id/状态/优先级/类型
2. **Context** — 输入引用/摘要/历史版本
3. **Product** — 产物类型/路径/版本/预览
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】如果让你设计桌面 Agent 的任务中心，会 | "就像项目管理系统的核心对象——任务(做什么)、资源(用什么)、交付物(产出什么)、权限(谁能做)、记" | 引入 |
| 0:20 | 概念图解 | "id/状态/优先级/类型" | Task |
| 0:45 | 对比表格 | "输入引用/摘要/历史版本" | Context |
| 1:15 | 代码截图 | "产物类型/路径/版本/预览" | Product |
| 2:15 | 总结卡 | "记住三个词：Task、Context、Product" | 收尾 |
