...
**30 秒电梯演讲：** 就像项目管理工具——每个任务有自己的交付物列表，点进去可以预览、编辑、局部修改，而不是在聊天记录里翻找。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】如果一个 Agent 会生成站点、表格和 P | "就像项目管理工具——每个任务有自己的交付物列表，点进去可以预览、编辑、局部修改，而不是在聊天记录里翻" | 引入 |
| 0:20 | 概念图解 | "物类型展示" | 按任务分组再按产 |
| 0:45 | 对比表格 | "版本/可编辑入口" | 每个产物带来源/ |
| 1:15 | 代码截图 | "整单重跑" | 支持局部重生成不 |
| 2:15 | 总结卡 | "记住三个词：按任务分组再按产、每个产物带来源/、支持局部重生成不" | 收尾 |
