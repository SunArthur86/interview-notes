...
**30 秒电梯演讲：** 就像VS Code插件——核心编辑器不变，装一个插件就多一种语言支持。新增Agent就像装插件。

**展开框架：**
1. **Agent能力标** — 准化为Skill接口
2. **前端注册表动态加** — 载/卸载
3. **新增Agent=** — 新增插件不改主架构
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】如果产品要加更多 Agent，前端怎样保持扩 | "就像VS Code插件——核心编辑器不变，装一个插件就多一种语言支持。新增Agent就像装插件。" | 引入 |
| 0:20 | 概念图解 | "准化为Skill接口" | Agent能力标 |
| 0:45 | 对比表格 | "载/卸载" | 前端注册表动态加 |
| 1:15 | 代码截图 | "新增插件不改主架构" | 新增Agent= |
| 2:15 | 总结卡 | "记住三个词：Agent能力标、前端注册表动态加、新增Agent=" | 收尾 |
