...
**30 秒电梯演讲：** 就像游戏回放功能——不光看结果，还能回看每一步操作，甚至从某一步开始'如果换一个选择会怎样'(分支重跑)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】AI-Native 桌面产品怎样做结果回放， | "就像游戏回放功能——不光看结果，还能回看每一步操作，甚至从某一步开始'如果换一个选择会怎样'(分支重" | 引入 |
| 0:20 | 概念图解 | "具调用/中间结果" | 记录每步推理/工 |
| 0:45 | 对比表格 | "时间轴分步回看" | 时间轴分步回看 |
| 1:15 | 代码截图 | "支重跑" | 支持从任意步骤分 |
| 2:15 | 总结卡 | "记住三个词：记录每步推理/工、时间轴分步回看、支持从任意步骤分" | 收尾 |
