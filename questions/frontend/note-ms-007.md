...
┌──────────────────────────┐   ┌────────────────────────────────────────┐
│   命令解析器               │   │  自然语言解析器                           │
│   CommandParser           │   │  NLParser                               │
│                          │   │                                        │
│  · 词法分析(tokenize)     │   │  · 发送到LLM做意图识别                    │
│  · 语法校验(validate)     │   │  · 提取操作类型+参数                      │
│  · 参数补全提示(prompt)    │   │  · 置信度评估(confidence)                │
│  · 生成 Task              │   │  · 低置信度→降级为命令模式                 │
│                          │   │  · 生成 Task                            │
└───────────┬──────────────┘   └──────────────────┬─────────────────────┘
            │                                      │
            │         ┌──────────────┐             │
...
**30 秒电梯演讲：** 就像VS Code——可以用GUI点击(自然语言)，也可以用命令面板Ctrl+P(命令式)，两种方式操作同一个编辑器。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】如果产品要支持命令式输入和自然语言输入并存， | "就像VS Code——可以用GUI点击(自然语言)，也可以用命令面板Ctrl+P(命令式)，两种方式" | 引入 |
| 0:20 | 概念图解 | "用户" | 命令式适合高频老 |
| 0:45 | 对比表格 | "和复杂请求" | 自然语言适合探索 |
| 1:15 | 代码截图 | "任务模型" | 两类输入映射同一 |
| 1:45 | 总结卡 | "记住三个词：命令式适合高频老、自然语言适合探索、两类输入映射同一" | 收尾 |
