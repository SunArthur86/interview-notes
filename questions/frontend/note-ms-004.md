...
│                        用户交互层                                   │
│   ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐         │
│   │ 网页拖入  │  │ PDF上传   │  │ 截图粘贴  │  │ 文件选择  │         │
│   └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘         │
└────────┼─────────────┼─────────────┼─────────────┼──────────────┘
         │             │             │             │
         ▼             ▼             ▼             ▼
┌──────────────────────────────────────────────────────────────────┐
│                    第一层：来源对象层 (Source Layer)                │
│                                                                    │
│   ┌─────────────────────────────────────────────────────────┐     │
│   │  SourceObject Store                                     │     │
│   │  ┌──────────┬──────────┬──────────┬──────────┐         │     │
│   │  │ webpage  │ document │  image   │  file    │         │     │
│   │  │ {id,url, │ {id,path,│ {id,blob │ {id,path,│         │     │
│   │  │  title,  │  pages,  │  ref,thu │  mime,   │         │     │
│   │  │  fetched}│  meta}   │  mbnail} │  size}   │         │     │
│   │  └──────────┴──────────┴──────────┴──────────┘         │     │
│   │  职责: 记录素材来源 + 原始数据(大对象只存引用)            │     │
│   └─────────────────────────────────────────────────────────┘     │
└──────────────────────────────┬───────────────────────────────────┘
                               │ 异步提取
                               ▼
┌──────────────────────────────────────────────────────────────────┐
│                 第二层：提取结果层 (Extraction Layer)               │
│                                                                    │
│   ┌─────────────────────────────────────────────────────────┐     │
│   │  ExtractionResult Store                                 │     │
│   │  ┌──────────┬──────────┬──────────┬──────────┐         │     │
│   │  │ webpage  │ document │  image   │  file    │         │     │
│   │  │ →md文本  │ →分段+   │ →OCR文本 │ →解析    │         │     │
│   │  │ +结构化  │  表格+   │ +描述+   │  结果    │         │     │
│   │  │  摘要    │  摘要    │  摘要    │  +摘要   │         │     │
│   │  └──────────┴──────────┴──────────┴──────────┘         │     │
│   │  职责: 存储提取后的结构化内容 + 摘要(轻量)                │     │
│   └─────────────────────────────────────────────────────────┘     │
└──────────────────────────────┬───────────────────────────────────┘
                               │ 按需引用
                               ▼
┌──────────────────────────────────────────────────────────────────┐
│                第三层：任务引用层 (Task Reference Layer)            │
│                                                                    │
│   ┌─────────────────────────────────────────────────────────┐     │
│   │  ContextReference Store  (当前对话/任务实际消费的引用)    │     │
│   │                                                         │     │
│   │  Task A: [source#1 摘要] [source#3 全文]                │     │
│   │  Task B: [source#2 摘要] [source#3 摘要]                │     │
│   │                                                         │     │
│   │  职责: 记录每个任务引用了哪些来源的什么粒度(全文/摘要)     │     │
│   │  用户可勾选/取消引用 → 完全可控                           │     │
│   └─────────────────────────────────────────────────────────┘     │
└──────────────────────────────────────────────────────────────────┘
         │                                              │
         ▼                                              ▼
┌─────────────────┐                        ┌─────────────────────┐
│   AI 对话引擎     │ ◄──── 构建prompt ──── │  上下文组装器         │
│  (LLM Request)   │                        │ (按引用层组装token)  │
└─────────────────┘                        └─────────────────────┘
```
...
**30 秒电梯演讲：** 就像厨师备菜台——食材来源(冰箱/市场/菜园)、处理状态(洗好/切好)、正在做哪道菜(任务引用)，三层分开管理。

**展开框架：**
1. **来源对象/提取结** — 果/任务引用三层拆开
2. **用户可见哪些素材** — 正被消费
3. **大对象存引用和摘** — 要不重复拷贝
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】多模态输入同时有网页、文档和本地截图时，桌面 | "就像厨师备菜台——食材来源(冰箱/市场/菜园)、处理状态(洗好/切好)、正在做哪道菜(任务引用)，三" | 引入 |
| 0:20 | 概念图解 | "果/任务引用三层拆开" | 来源对象/提取结 |
| 0:45 | 对比表格 | "正被消费" | 用户可见哪些素材 |
| 1:15 | 代码截图 | "要不重复拷贝" | 大对象存引用和摘 |
| 2:15 | 总结卡 | "记住三个词：来源对象/提取结、用户可见哪些素材、大对象存引用和摘" | 收尾 |
