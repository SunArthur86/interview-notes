...
**30 秒电梯演讲：** 就像机场安检——先查证件格式(格式校验)，再查行李有没有违禁品(内容校验)，最后看登机口对不对(语义校验)。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】如果用户给了错误的文件上下文，前端怎样尽早发 | "就像机场安检——先查证件格式(格式校验)，再查行李有没有违禁品(内容校验)，最后看登机口对不对(语义" | 引入 |
| 0:20 | 概念图解 | "文件类型/大小/编码" | 格式校验 |
| 0:45 | 对比表格 | "空文件/损坏/格式错误" | 内容校验 |
| 1:15 | 代码截图 | "跟当前任务相关性预判" | 语义校验 |
| 2:15 | 总结卡 | "记住三个词：格式校验、内容校验、语义校验" | 收尾 |
