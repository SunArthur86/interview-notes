...
**30 秒电梯演讲：** 就像新闻发布流程——记者写稿(AI生成)→编辑审稿(用户确认)→排版发布(落地到文件)，不能记者写完直接发。
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【月之暗面面经】桌面端为什么要把结果编辑、确认和发布拆成三步 | "就像新闻发布流程——记者写稿(AI生成)→编辑审稿(用户确认)→排版发布(落地到文件)，不能记者写完" | 引入 |
| 0:20 | 概念图解 | "用户可修改AI产出" | 编辑 |
| 0:45 | 对比表格 | "审核后再进入下一步" | 确认 |
| 1:15 | 代码截图 | "落地到文件/站点/系统" | 发布 |
| 1:45 | 总结卡 | "记住三个词：编辑、确认、发布" | 收尾 |
