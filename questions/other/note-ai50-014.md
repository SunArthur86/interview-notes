...
**30 秒电梯演讲：** 就像服务员点餐——同步是等一道菜做完才点下一道(串行)，异步是同时点完所有菜然后等上菜(并发)，厨房(服务器)效率最大化

**展开框架：**
1. **asyncio是** — Python异步编程的标准库
2. **Agent中必须用异步的场景** — 并行工具调用、流式输出、多用户并发
3. **关键** — await是"让出控制权"，不是"等待"

**收尾：** asyncio和threading在Agent场景下怎么选？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Python异步编程在Agent开发中的应用 | "就像服务员点餐——同步是等一道菜做完才点下一道(串行)，异步是同时点完所有菜然后等上菜(并发)，厨房" | 引入 |
| 0:20 | 概念图解 | "Python异步编程的标准库" | asyncio是 |
| 0:45 | 对比表格 | "并行工具调用、流式输出、多用户并发" | Agent中必须用异步的场景 |
| 1:15 | 总结卡 | "记住三个词：asyncio是、Agent中必须用异步的场景、关键" | 收尾 |
