...
**30 秒电梯演讲：** 编解码器像"翻译官"。客户端和服务端都说着不同的"方言"——网络上只能传字节，但你的业务逻辑只懂 Java 对象。Decoder 把收到的字节"翻译"成对象给业务用；Encoder 把业务要发的对象"翻译"成字节发出去。Netty 还...

**展开框架：**
1. **编解码器=特殊的** — ChannelHandler,分离编解码逻辑与业务逻辑
2. **核心** — Decoder(decode)字节→对象;Encoder(encode)对象→字节
3. **关键基类** — ByteToMessageCodec/MessageToMessageCodec
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Netty 的编解码器（Codec）机制？ | "编解码器像"翻译官"。客户端和服务端都说着不同的"方言"——网络上只能传字节，但你的业务逻辑只懂 J" | 引入 |
| 0:20 | 概念图解 | "ChannelHandler,分离编解码逻辑与业务逻辑" | 编解码器=特殊的 |
| 0:45 | 对比表格 | "Decoder(decode)字节→对象;Encoder(encode)对象→字节" | 核心 |
| 1:15 | 代码截图 | "ByteToMessageCodec/MessageToMessageCodec" | 关键基类 |
| 2:15 | 总结卡 | "记住三个词：编解码器=特殊的、核心、关键基类" | 收尾 |
