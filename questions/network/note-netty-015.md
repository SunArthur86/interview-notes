...
**30 秒电梯演讲：** Netty 像"互联网基建的水管网络"——你平时用着各种 App（Dubbo、Spark、ES、RocketMQ），表面上看不到 Netty，但它们的底层通信都流淌在 Netty 这套水管里。就像你拧开水龙头有水，但你不会看到埋在地下...

**展开框架：**
1. **Netty是Ja** — va高性能通信的事实标准
2. **三大主战场** — 中间件通信底座/协议栈基础框架/各领域模块数据分发
3. **代表用户** — Apache/Twitter/Facebook/Cassandra/ES/Spark/Alibaba(Dubbo)/JD(JSF)

**收尾：** Dubbo 为什么选择 Netty 作为默认通信框架？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Netty 在工业界的应用场景？ | "Netty 像"互联网基建的水管网络"——你平时用着各种 App（Dubbo、Spark、ES、Ro" | 引入 |
| 0:20 | 概念图解 | "va高性能通信的事实标准" | Netty是Ja |
| 0:45 | 对比表格 | "中间件通信底座/协议栈基础框架/各领域模块数据分发" | 三大主战场 |
| 1:15 | 总结卡 | "记住三个词：Netty是Ja、三大主战场、代表用户" | 收尾 |
