...
**30 秒电梯演讲：** gRPC就像军队的加密对讲机——专用频段、压缩编码、双向通话，高效但需要双方都配设备；HTTP就像普通电话——通用、人话可读，但传输效率低

**展开框架：**
1. **gRPC** — = HTTP/2 + Protobuf序列化 + IDL（.proto文件）
2. **Protobuf** — 二进制序列化比JSON小3-10倍，解析快20-100倍
3. **HTTP/2多路** — 复用、头部压缩、双向流
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：【拼多多 Java服务端】AI助手项目接入层协议选型：HTT | "gRPC就像军队的加密对讲机——专用频段、压缩编码、双向通话，高效但需要双方都配设备；HTTP就像普" | 引入 |
| 0:20 | 概念图解 | "= HTTP/2 + Protobuf序列化 + IDL（.proto文件）" | gRPC |
| 0:45 | 对比表格 | "二进制序列化比JSON小3-10倍，解析快20-100倍" | Protobuf |
| 1:15 | 代码截图 | "复用、头部压缩、双向流" | HTTP/2多路 |
| 1:45 | 总结卡 | "记住三个词：gRPC、Protobuf、HTTP/2多路" | 收尾 |
