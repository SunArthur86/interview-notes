...
**30 秒电梯演讲：** 用点外卖来理解——BIO：坐在门口小板凳死等外卖小哥到；NIO：每隔一段时间去门口看一眼，没到就回来写代码；AIO：外卖到了小哥主动打电话叫你去拿。

**展开框架：**
1. **BIO=阻塞+一** — 连接一线程→并发上万时线程爆炸
2. **NIO=非阻塞+** — Selector事件通知→一个线程管多连接
3. **AIO=异步回调** — +OS完成后通知→真正异步(但Linux下epoll模拟,无优势)

**收尾：** Selector 的底层实现（epoll/poll/select）？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Java 三种 IO 模型 BIO / NIO / AIO  | "用点外卖来理解——BIO：坐在门口小板凳死等外卖小哥到；NIO：每隔一段时间去门口看一眼，没到就回来" | 引入 |
| 0:20 | 概念图解 | "连接一线程→并发上万时线程爆炸" | BIO=阻塞+一 |
| 0:45 | 对比表格 | "Selector事件通知→一个线程管多连接" | NIO=非阻塞+ |
| 1:15 | 代码截图 | "+OS完成后通知→真正异步(但Linux下epoll模拟,无优势)" | AIO=异步回调 |
| 1:45 | 总结卡 | "记住三个词：BIO=阻塞+一、NIO=非阻塞+、AIO=异步回调" | 收尾 |
