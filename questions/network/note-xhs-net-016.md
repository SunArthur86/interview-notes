...
**展开框架：**
1. **WebSocke** — t=全双工长连接，HTTP握手升级101
2. **解决HTTP轮询** — 的低效问题
3. **数据帧格式** — FIN+opcode+mask+payload

**收尾：** WebSocket 和 SSE（Server-Sent Events）的区别？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：WebSocket 主要是用来做什么的？如何设计实现一个 W | "HTTP像写信（你寄一封信等回信），WebSocket像打电话（接通后双方随时说话）。握手过程就是「" | 引入 |
| 0:20 | 概念图解 | "t=全双工长连接，HTTP握手升级101" | WebSocke |
| 0:45 | 对比表格 | "的低效问题" | 解决HTTP轮询 |
| 1:15 | 代码截图 | "FIN+opcode+mask+payload" | 数据帧格式 |
| 2:15 | 总结卡 | "记住三个词：WebSocke、解决HTTP轮询、数据帧格式" | 收尾 |
