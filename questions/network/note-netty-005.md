...
**30 秒电梯演讲：** 把 Netty 想象成一家餐厅。Channel 是"传送带"（连接通道，运数据），EventLoop 是"服务员"（一个服务员固定服务几张桌子的控制流），ChannelFuture 是"取餐号"（异步通知你菜好了）。ChannelHa...

**展开框架：**
1. **网络抽象三件套=** — Channel(连接)+EventLoop(控制流/并发)+ChannelFuture(异步通知)
2. **数据处理三件套=** — ChannelHandler(业务)+ChannelPipeline(处理链)+ByteBuf(数据)
3. **ChannelP** — ipeline编排ChannelHandler的顺序

**收尾：** Channel 和 EventLoop 是什么关系？
...
| 时间 | 画面/字幕 | 口播台词 | 讲解要点 |
|------|----------|----------|----------|
| 0:00 | 标题卡：Netty 的核心组件有哪些？ | "把 Netty 想象成一家餐厅。Channel 是"传送带"（连接通道，运数据），EventLoop" | 引入 |
| 0:20 | 概念图解 | "Channel(连接)+EventLoop(控制流/并发)+ChannelFuture(异步通知)" | 网络抽象三件套= |
| 0:45 | 对比表格 | "ChannelHandler(业务)+ChannelPipeline(处理链)+ByteBuf(数据)" | 数据处理三件套= |
| 1:15 | 代码截图 | "ipeline编排ChannelHandler的顺序" | ChannelP |
| 1:45 | 总结卡 | "记住三个词：网络抽象三件套=、数据处理三件套=、ChannelP" | 收尾 |
